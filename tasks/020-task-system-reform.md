...
3. **`.zcode/` 三文件同刻本地改毕（不入库，R1）**：三 Agent read-set 统一（删 TASKS.md 引用；park-shader 的「DECISIONS.md 中相关决策」改点名定向读取；procedural/park-shader 尾部「以当前 `tasks/*.md` 为准」改「当前任务书」）；`workflows/tree.md` 五点（read-set 表三列化 Path|Section|Mode 共 28 行 / Step 1 读取顺序 + 观察钩子 / 「索引只导航不承重」改「方法摘要 + 导航」/ 测试纪律五者判据 + 三门槛判据式引用 / §5 族级改七项骨架节）。
...
3. **第 11 项 16 软跳过清除**：12 ShapeSlots + 4 Lod（fraxinus/ligustrum/salix/sophora）的「动态 import + catch null + console.warn + expect(true).toBe(true)」并行交付期防御分支全部改为文件头静态 import 直接断言（import 失败即测试红——真缺陷摘除）；it 由 async 改同步、标题与头注释「未合并时优雅跳过」口径同步修正。收口 grep：tests/ 下 `expect(true).toBe(true)` 与「动态 import + catch null」守卫形态全归零（尚存 4 处 `await import` 为真实断言用法非防御守卫——tests/app/bootstrap.test.ts:622 / tests/editor/services/MeasureSession.test.ts:89 / tests/ui/panels/hierarchyModel.test.ts:99,107，合法保留）；11 Materials 残留「零碰撞」字样均为头注释去向记档句（指向 assetTaxonomy 收容断言）。
...
6. **边界追认记档（2026-09-23 收口会话）**：只读审核子代理逐块分类发现任务书第 9 项「13 份重复脚手架**合一**」与第 12 项 / R6 裁定「既有 66 文件**不迁移**」存在内部张力——本阶段实际按第 9 条宽读法执行（39 文件 harness 换线）。**用户审核后追认宽读法**，依据：①逐块分类零语义迁移（29 个 it 删除 = 11 碰撞块 + 16 软跳过守卫；净 −11 与 4034→4023 精确吻合）②13 份旧本地副本归一化 md5 字节级等价证明 ③三门槛独立复测全绿（4023 / 604 / 零错）④集中化断言覆盖面严格变宽（22 个注册资产全局唯一 vs 原 12 树种两两）。第 12 项据此收窄理解为「**断言层**等价重构另行立项」；本行即偏差申报与追认记录。
