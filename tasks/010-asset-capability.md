...
从夏栎代码中提炼可复制的「方法」（而非复制代码）：家族契约、文件组织规范、元数据与分类契约、LOD/Shadow 声明规范、视觉验收 SOP——使后续植物（及一切程序化资产）按模板批量生产。**不是建立独立的植物平台**：公共能力从真实消费者（夏栎 + 即将到来的第二棵树）中提炼。
...
依赖链：010.1 → 010.2 →（010.3 ∥ 010.4）→ 010.5。
