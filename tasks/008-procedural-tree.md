...
- [x] T008.5 T008 收官验收门（done 2026-09-19：全链路+同 sourceKey 合批三次实证（6 同槽 drawCalls 恒 7）+零错误五拆+8 槽终态复核全过；2542 全绿；**T008 收官 6/6**）→ [008.5-acceptance.md](008.5-acceptance.md)
...
- 2026-09-19 T008.5 完成（**6/6 epic 收官**）：主代理执行验收门——全链路（入库/缩略图/Ghost 真形态/放置/重掷 17 样本 8 槽全覆盖/undo-redo 16+16/复制孪生/Gizmo+undo/存档导出→真重载→导入逐位一致）+ 同 sourceKey 核心项三次独立场景实证（6 同槽树 drawCalls 恒 7 = 单 InstancedMesh×2 组×主/影，geometries 恒 5）+ 零错误五拆（console 零错误、Ghost 不 dispose、23 轮放删有界无泄漏；TransformControls gizmo 计数定性为 UI helper 非泄漏）+ 8 槽终态复核（mountSlots 42m 全景与 009.3 用户门一致）全过；三重门槛 2542 全绿。取证 `docs/acceptance/t008/008.5/`。**T008 树协议冻结为稳定基线**（T003 恢复/T011 族建设的前提）。
