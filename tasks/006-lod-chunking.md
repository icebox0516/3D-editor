...
- [x] T006.5 T006 验收门（done 2026-09-19：双档达标——城市 10 万路灯 175-190fps/531≤650、园区 2 万树 206-229fps+2 万灯 272-309fps@1080p（核显 BIOS 禁用，2080 Ti 实测 + 临界外推记档 + 遗留核显复验）；七项报表全项；范围门三项达标 Impostor 不立项；树 A/B 207↔17fps；换档序列单调无震荡 + diff 不超运动基线 + 窗口收口；LOD_THRESHOLDS/BATCH_POLICY 候选→锁定；2657 零回归）→ [006.5-acceptance.md](006.5-acceptance.md)
- [x] T006.6 LOD 选档稳定基准（Step 1 测试先行 + Step 2 稳定基准实装 done 2026-09-20〔Step 2 经用户指令提前于 T011〕；**Step 3 迁 T021.8 承接、任务 2026-09-23 收口〔D41〕**）→ [006.6-lod-reference-bounds.md](006.6-lod-reference-bounds.md)
...
- 2026-09-19 006.5 done（5/5，**epic 收官**）：双档验收达标（城市 175-190fps、园区 206-309fps——核显 BIOS 禁用环境限制如实记档 + 临界外推 24-42fps + 遗留核显复验）；范围门三项全绿 Impostor 不立项；树场景 A/B 207↔17fps（1.84 亿 → 638 万三角）为 LOD 治理价值终证；换档序列单调无震荡 + 帧间 diff 不超运动基线 + 视觉核验过——人工复核窗口收口（异议窗口开放）；LOD_THRESHOLDS 6/16/60/0.15 与 BATCH_POLICY 全量候选→锁定（代码/lod-spec/断言标注同步，数值不变零行为改动）；2657 零回归。遗留：核显复验、散布 seed 路由（D20.4 归 T003 解冻后）。
- 2026-09-20 D28 裁定追加 006.6（选档稳定基准 + GLB LOD 消费硬约束）：用户检查 LOD 系统发现的卫生债——选档基准取当前档几何包围球、三档 bounds 实际不等（≤0.4m 容差），漂移被 15% 迟滞带吸收无病症；演进方向 = High 档派生稳定资产级半径（D28.2），测试先行本会话完成（Step 1）。epic 5/5 收官状态与 006.5 锁定结论不变（Step 3 将判定阈值是否需重锁）。
- 2026-09-20 同日 006.6 Step 2 done（用户指令提前于 T011）：新增 `lodReference.ts` High 档派生基准球冻结缓存，两链选档换轨稳定基准（整球消费——放置链代表点 = 基准球心过实例矩阵；剔除不跟随、评估器/Renderer 零改动、零资产声明字段、`AssetSourceState.radius` 死字段删除）；+2 换档历史无关性测试 → 2850 全绿。余 Step 3（A/B 实测 + LOD_THRESHOLDS 重锁判定，预期不触发）。
- 2026-09-23 **D41 处置——T006 部分取代**：level 统一语义（含 `culled` 在类型联合、`levelInstanceKeep` 密度耦合、桶键 level 维度）由 **T021 LOD→Representation 运行体系**接替（运行面真相源 = `docs/procedural-assets/representation-runtime.md`，声明面 = `lod-spec.md`）；T006.1–6.5 + 006.6 Step 1/2 全部交付**保留为 T021 技术基线**（选档评估器 / 稳定基准球 / 分桶 / 批次 / 拾取 / 缓存所有权继续有效）；006.6 Step 3 迁 T021.8、任务收口。本行以上完成记录不回改（D31.1）。
