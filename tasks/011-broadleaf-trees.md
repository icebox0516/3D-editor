...
阔叶家族从单实例（夏栎）扩到四树种生产资产：朴树 / 香樟 / 榉树 / 银杏——每树种独立 Reference Spec（D26 Research Gate，**不沿用他树**）+ 阔叶家族契约实例化（数值各自锚定各自 Spec）+ LOD 三档 + Shadow 通道 + 8 槽形态向量全链；SOP 六步（shadow-visual-sop §5）逐树种走全——**011.3 起 Step 4 按 D30 增量口径**（011.1/011.2 按当时完整口径已毕，全量证据保留），epic 末统一验收门按 **D30 族级集体验收口径**收官（横向一致性 + 混植 + 性能 + 契约收口 + 回归，不重跑逐树已完成项），家族契约按真实缺口增量修订。
...
依赖链：011.1 → 011.2 → 011.3 → 011.4 → 011.5。011.1（首树）承担方法复制与契约缺口发现；后续树按固化流程走（**011.3/011.4 按 D30 增量口径**；顺序非硬功能依赖——各树文件集不相交，串行执行以维持会话粒度纪律与审查带宽；若发现阻塞级契约缺口，先行增量修订再继续）。
