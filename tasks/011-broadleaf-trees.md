...
> 立项：2026-09-19（TASKS.md 占位转正）。前置：T010 5/5 收官（启动条件确认见 010.5——SOP 六步 + asset-research 技能 + Spec 模板 + 家族契约与预算行 + taxonomy broadleaf 就绪）、T006 5/5 收官（Runtime LOD 调度就绪，族建设放量前调度已验收）。**状态：进行中（3/5，011.3 done 2026-09-20）**。

## Goal

阔叶家族从单实例（夏栎）扩到四树种生产资产：朴树 / 香樟 / 榉树 / 银杏——每树种独立 Reference Spec（D26 Research Gate，**不沿用他树**）+ 阔叶家族契约实例化（数值各自锚定各自 Spec）+ LOD 三档 + Shadow 通道 + 8 槽形态向量全链；SOP 六步（shadow-visual-sop §5）逐树种走全——**完整流程 ≠ 重复完整证据集**（D31.5）：**011.3 起 Step 4 按 D30 增量口径**（011.1/011.2 按当时完整口径已毕，全量证据保留），epic 末统一验收门按 **D30 族级集体验收口径**收官（横向一致性 + 混植 + 性能 + 契约收口 + 回归，不重跑逐树已完成项），家族契约按真实缺口增量修订。
...
- **家族契约实例化（organization.md §3）**：BroadleafShapeProfile 契约字段填各树数值（数值依据引自各自 Spec，夏栎数值不是家族真理）；契约缺口逐树记档——默认只记录、不提前泛化，**统一归 011.5 族级收口**；例外（D31.2）：缺口直接导致当前资产不修订就无法继续开发时，允许提前最小必要修订（仅解当前阻塞、不扩大公共抽象、完成记录留痕、011.5 再次复核；阻塞判定 = 主代理按「不修订则无法继续开发」执行判断，不建抽象定义）；修订连带夏栎逐位零回退（回归锁全过即证）。
...
- **视觉验收默认口径（D23.7 默认 + D30 分层，不设逐树用户门）**：011.3 起逐树走 D30 增量口径——Spec 身份判定记档 + 统一基线帧（SOP §5 Step 4：M25 + day 预设 + freezeTime）+ 契约缺口记档三必做，其余按疑点触发；校准默认一轮、两轮封顶，疑似结构性问题上收 011.5（011.2 envMap 先例）。机器判定为判据，用户观感复核窗口开放（基线帧 + 取证目录可回溯，008.6/009.7 处置先例）。理由：方法与视觉基调已由夏栎 T008/T009 + 011.1/011.2 两实例全量验证确立；树种辨识度对照各自 Spec「身份/识别特征」节判定，判据客观化。
...
## Acceptance（epic 级，011.5 执行——D30 族级集体验收口径，不重跑逐树已完成项）

- 四树横向一致性（基线帧同源比较）+ 树种间真实差异成立（对照各自 Spec「身份/识别特征」节）
- 混植场景（四树 + 夏栎同场）检视 + 资产库呈现（plant/broadleaf family 分组）
- 家族预算与 LOD：四树预算行复核 + 档间连续抽查（逐树层按需验项不重复跑）
...
- [ ] T011.5 T011 验收门（D30 族级集体验收：四树横向一致性 + 树种差异成立 + 混植 + 预算/LOD + 性能 + 契约修订 + 回归）→ 011.5-acceptance.md

依赖链：011.1 → 011.2 → 011.3 → 011.4 → 011.5。011.1（首树）承担方法复制与契约缺口发现；后续树按固化流程走（**011.3/011.4 按 D30 增量口径**；顺序非硬功能依赖——各树文件集不相交，串行执行以维持会话粒度纪律与审查带宽；若发现阻塞级契约缺口，按 D31.2 口径先行最小必要修订再继续）。
