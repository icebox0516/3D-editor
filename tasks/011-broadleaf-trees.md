...
> 立项：2026-09-19（TASKS.md 占位转正）。前置：T010 5/5 收官（启动条件确认见 010.5——SOP 六步 + asset-research 技能 + Spec 模板 + 家族契约与预算行 + taxonomy broadleaf 就绪）、T006 5/5 收官（Runtime LOD 调度就绪，族建设放量前调度已验收）。**状态：进行中（1/5，011.1 done 2026-09-19）**。
...
- [x] T011.1 朴树全链（done 2026-09-19：首树方法复制成立——BroadleafShapeProfile 零修改实例化无阻塞缺口；Spec 1.0 + 几何/材质/8 槽/LOD 三档/Shadow 全链 + 十项 9/9 过 + 2746 零回归；视觉核验通道故障与 IAB 直验处置记档）→ [011.1-celtis.md](011.1-celtis.md)
- [ ] T011.2 香樟全链（asset_tree_camphor，常绿阔叶——叶革质全缘/枝叶气味语义不建模）→ 011.2-camphor.md
...
- 2026-09-19 011.1 done（1/5）：朴树全链交付 + 十项判定 9/9 过 + 契约缺口清单记档（阻塞级无、共性候选 5 条归 011.5）；011.2 香樟解锁。
