...
| D16 | 有效 | 会话粒度不变；验收口径经 D23.7 人工门例外 + D30 分层细化 | 任务粒度 = 会话粒度 | `2026-09-16 · D16 任务粒度 = 会话粒度（grilling 会话裁定）` |
...
| D30 | 有效 | 第 2 条「基线帧参数 SOP §5」指针已修（2026-09-23 行内注 → shadow-visual-sop §3） | 资产验收分层 | `2026-09-20 · D30 资产验收分层：单资产增量验证 + 族级集体验收（grilling 两轮八题裁定）` |
