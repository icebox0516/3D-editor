...
Status 三值：**有效**（当前依据；含行内修订的条目仍属有效，修订内容见 Note）／**被取代**（整条已被后续裁定取代）／**历史**（保留为记录，不再作为当前依据）。**部分取代写 Note 列**，不新增子编号体系。Anchor = 标题原文（行号随追加失效，不作索引）。

**新增决策必须同步本表一行**（ID / Status / Note / Topic / Anchor）——漏登即索引腐化，检查脚本按「标题 D 号集合 ⊇ 索引 ID 集合」核对。
...
| D12 | 有效 | 行内修订：uTime 服务拉前至 T008.1（D19.7）；T005.1 瘦身为只剩水面 | 动画：轻通路 + 全套表现 | `D12 · 动画：轻通路 + 全套表现` |
| D13 | 有效 | — | 零外部资产边界 | `D13 · "0 外部资产"边界` |
| D14 | 有效 | — | 样式选择 = 可视化画廊 | `D14 · 样式选择 = 可视化画廊` |
| D15 | 有效 | 植物清单由 D19/D20 路线接续；同节后附「默认假设」块第 3 条测试基线数过时、第 4 条已由 D25 取代 | v1 内容清单 | `D15 · v1 内容清单` |
| D16 | 有效 | 会话粒度不变；验收口径经 D23.7 人工门例外 + D30 分层细化 | 任务粒度 = 会话粒度 | `2026-09-16 · D16 任务粒度 = 会话粒度（grilling 会话裁定）` |
| D17 | 有效 | 行内修订：第 2 项 build 签名已由 D19.2 扩为可选参（缓存键升 sourceKey） | 资产注册与生成契约 | `2026-09-16 · D17 程序化资产注册与生成契约（T002.1 grill 拷问门裁定）` |
...
| D27 | 有效 | 第 3 项经 D28.5 升格为硬约束；第 11 项 T006 排期由 D28.2 / D31.4 改写 | 通用 Asset Runtime LOD 架构 | `2026-09-19 · D27 通用 Asset Runtime LOD 架构（grilling 两轮 + 对抗审核，Q1–Q10 逐题裁定）` |
| D28 | 有效 | 第 2 项排期被同日用户指令提前（Step 2 已落地、Step 3 待做），D31.4 确认旧排期失效 | LOD 选档基准稳定化 | `2026-09-20 · D28 LOD 选档基准稳定化方向与 GLB LOD 消费硬约束（grilling 五题裁定）` |
...
| 默认假设 | 有效 | 第 3 条（1989 测试基线）过时、第 4 条（threejs-expert 派发）经 D25 失效；其余有效。无 D 号——D15 同级独立决策单元 | 未单独提问的默认执行项 | `默认假设（未单独提问，按最合理默认执行，可否决）` |
