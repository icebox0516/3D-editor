...
>
> **读取方式（2026-09-23 起）**：本文档不作全量加载——先查下方索引定位，再按标题锚点只读该条。索引只是导航：**Status/Note 是摘要，与条内正文冲突时以正文为准**。

---

## 索引（导航用，2026-09-23 建立）

Status 四值：**有效**（当前依据；条内自足的行内修订仍属有效，见 Note）／**部分取代**（该条部分条款已由后续裁定取代或废止、条内未整体重写——读该条前必看 Note）／**被取代**（整条已被后续裁定取代）／**历史**（保留为记录，不再作为当前依据）。不新增子编号体系。Anchor = 标题原文（行号随追加失效，不作索引）。

**新增决策必须同步本表一行**（ID / Status / Note / Topic / Anchor）——漏登即索引腐化；检查脚本按 **「标题 D 号集合 == 索引 ID 集合」双向核对**（漏登新决策与幽灵行都报）。

| ID | Status | Note | Topic | Anchor |
|---|---|---|---|---|
| D1 | 有效 | — | 第一性设计基准换轨 | `D1 · 设计基准换轨：第一性设计 + 逐子系统裁决` |
| D2 | 有效 | — | 文档五件套轻量记忆 | `D2 · 文档体系：五件套轻量记忆` |
| D3 | 有效 | — | 三层内容模型 | `D3 · 三层内容模型` |
| D4 | 有效 | — | 散布配方属于 Style | `D4 · 散布配方属于 Style` |
| D5 | 有效 | — | 散布派生式 + 可烘焙 | `D5 · 散布产物 = 派生式 + 可烘焙` |
| D6 | 有效 | — | 手动放置 = 烘焙变体 | `D6 · 手动放置 = 烘焙式实例变体` |
| D7 | 有效 | — | 程序化资产代码自注册 | `D7 · 程序化资产 = 代码自描述注册` |
| D8 | 有效 | — | CPU 撒点 + GPU 实例化 | `D8 · "GPU Scatter" = CPU 撒点 + GPU 实例化` |
| D9 | 有效 | — | 实例化架构：分块优先 | `D9 · 实例化架构：分块优先` |
| D10 | 有效 | — | 双档性能验收 | `D10 · 双档性能验收` |
| D11 | 被取代 | LOD 内容归 T009.6、切换归 T006（D23 第 1 条）；植物专属面数与统一距离阈值旧口径经 D23 第 3 条修正 | LOD 本期完整交付 | `D11 · LOD 本期完整交付` |
| D12 | 部分取代 | uTime 服务拉前至 T008.1（D19.7）、T005.1 瘦身为只剩水面——实由 D19 修订；条内自足部分仍有效 | 动画：轻通路 + 全套表现 | `D12 · 动画：轻通路 + 全套表现` |
| D13 | 有效 | — | 零外部资产边界 | `D13 · "0 外部资产"边界` |
| D14 | 有效 | — | 样式选择 = 可视化画廊 | `D14 · 样式选择 = 可视化画廊` |
| D15 | 部分取代 | 植物清单由 D19/D20 路线接续；同节另有**无 D 号的「默认假设」块**（位于 D15 与 D16 之间，故不占索引行）——其第 3 条测试基线数过时、第 4 条（threejs-expert 派发）已由 D25 废止 | v1 内容清单 | `D15 · v1 内容清单` |
...
2. **单资产层 = 增量视觉校准与资产身份确认**（「这棵树/这个资产对不对」），必做三件：① **Spec 身份判定**——对照 Reference Spec「身份/识别特征」节出结论并记档；② **统一基线帧**——全族同机位/同光照/同背景一张（参数定一次全族复用。**2026-09-23 指针修正**：原文写「参数 SOP §5 定一次全族复用」，该节已删除，参数现落 `docs/procedural-assets/shadow-visual-sop.md` §3），兼作用户观感复核窗口素材与族门横向比较同源基线；③ **契约缺口记档**（有无都记，修订归族门）。其余（多机位取证/探针/风动/console/LOD 档间视觉复查）**按需触发**——由身份判定疑点或开发 agent 自检信号触发；十项检查单在单资产层降格为**疑点对照清单**。
...
2. **Workflow 文件是完整生产流程编排的唯一规范落点**；docs/procedural-assets/ 仅保留各自领域的稳定工程规范与局部操作要求（含领域速查），不再定义独立完整的资产生产流程。〔遗留清理记档：shadow-visual-sop §5 六步 SOP 于后续 docs 清理轮改为对 workflow 的引用，本轮零改动。执行记档 2026-09-22 清理轮：§5 已标 Legacy（执行真相源 = asset-production Workflow，正式生产不得以该节为流程入口；基线帧参数已并入 §3），正式删除待 011.11 试点迁移完成〕。**执行记档 2026-09-23：§5 已删除**（原位留墓碑节 + 迁移映射；D23.7 两个人工作裁定门迁入 workflow §Step 4「人工裁定门的例外」；基线帧参数在 §3）
...
## 2026-09-22 · D38 调研层瘦身：Spec 两层化 + 终审采样 + 证据封顶（grilling 六题点选裁定，纯文档零代码）

**背景**：011.11 女贞完成后用户质疑调研流程冗余（调研文件十分之长、主代理终审还要识别大量图片、「几百个字还不能描述吗」）。事实核查：13 份树 Spec 全部 28–85KB（女贞 66KB），生产消费事实几百字可容，膨胀确诊三病灶——①「可追溯」实践走形为全文搬运：来源表整段重贴正文已逐句引用的原文 + 域扩展节复述正文 ≈26KB 纯重复；②会话调试日志混入事实文件：检索可达性 / 下载重试 / 槽位落空 / 工具误触记档 ≈4KB（违反 Spec「现实事实唯一来源」自身定义）；③证据采集与终审均无上限：18 张照片全量双问 ~40 次判读（终审后真实承重仅 7 张）+ 主代理终审 33 项硬数值逐位比对 + 6 张照片双问（其中过半查的是产地 / 用途 / 异名等永不进参数面内容）。终审机制本身有效（本次否证 form-cn 整树误判、修正冠形证据基础），问题在覆盖面超标。裁定：

1. **Spec 两层单文件**：头部「消费摘要」区（几百字——本次生产直接消费的全部现实事实，Step 1–4 默认只读本节）+ 压缩证据层（来源表只留锚句与端点信息不整段重贴原文、照片明细一行化、域字段内联标注于正文各节不设独立复述节）。**内容资格线**：只有「被 Step 1–4 消费的现实事实 + 其承重证据锚」有资格入 Spec，会话日志一律归任务完成记录。D26「唯一事实源」与 Evidence Status 两轴纪律不变。
2. **终审采样**：主代理终审抽样框 = 消费摘要区——硬数值只抽查直接进参数面的承重锚（≈5–8 项：叶尺度 / 果形果色 / 果期 / 树高档），照片只双问承重结构 / 比例主张槽位（整树 / 冠形 ≈2–3 张），其余信调研稿、保留随时抽查权。「疑点触发」方案否决——反例：form-cn 在调研稿中非自报弱项、属无痕「自信错误」，仅终审独立双问捕获。双问不一致即不承重、不取折中口径不变（photo-verification §1）。
3. **证据封顶**：承重照片槽 8–10 张双问（整树×2 / 冠×1 / 叶×1–2 / 果×1–2 / 皮×1，中国语境优先），其余槽位快筛不双问；季相 / 物候窗口主张靠文献双源承重、照片仅留 1–2 时点佐证；近缘差分链压至 2–3 条决定性判据，属内首例建档全链、后续同属资产引用不重查。整链视觉调用量约为原口径四成（52→~22 次）。
4. **终审落档改直写**：终审结论一行化（「终审：通过｜修正项｜日期」）落 Spec，修正**直接改写摘要区与正文对应条目**——不再追加文末附录节（011.11 教训：修正沉在附录，Step 1 需另行告知「以终审节为准」）；详细双问过程归任务完成记录。
5. **生效与冻结**：一次到位改四个落点（本条 + asset-research SKILL.md / spec-template.md + tree.md §3 Step 0 + photo-verification.md §2），下一棵树生效；既有 13 份 Spec 历史冻结不回改（D31.1 口径）。下一棵新格式树收尾时「Step 2 是否被迫下钻证据层」作为流程教训四问输入，替代试点阶段。
6. **分级档位本轮不做**：三层刀后首例全量档边际收益已小、且「轻到什么程度安全」暂无数据；等新格式树实跑 + 族门收口数据再议（经四问筛选器自然升格，D30 族门口径不变）。

---

## 2026-09-23 · D39 T018 过曝处置：显示域压缩选定，tone mapping 重审弃选（018.5 验收收口裁定）

**背景**：018.3 定论「Preetham HDR 天空 × NoToneMapping 大面积 clamp」——day（地面/受光物 254 饱和平台）与 tech（天空满白 + 金属反射过曝）双预设过曝，处置路径二选一（018.3/018.4 移交 018.5 收口）。裁定：

1. **选定显示域压缩**：`displayIntensity`（Sky 材质显示单侧 uniform，D29.13 硬约束路径）+ `iblIntensity`（scene.environmentIntensity 环境光贡献）双旋钮分治——天空观感与 PBR 环境光贡献可独立定值。终值：**day 0.22 / 0.15、tech 0.2 / 0.15**（四重依据：分离/组合扫描 + 视觉判读 + 视口分带定量 + 与 legacy 渐变天空逐位对齐），dusk/night 实测无过曝维持 1 / 0.85·0.35（dusk 白区经反打机位定案为低太阳正对机位的自然眩光非过曝）。预设面 `EnvironmentPreset.displayIntensity` 键 + 构建期应用落地，正式路径交付不依赖 DEV 面。
2. **弃选 tone mapping 重审**：D29.3（T018 ≠ Tone Mapping，NoToneMapping 不动）修订须用户裁定，018.5 不触发——显示域压缩已在既有架构面内达成可辨改善（day 天空带与 legacy 渐变逐位对齐、地面脱离饱和平台、阴影对比回归；tech 冷蓝渐变 + 金属阶梯恢复可辨），无证据表明必须重审。**ACES 实证证据（56% 画面改善）留作 T019 素材指针**：`docs/acceptance/t018/018.3/README.md`——若后续全局高光控制需求成立，T019 颜色管线立项时一并评估。D29.3 本体不修订。
3. **配套记档**：018.5 同会话裁定 p95 增量口径 = 同实例 IBL on/off 差（跨实例漂移 ≈+0.6ms 淹没信号；实测 Δ+0.1~0.2ms 双判据过，相对判据有效域 = 重载档，empty 小帧 regime 10% 阈值小于噪声不可测）。

## 2026-09-23 · D40 任务执行体系收敛：Lean Task + read-set 白名单 + 测试纪律（T020 grilling 逐题裁定）

**背景**：任务系统收敛 grilling 会话（2026-09-23 两会话逐题裁定，R1–R8 与两待裁项全录 `tasks/020-task-system-reform.md` 裁定记档表）确立新执行协议——**唯一来源（Workflow / Spec）→ 最小上下文（read-set 白名单）→ 新增语义验证（测试纪律）**。第一批历史卫生（`5aa70e1` / `bcd7f04` / `0bc08ae`）已清错误指针 / 双源 / 历史脏数据；本条切换当前执行规则。**切换纪律**：rule 层原子切换——tracked 文件一次提交 + `.zcode/` 同刻本地改毕（本地私有 R1），两批之间与切换中途不得存在「已生效但未同步到所有消费方」的新规则。**组织纪律**：本条单条 + 子节组织，细则不占独立 D 号——未来单点修订走新 D 号并在本条索引行 Note 指向（防两级地址歧义：细则占号会产生「D35.1 第 4 条」式引用，不可读）；索引粒度 = 可独立被取代的决策单元。

1. **任务书（Lean Task）**：任务书只装本次实例差异，不复述稳定生产流程（标准 Step 流程 / 通用派遣 / 通用调研方法 / 通用视觉 SOP / 通用 LOD·Shadow·Runtime 方法 / 三门槛定义 / 通用测试组织一律引用当前 Workflow / Skill / docs）。允许字段 = Epic / `workflow:` / `spec:`@版本 / 前置 / Goal=本次差异 / Research Gate 留痕 / 重点调研面 / 待裁决位 / Asset Scope / 特殊 Acceptance / 取证路径 / 完成记录（规范正文 = `docs/task-authoring.md`，模板 = `tasks/_template.md`，无 Step 复制区）；适用一切任务类型（非程序化资产专属——UI / 编辑器 / 调查任务同理）。先例索引定位 = **方法摘要 + 导航**（既非纯导航、也非事实源——数值与细节仍回原任务书 / Spec；D36 第 8 条「只做导航」按此修订）。检查面 = `npm run check:tasks`（`scripts/check-task-authoring.mjs`，v1 report-only 不硬失败，信号集：任务书出现 workflow/docs 同名固定段标题 / 与 AGENTS·workflow·docs 行级重合字节占比 / 基线数按章节报告〔Acceptance·Constraints → warning；完成记录 → normal；声明固定外部基线 → 允许〕+ 同文件 ≥2 个不同基线数算术交叉〔011.12「3245 vs 3803」实证腐例〕/ DECISIONS 索引「标题 D 号 == 索引 ID」双向核对；不做跨任务书比对——防合法资产事实重复误报）。
2. **Read-set 白名单（最小上下文）**：默认上下文 = AGENTS.md + 当前任务书 + 当前 Step 的 resolved read-set + Step 显式指定的 Skill / reference。Workflow 显式授权的按需先例读取不算违规；禁「可能相关」自行扩大；缺口先报主代理裁决。Workflow read-set 表三列化（Path | Section | Mode；**Section = 读取上限**，不设每 Step 文件数硬上限——无证据基础）；Step 1 读取顺序 = 先例索引（方法 / 差异 / 先例选择）→ 需承重数值才定点读 Spec / 任务书对应节 → 仍不足报主代理授权扩大，观察钩子 = 完成记录记 `index-only / point-read / full-read + reason`（记档非门槛，不设自动触发线）。三 Agent 定义（`.zcode/agents/*.md`，本地私有 R1）read-set 统一为「AGENTS.md + 当前任务书 + 当前 Step 的 resolved read-set」、删 TASKS.md 引用（D31.1：TASKS 仅导航不产生规则）；DECISIONS 定向读取 = 主代理派遣简报点名 Dxx → 索引定位只读该条，不全量加载；Agent 定义只管职责边界 / Skill 路由 / 交付格式 / 停止条件，不重定义任务体系。可观察面（R4）= 派遣简报三列 + Agent 完成时报 `Additional Read: N (+ reason)`；不做阅读审计日志、不做独立 audit。
3. **测试纪律（新增语义验证）**：新测试须对应五者之一——新增行为 / 新增结构·参数·材质语义 / 新任务新契约 / 新风险 / 公共能力新场景边界首现；已覆盖公共能力不得因「新增资产」复制测试；测试结构可参考先例、测试内容按新增语义裁定。自 T020 阶段二起新消费面按此执行（R6）；既有 66 个树测试文件不迁移（等价重构 33.9k 行是独立大任务，另行立项）；共享 harness 合一、O(n²) 键碰撞断言集中化、16 处软跳过清除归 T020 阶段二。
4. **三门槛判据式口径**：完成定义由「三重门槛全绿」改为判据式——**代码执行面变化（`src/**`、`tests/**`、`package.json`/lock、vite/vitest/tsconfig、构建·测试脚本）才必须 `npm test`；`check:layers` 与 `typecheck` 恒跑**。D16 验收分级与 D30 第 1 条「npm test 每任务必跑」的触发条件子句被本子节取代（历史原文不回写，D31.1）。
