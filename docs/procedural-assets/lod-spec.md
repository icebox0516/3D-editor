...
- 阈值数值 = Runtime 全局策略常量，**候选待 T006 实测锁定**（D27 候选栏——任何文档中的候选值不得当作锁定值）。
...
- 两者为候选策略（D27 候选栏），实际效果 T006 实测锁定；架构不锁死「整块同档」——需要时演进为 chunk 内多 Render Bucket（lod-reference §7 收录为演进方向）。
