...
预算单位 = 三角形实数（多材质组 = 各组之和）；High 锁单上限，Mid / Low 锁区间——**下限防「省面当目标」观感塌方**（003.4 教训：实交付 136–688 面远没用满预算）。Canopy 列 = 远景冠层代理表示预算（运行表示 `RuntimeRepresentation`，非构建档——声明只进 `representations` 不进 `levels`；含简化树干预柱，内容与预算语义见 `representation-runtime.md` §6.1/§6.2）。

| 家族 | High | Mid | Low | Canopy | 状态 | 锁定记录 |
|---|---|---|---|---|---|---|
| 阔叶乔木 broadleaf（夏栎第一实例） | ≤ 40000 | 6000–10000 | 1500–3000 | ≤ 500 | High/Mid/Low 已锁定 2026-09-19（T009.6）；Canopy 已锁定 2026-09-23（T021.6） | `asset_tree_3a.asset.ts` 模块头账目 + broadleafLod 测试不变量；Canopy = `tree/broadleafCanopyProxy.ts` 模块头账目 + broadleafCanopyProxy 测试不变量（13 树种 × tree3a 8 槽实测恒 487 = 干柱 35 + 冠卡 452〔70 双卡簇 × 4 + 86 单卡簇 × 2〕，簇位 < 156 才退化、实测无退化槽） |
