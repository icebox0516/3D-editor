...
- 基准半径现状与演进（D28，2026-09-20）：现状 `r` 取**当前档位**几何包围球（两链一致——放置随桶迁档换源、散布按当前档位键取缓存），依赖档间轮廓连续不变量（各档近似同值，资产侧测试锁容差——阔叶先例 ≤0.4m），迁档后半径换源的读数平移由 hysteresis 吸收；**已裁定演进方向 = High 档派生的稳定资产级半径**（T006.6 实装，剔除不跟随，A/B 实测记档义务）——实装后本条更新为稳定基准语义。
