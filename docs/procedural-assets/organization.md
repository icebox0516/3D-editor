...
2. **家族判定**：家族已有契约 → 实例化契约字段填自己的数值（数值依据引自本资产 Reference Spec，D26 不沿用他树）；家族无契约且**族已立项、经 Workflow 生产** → 首例创建**家族契约草案**——字段全部有真实消费点、全标【家族共性候选】，第二例真实消费后转正定稿（流程见对应 Workflow「新 Family 首例流程」，D37）；家族无契约且为非族简单资产 → 单资产先行，**不主动建家族层**（等真实第二消费者，D20.3）。
