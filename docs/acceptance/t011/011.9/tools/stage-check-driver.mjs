// T011.9 sophoraStage 合并后真实编译验证 CDP 驱动（011.8 tools/cdp.mjs 连接模式扩展：
// 自持 ws 事件流——Runtime.consoleAPICalled / exceptionThrown 全量捕获，供三档 GLSL
// 零错误零警告判定）。用法：
//   node docs/acceptance/t011/011.9/tools/stage-check-driver.mjs
// 前置：vite dev 5174 + Chrome --remote-debugging-port=9333。
import { writeFileSync } from 'node:fs';

const DEBUG_PORT = 9333;
const APP_URL = 'http://localhost:5207/';
const SHOT = 'D:/3D-editor/screenshots/t0119-sophora-stage-m25-tmp.png';
...
async function main() {
  ...
  // 等待 app 启动挂出 __sophora（DEV 守卫 + Renderer 装配）
  let booted = false;
  for (let i = 0; i < 40; i++) {
    booted = await evalJs('typeof window.__sophora !== "undefined"');
    if (booted) break;
    await sleep(500);
  }
  if (!booted) {
    return { ok: false, reason: 'window.__sophora 未出现（app 未启动或 DEV 守卫未过）', consoleEvents };
  }
  ...
  // Phase 2：三档横排 mountLevels（Mid/Low 材质 + 深度材质档位变体编译）
  await mark('mountLevels');
  await evalJs('window.__sophora.mountLevels()');
  await sleep(2500);
  const levelStats = await evalJs('JSON.stringify(window.__sophora.stats())');

  // Phase 3：回到 slot-0 单树 M25 基线取证（freezeTime 固定风相位）
  await mark('m25');
  await evalJs('window.__sophora.mount(); window.__sophora.view(); window.__sophora.freezeTime()');
  await sleep(800);
  const mountStats = await evalJs('JSON.stringify(window.__sophora.stats())');

  // 树高实测：页内动态 import 同一 asset build（与 mount() 同缺省 seed = slot-0 锚点，
  // 确定性逐位一致 → 即挂载树的实际组包围盒；测后资源即释放）
  const height = await evalJs(`(async () => {
    const m = await import('/src/runtime/procedural/assets/asset_tree_sophora.asset');
    const s = m.build();
    s.geometry.computeBoundingBox();
    const bb = s.geometry.boundingBox;
    const out = { maxY: bb.max.y, minY: bb.min.y, maxX: bb.max.x, maxZ: bb.max.z, minX: bb.min.x, minZ: bb.min.z };
    s.geometry.dispose();
  ...
  const x4000 = warnings.filter((w) => String(w).includes('X4000')); // FXC 保守误报——011.6 终裁口径接受记档
