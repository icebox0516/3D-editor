// dusk 第三轮：turbidity 为主因假设（totalMie ∝ turbidity × mieCoefficient）——低 turbidity × 标准 mie 组合
export default async ({ navigate, setViewport, evalJs, sleep, send }) => {
  ...
  const METRIC = `(async (b64) => {
    ...
    return { upper: band(0.03, 0.20), mid: band(0.20, 0.33), hor: band(0.33, 0.40) };
  })`;
  const { writeFileSync } = await import('node:fs');
  const log = { candidates: [] };
  // H: turbidity 3.5 标准 mie（总 Mie ≈ 0.7× 默认）；I: turbidity 2.5 + rayleigh 2.4（低浊高蓝）；J: turbidity 5 + mie 0.002
  const CANDIDATES = [
    { id: 'H', turbidity: 3.5, rayleigh: 2.2, mie: 0.003, g: 0.8, display: 1.0 },
    { id: 'I', turbidity: 2.5, rayleigh: 2.6, mie: 0.0035, g: 0.8, display: 1.0 },
    { id: 'J', turbidity: 5, rayleigh: 2.2, mie: 0.002, g: 0.8, display: 0.9 },
  ];
  for (const cand of CANDIDATES) {
    await evalJs(`window.__envProbe.setPreset('dusk')`);
    await sleep(700);
    await evalJs(`window.__sky.patchAtmosphere({ turbidity: ${cand.turbidity}, rayleigh: ${cand.rayleigh}, mieCoefficient: ${cand.mie}, mieDirectionalG: ${cand.g} }); window.__sky.setDisplayIntensity(${cand.display}); true`);
    await sleep(700);
    const r = await send('Page.captureScreenshot', { format: 'png' });
    writeFileSync(`../du3-${cand.id}.png`, Buffer.from(r.data, 'base64'));
