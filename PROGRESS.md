...
- **下一步**：T021.7 两链接线 + 编辑态 pin（全前置已满足：canopy 源路由 / representations 声明接线 / streetlamp 收编中间分支删除位）；其后 021.8 标定与验收门（吸收 T006.6 Step 3；A/B 复核组 = mid depth / canopy receive / low 实心影 / 合并桶 OR 短暂双投）。资产扩充线已定稿（2026-09-24 共识会话）：T022 清理 → T023 spine 提取 → T024 色卡预设 → T012–T015 植物四族（成员立项时勾选）→ T025 设施（轻量批量制）→ T026 人车 → T027 自然小物 → T028 建筑/设备（阶段顺序锁定；与 021 收尾的穿插排期待定；T016 已取消、T019 为 Tone Mapping 占位与色卡无关）。
