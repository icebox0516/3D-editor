...
- Workflow 文件正文数值零出现（流程参数化，非参数数据库）；Family 差异走 Workflow 内 Family 参数位，不动 Workflow 正文。
...
- 工程完成定义 = 三重门槛（`npm test` / `npm run check:layers` / `npm run typecheck`）全绿；执行模式按 DECISIONS 当前有效裁定。
