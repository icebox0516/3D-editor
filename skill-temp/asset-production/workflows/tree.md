...
| 族级规则与状态 | 所属 Epic（阔叶 = `tasks/011-broadleaf-trees.md`：族级契约规则 / 成员清单与增补规则 / 族级验收门 / 依赖与进度） |
...
- 调研执行：asset-research 技能 → 结论落盘 `docs/research/<asset>-reference.md`（通用八节骨架 + 域扩展节〔植物域 schema 见 asset-research references〕；Form / Evidence Status 两轴标注；Spec Version 头）。来源路由按 D32 / asset-research 检索节。
- **主代理终审**：硬数值独立抽查（独立重拉权威原句逐位比对，不抄调研稿）+ 照片终审按 `asset-research/references/photo-verification.md`（双问一致 / 加载确认 / 像素内参照物 / 字节溯源边界）；分歧取中庸；终审裁决记档追加于 Spec 文末。
...
- **三必做**：①Spec 身份判定——对照 Spec「身份 / 识别特征」节出结论记档（含与先例的横向对照读向）；②统一基线帧 `baseline.png`（固定机位 / day 预设 / freezeTime——参数与命名按 shadow-visual-sop §5 Step 4 与 §3）落 `docs/acceptance/<epic>/<子任务>/`；③契约缺口记档（有无都记，归族级验收门）。
...
| 4 | 主代理 | shadow-visual-sop.md §2–§3 + §5 Step 4；DECISIONS D30；当前任务书（疑点清单 / Acceptance） |
...
- **先例 / 知识索引同步**：对应索引存在则加行 / 补项（Family 参数位指定索引）——至少一条资产入口；产生新形态 / 材质 / 结构先例则补对应索引项；不灌水。
