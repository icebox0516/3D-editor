...
- **主代理终审**：硬数值独立抽查（独立重拉权威原句逐位比对，不抄调研稿）+ 照片终审按 `asset-research/references/photo-verification.md`（双问一致 / 加载确认 / 像素内参照物 / 字节溯源边界——双问不一致即该照片不承重处置并记档，不取折中）；终审裁决记档追加于 Spec 文末。
