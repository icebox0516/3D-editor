...
- [~] T011 阔叶乔木族生产任务集合（D33 成员集合口径；**D34 扩充后成员 12 树种：10 done / 2 排产（标准模式逐树推进——D35.1 复位后 011.7 单会话首跑、011.8/011.9/011.10 跨会话续跑交付〔011.10 含用户指示中间提交一次〕：单会话/主代理终审与验证/三门槛逐子任务 3689 全绿/566/零错）**；已 done：011.1 朴 / 011.2 樟 / 011.3 榉 + 011.4 银杏〔扇形 SDF 三件组合扩展+二叉脉+长短枝双挂点+圆锥窄冠〕+ 011.5 悬铃木〔掌状裂 SDF 新路径「叶基放射角窗 dip 族」+大叶疏簇+宿存果序首建模+树皮第六语言+12m 树高锚，零校准轮〕+ 011.6 栾树〔**复叶首例**：K. bipinnata 黄山栾树相 + 复叶卡 1 卡整枚二回羽叶 SDF 两级窗列 + 花果零 rng 确定性账目 + 树皮第 7 语言浅色光滑皮孔麻点〔主代理补证双系统验证〕+ X4000 FXC 误报终裁接受记档〕+ 011.7 乌桕〔**菱形 SDF 第七叶形** + 绿闭果零 rng 账目 + 树皮第 8 语言暗灰窄裂碎翘 + 开展圆头 9.5m 锚；跨 include 作用域 GLSL 编译错误事故修复+回归守卫；零校准轮〕+ 011.8 重阳木〔**三出复叶 SDF 第二型「三叶场并集法」**：放射对称三小叶同点放射 + 顶/侧长比 1.30–1.43 JS 锚 + 中距宽卵自检剖面 + 两段 scaffold + 树皮第 9 语言褐纵裂深沟宽脊扭转网状 + 果/花判定不做；Spec 终审通道降级三重替代口径如实记档；bisRidge GLSL 类型事故修复〔011.7 同型第二起〕；**X4000 未复现**（三出 1 卡整枚未触发——011.6 终裁边界实证）；零校准轮〕）+ 011.9 国槐〔**一回奇数羽状复叶 = 复叶第三型窗列单级化**：沿承 011.6 复叶卡路径——对生相位 + 顶生独立窗 + 裸轴 bare 双减法门控；念珠荚果零 rng 账目 49–63 串〔moniliform 九资产独有果型〕；树皮第 10 语言 6 板状厚脊 + 散在暗色瘤突独有维度 + 沟底红褐降级不做；Spec 终审通道发现〔iplant FRPS 按现用名索引〕+ FOC 未拆属证伪 + 照片双问 5 承重/2 不承重；全链三轮真实编译零 GLSL 事故〔011.8 纪律首正用例〕；**X4000 未复现**〔一回单级窗列——复叶系边界第三数据点〕；零校准轮；密度裁定细碎通透非缺叶归族门〕）+ 011.10 白蜡〔**复叶第四型 = 一回奇数羽状对生严格变体**：沿 011.9 单级窗列——右列降 0.05·space（vs 国槐近对生 0.18 相位压缩）+ 带数 2-3 对+顶生近等大 1.01-1.14×（计数少而整齐 vs 国槐 4-7 对）+ 裸轴 0.285 起排更长 + **缘锐锯齿 9 齿载波回归**（zelkova 先例）vs 国槐全缘〔复叶系缘齿对照轴闭合双样本〕；**decussate 对生挂点（对生系首例核心分化位）+ 三重对生**（复叶对生+小叶对生+芽对生）身份结构；匙形翅果帘幕簇零 rng 账目（满冠带 vs 国槐冠缘散点、50-67 簇——6m 判读 30-50 簇帘幕感+画面主导）；树皮第 11 语言（灰褐浅-中纵裂+幼干近光滑+皮孔小不明显——vs 栾密麻点/国槐板状厚脊瘤突）；树高 10.50m 志书「高10-12米」直给锚（profile 探针回调）；Spec 终审 34/34+照片双问 5/5 零不承重〔任务书「尖萼梣/大叶蜡树种下等级」表述证伪——锚定 subsp. chinensis 本尊〕；第十一实例化零修改；triangleCount 33920 精确吻合；VIEW_TARGET_Y 6.3（中位分歧冠底上抬记档）；两轮真实编译零 GLSL 事故〔材质 warp 虚构锚重验实质修正——「中断即未验证」第三树有效〕；**X4000 未复现**〔复叶系第四数据点〕；零校准轮〔中通透 B25 空隙 25-35% 精确落 Spec 域——族门密度第二对样本；L3.5 半透明孤证三帧仲裁=透射特性〕；**JS 镜像重测试 timeout 30000 机械加固 4 条**（全量满核并行饿超默认 5s——裁定更新记档）；缺口 6+7+1+族级 1 归 011.13〕→ **下一树 = 011.11 女贞（常绿第二例〔香樟后〕+ 革质叶——任务书预立核查后启动）→ 后续 011.12–011.13 照旧** → [tasks/011-broadleaf-trees.md](tasks/011-broadleaf-trees.md)
