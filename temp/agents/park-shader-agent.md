...
实现技术（SDF / 噪声分层 / 光照模型 / 微表面变化）从 `shader-dev` 按 Skill Routing 获取，不在本 Agent 重复。

项目不变量：

* 不为简单纹理引入外部资源、不增加不必要的贴图依赖
* 避免大量高频噪声与复杂但无法解释的随机效果
...
按问题加载 Skill，不把 GLSL 技术教程复制进本 Agent，不默认加载全部：

```text
web-shaders + shader-dev
    默认技能
    web-shaders：WebGL2、GLSL、ShaderMaterial / RawShaderMaterial、浏览器 Shader 排错
    shader-dev：SDF、Noise、Lighting、Water、Shadow、Procedural Shader 技术

threejs-procedural-materials
    仅研究 NodeMaterial/TSL 或程序化材质架构时

threejs-visual-validation
    固定机位取证，或需要视觉 / 机制 / 性能证据时

threejs-debugging
    仅具体 Three.js / Shader Runtime 故障时
```

threejs-perf 不作为常驻技能。

## Verification

Shader 修改至少通过 AGENTS.md 三重门槛（npm test / check:layers / typecheck），命令不在此重复。
