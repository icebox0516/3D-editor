/**
 * tests/runtime/procedural/tree/ginkgoShapeSlots.test.ts —— 银杏 8 槽形态向量表测试
 * （T011.4，对称榉树 zelkovaShapeSlots 组织——方法复制；数值锚为银杏自己的实测）。
 *
 * 覆盖（零 mock——真实几何生成；GINKGO_SHAPE_PROFILES 八槽完整组合 = 「8 组向量即
 * config 雏形」的契约锁，与 ginkgoStructure.test.ts（slot-0 结构语义）互补）：
 ...
 *   （扇形宽>高反向口径 Spec [1][2][7]）与 barkRelief（槽间恒等——非形态差异维度）
 ...
 *   使同 seed 对比弱于榉树 1.56，规范种子比 5.89/3.82 ≈1.54 为主读数，记档）；低冠
 *   vs 高冠叶卡最低 Y（视觉冠底——干高工程默认域的槽维度展开）差 ≥ 0.9m（实测
 *   ≈0.95m）；疏松 vs 丰满叶卡数比 ≤ 0.85（实测 ≈0.82——crown_transparency 25–35%
 *   带内疏密两端）；偏冠 vs 标准——6 种子面板质心均值比 ≥ 1.15（实测 ≈1.20——
 *   excurrent 强领导中轴稀释偏冠读向、弱于榉树 1.51，记档归 011.13 缺口候选）；规范
 *   种子下偏冠槽质心绝对 ≥ 0.4m 且 ≥ 1.35× 标准槽（实测 0.45m / ×1.44）；8 槽叶卡数
 *   （规范种子）不全相同且各落预算卡域 [(27000−20782)/2, (40000−20782)/2]；
 * - 物种锚点（规范种子 slot-0）：总高落 7.5–8.8m 锚域、视觉冠底/实高落工程默认域带
 *   [0.28, 0.37]（Unknown·无实证 per 终审 C-3 ±容差）、bbox 冠幅比落工程域 0.55–0.65
 *   （Step 3 探针定档 0.581——冠幅参数校准的实测验收锁）；
 * - 通透不变量不破（规范种子逐槽）：外壳（q>0.6）保留率 ≥ canopyDensity×0.85（槽自
 *   适应）且 ≥ 内核（q<0.4）保留率 ×1.8（外密内疏，Spec crown_fill_gradient Inferred
 *   [7]）；每槽总面数（皮+叶）落 High 档预算带 [27000, 40000]（家族行 High 上限语义，
 *   带下沿为银杏扇形大卡口径参考——见资产模块头记档）。
...
const ASSET_ID = 'asset_tree_ginkgo';
...
function buildSlot(slot: number): GinkgoGeometryResult {
  const result = buildGinkgoGeometry(mulberry32(SEEDS[slot]!), GINKGO_SHAPE_PROFILES[slot]!);
  built.push(result);
  return result;
}

/** 几何直调（指定种子 = 同 seed 跨槽对比——隔离 profile 效应） */
function buildSlotAt(slot: number, seed: number): GinkgoGeometryResult {
  const result = buildGinkgoGeometry(mulberry32(seed), GINKGO_SHAPE_PROFILES[slot]!);
  built.push(result);
  return result;
}
...
function leafMetrics(result: GinkgoGeometryResult): LeafMetrics {
...
describe('结构计数恒等（8 槽皮面数与 rng 消费恒等的前提）', () => {
  ...
  it('配置侧锁：结构计数类字段 / 叶卡尺寸域 / barkRelief 逐位同 slot-0；canopyDensity ≤ 1', () => {
    const anchor = GINKGO_SHAPE_PROFILES[0]!;
    expect(GINKGO_SHAPE_PROFILES).toHaveLength(8);
    for (let slot = 1; slot < 8; slot++) {
      const p = GINKGO_SHAPE_PROFILES[slot]!;
...
describe('确定性（同 seed 同槽逐位复现）', () => {
  it('代表槽（0/3/7）同 seed 两次构建 position 逐位相等 + 全属性 + stats 全等（全 8 槽确定性由 ginkgoStructure slot-0 锁 + 结构计数恒等 + 本文件路由/差异测试间接覆盖——全量 16 次构建裁剪为 6 次，控制全量套件并行负载，记档）', () => {
...
describe('槽间真实差异（同 seed = SEED0 跨槽对比——差异纯来自形态向量，非 seed 运气）', () => {
  ...
  it('挺拔 vs 展开：XZ 包围盒宽度比 ≥ 1.15（实测 ≈1.18——近轮生 + 强领导冠宽底宽的同 seed 弱化；规范种子比 ≈1.54 主读数，见 slot-1 注释；Princeton Sentry 窄端 vs Autumn Gold 阔圆锥端）', () => {
    const narrow = m(1);
    const broad = m(2);
    expect(narrow.xzWidth).toBeGreaterThan(0);
    expect(broad.xzWidth / narrow.xzWidth, '展开/挺拔 宽度比').toBeGreaterThanOrEqual(1.15);
  }, 120000);

  it('低冠 vs 高冠：叶卡最低 Y（视觉冠底）差 ≥ 0.9m（实测 ≈0.95m——干高工程默认域的槽维度展开）', () => {
    const low = m(4);
    const high = m(5);
    expect(high.minLeafY - low.minLeafY, '高冠-低冠 视觉冠底差').toBeGreaterThanOrEqual(0.9);
  }, 120000);

  it('疏松 vs 丰满：叶卡数比 ≤ 0.85（疏松显著少——crown_transparency 25–35% 带内疏密两端；实测 ≈0.82）', () => {
    const sparse = m(6);
    const full = m(7);
    expect(sparse.cards / full.cards, '疏松/丰满 卡数比').toBeLessThanOrEqual(0.85);
  }, 120000);

  it('偏冠 vs 标准：6 种子面板质心均值比 ≥ 1.15（实测 ≈1.20——excurrent 强领导中轴稀释偏冠读向、弱于榉树 1.51，记档归 011.13 缺口候选）；规范种子偏冠槽绝对 ≥ 0.4m 且 ≥ 1.35× 标准槽（实测 0.45m / ×1.44）', () => {
  ...
  it('8 槽叶卡数（规范种子）不全相同且各落预算卡域 [3110, 9609]（总面带 [27000,40000] 折算）', () => {
    const counts = new Set<number>();
    for (let slot = 0; slot < 8; slot++) {
      const { stats } = buildSlot(slot);
      counts.add(stats.leafCards);
      expect(stats.leafCards, `slot-${slot} ${SLOT_NAMES[slot]} 叶卡数应 ≥ (27000−20782)/2`).toBeGreaterThanOrEqual(3110);
...
describe('物种锚点（规范种子 slot-0——Spec 尺度与比例域）', () => {
  it('总高 ∈ [7.5, 8.8]、bbox 冠幅比 w/h ∈ [0.55, 0.65]、视觉冠底/实高 ∈ [0.28, 0.37]（工程默认域 ±容差·无实证 per 终审 C-3；实测 h 8.17 / w/h 0.581 / 冠底 0.328——2026-09-20 探针）', () => {
    const r = buildSlot(0);
    ...
    expect(height, 'slot-0 总高应落 ≈8m 锚域（工程锚弱推断 per 终审 C-1）').toBeGreaterThanOrEqual(7.5);
    expect(height).toBeLessThanOrEqual(8.8);
    // 最窄冠读向：冠幅比落工程域 0.55–0.65（园艺域 + fall 实测支撑——终审 C-2；冠幅
    // 参数 Step 3 探针定档 0.55 的实测验收锁，见 ginkgoShapeProfile crownWidthRatio 注释）
    expect(width / height, 'slot-0 bbox 冠幅比应落工程域 0.55–0.65（Spec §3/终审 C-2）').toBeGreaterThanOrEqual(0.55);
    expect(width / height).toBeLessThanOrEqual(0.65);
    const crownBase = metrics.minLeafY / height;
    expect(crownBase, 'slot-0 视觉冠底/实高应落工程默认域带（Unknown·无实证 ±容差）').toBeGreaterThanOrEqual(0.28);
    expect(crownBase).toBeLessThanOrEqual(0.37);
  }, 120000);
});

describe('通透不变量与预算带（规范种子逐槽）', () => {
  it('外壳（q>0.6）保留率 ≥ canopyDensity×0.85（槽自适应）且 ≥ 内核（q<0.4）保留率×1.8（外密内疏）；总面数落 [27000, 40000]', () => {
    for (let slot = 0; slot < 8; slot++) {
      const { stats } = buildSlot(slot);
      const profile = GINKGO_SHAPE_PROFILES[slot]!;
      ...
      // 外密内疏读向（crown_fill_gradient Inferred [7]）
      ...
      expect(total, `slot-${slot} ${SLOT_NAMES[slot]} 总面数应 ≥ 27000（银杏扇形大卡口径参考下沿——低于夏栎 28754 不阻塞，实测锁定见资产模块头）`).toBeGreaterThanOrEqual(27000);
    }
  }, 120000);
});
