/**
 * tests/runtime/procedural/tree/camphorStructure.test.ts —— 香樟枝干结构真实性测试
 * （T011.2，对称朴树 celtisStructure 组织——方法复制，数值锚为香樟自己的实测）。
 *
 * 覆盖（零 mock——真实几何生成；直调 buildCamphorGeometry 消费扩展 stats，与资产入口
 * 契约（asset_tree_camphor.asset）互补——本文件锁「结构怎么长」，那边锁「契约怎么传」）：
 * - 确定性：同 rng seed + 同 shapeProfile 两次构建 → 全属性逐位相等、stats 账目全等；
 *   rng 消费恒等 147058（消费顺序即契约——次数变化 = 随机流重排）；
 ...
 *   （宽 0.06–0.095m / 长宽比 1.8–2.4 落 profile 域——香樟叶身份 Spec [1][2][5][6]）；
 * - 贴地契约：minY 精确 0（原点 = 底部中心）；
 * - 树皮起伏确定性：主干环截面起伏存在（极差/环均径带 + 厘米级深沟量级——纵裂深沟
 *   Verified [1][2][6]）、沿枝级递减（主干强末梢弱）、wrap 位浮点级无缝；
...
import { CAMPHOR_SLOT0_PROFILE } from '../../../../src/runtime/procedural/tree/camphor/camphorShapeProfile';

const SEED0 = morphSeedOf('asset_tree_camphor', 0);
/** slot-0 High rng 消费快照（探针实测——任何条件跳过消费都违约） */
const RNG_CALLS_LOCK = 147058;

const built: CamphorGeometryResult[] = [];

function buildTracked(seed: number, profile = CAMPHOR_SLOT0_PROFILE): CamphorGeometryResult {
  const result = buildCamphorGeometry(mulberry32(seed), profile);
  built.push(result);
  return result;
}
...
describe('确定性（shapeProfile 路径 + rng 消费恒等）', () => {
  ...
  it('profile 缺省 = slot-0 标准组合显式传入（锚点回落路径逐位一致）', () => {
    const a = buildCamphorGeometry(mulberry32(SEED0));
    const b = buildTracked(SEED0, CAMPHOR_SLOT0_PROFILE);
    built.push(a);
...
describe('冠内通透（显式规则：通道 / 内层密度衰减 / 局部空腔）', () => {
  ...
  function cardRootMidpoints(result: CamphorGeometryResult): { x: number; y: number; z: number }[] {
  ...
  it('外密内疏：内核（q<0.4）叶卡保留率显著低于外壳（q>0.6），外壳近满（crown_fill_gradient）', () => {
    ...
    expect(shellRet, '外壳保留率应近满（常绿满密冠壳——Spec §3 Verified [6]）').toBeGreaterThanOrEqual(0.9);
...
describe('枝梢驱动叶簇（家族方法沿用：枝梢 → 簇空间 → 叶片分布）', () => {
  it('簇-枝梢绑定：簇数 > 0、L5 末梢挂簇占主导；簇中心贴挂点（cm 级）；簇方向 = 枝切向单位向量；簇账目守恒', () => {
    ...
    expect(stats.clusters.length + stats.clustersCulled).toBe(
      stats.levelBranches[4]! * CAMPHOR_SLOT0_PROFILE.clustersL5 + stats.levelBranches[3]! * CAMPHOR_SLOT0_PROFILE.clustersL4,
    );
  ...
  it('shellBias 外偏：存活卡簇内归一化半径均值 ≥ 0.6（叶沿簇壳分布、簇内稀疏成腔）', () => {
    ...
    expect(mean, '存活卡 r̂ 均值应 ≥ 0.6（shellBias=0.66/γ=0.8 外偏）').toBeGreaterThanOrEqual(0.6);
  }, 30000);

  it('每簇叶量域 + 卡几何域：逐簇存活数 ≤ 每簇预算且总数守恒；卡宽/长宽比落 profile 域（香樟叶身份 Spec Verified/Inferred）', () => {
    const result = buildTracked(SEED0);
    const { stats } = result;
    const budget = Math.max(CAMPHOR_SLOT0_PROFILE.clusterLeavesL5, CAMPHOR_SLOT0_PROFILE.clusterLeavesL4);
    ...
    for (let base = leaf.start; base < leaf.start + leaf.count; base += 6) {
      ...
      const aspect = len / w;
      if (
        w < CAMPHOR_SLOT0_PROFILE.leafWidthMin - 1e-6 ||
        w > CAMPHOR_SLOT0_PROFILE.leafWidthMin + CAMPHOR_SLOT0_PROFILE.leafWidthSpan + 1e-6 ||
        aspect < CAMPHOR_SLOT0_PROFILE.leafAspectMin - 1e-6 ||
        aspect > CAMPHOR_SLOT0_PROFILE.leafAspectMin + CAMPHOR_SLOT0_PROFILE.leafAspectSpan + 1e-6
      ) {
        bad++;
      }
    }
    expect(bad, '卡宽应 ∈ [0.06, 0.095]、长宽比 ∈ [1.8, 2.4]（profile 域）').toBe(0);
  }, 30000);
});

describe('树皮起伏确定性（纵裂深沟语言——amplitude 0.036 / {3,4,6} / drift 3.2）', () => {
  /** 皮组某管某段 ring A 的顶点索引（a0 = 每四边形首顶点；发射序 = 主干 → 底盖 → L1 → 深度优先首链） */
  function ringRadii(result: CamphorGeometryResult, base: number, seg: number, radial: number): number[] {
    const pos = result.geometry.getAttribute('position');
  ...
  const TRUNK_RADIAL = CAMPHOR_SLOT0_PROFILE.trunk.radial;
  const TRUNK_SEGS = CAMPHOR_SLOT0_PROFILE.trunk.segs;
  ...
  it('主干全部环截面极差/环均径 ∈ [2%, 12%]，最大极差 ≥ 12mm（厘米级深沟——中-强浮雕 Spec bark_relief Verified [6]）', () => {
    const result = buildTracked(SEED0);
    ...
    for (let seg = 0; seg < TRUNK_SEGS; seg++) {
      ...
      expect(range / mean, `主干环 ${seg} 极差/均径应 ≥ 2%（深沟起伏可辨）`).toBeGreaterThanOrEqual(0.02);
      expect(range / mean, `主干环 ${seg} 极差/均径应 ≤ 12%（中-强浮雕端不夸张）`).toBeLessThanOrEqual(0.12);
    }
    expect(maxAbs * 1000, '主干最大极差应 ≥ 12mm（厘米级——基径 0.26–0.32m × flare × 幅度比 3.6%）').toBeGreaterThanOrEqual(12);
  }, 30000);
  ...
  it('起伏幅度沿枝级递减：主干极差 > L1 首环极差 > L5 首环极差；L5 绝对量 < 1mm 级（亚视觉地板 → 圆管）', () => {
    ...
    const l1 = ringRadii(result, L1_BASE, 0, CAMPHOR_SLOT0_PROFILE.levels[0]!.radial);
    const l1Range = Math.max(...l1) - Math.min(...l1);
    const l5 = ringRadii(result, L5_BASE, 0, CAMPHOR_SLOT0_PROFILE.levels[4]!.radial);
    const l5Range = Math.max(...l5) - Math.min(...l5);
    expect(trunkMax, '主干极差应大于 L1').toBeGreaterThan(l1Range);
    expect(l1Range, 'L1 首环极差应 ≥ 2mm（粗枝仍有纵裂起伏）').toBeGreaterThanOrEqual(0.002);
...
describe('贴地契约与参数面驱动', () => {
  ...
  it('canopyDensity 压低 → 叶卡数显著下降（同 seed 同拓扑，密度场真实消费）', () => {
    ...
    const sparse = buildTracked(SEED0, { ...CAMPHOR_SLOT0_PROFILE, canopyDensity: 0.25 });
