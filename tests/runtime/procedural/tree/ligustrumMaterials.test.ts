 ...
 *   ginkgo/platanus/koelreuteria/triadica/bischofia/sophora/fraxinus 十先例 90 键
 *   零碰撞（十一先例中 tree3a 为一期 plant: 键系不在 level 扫描面）；叶 Mid 去脉
...
const sdfOf = (fragmentShader: string): string => {
  const start = fragmentShader.indexOf('float lguLeafAlpha(vec2 lguUv, float lguRand)');
...
describe('分档实装（level 参数；缺省 high = 显式 high）', () => {
  ...
  it('与 zelkova/camphor/celtis/ginkgo/platanus/koelreuteria/triadica/bischofia/sophora/fraxinus 十先例 90 键零碰撞（ligustrum 前缀不与十一先例混缓存——tree3a 为一期 plant: 键系不在 level 扫描面）', () => {
    ...
    expect(foreignKeys.size).toBe(90); // 十先例 × 3 工厂 × 3 档（含 011.10 白蜡 81 键面 + fraxinus 9 键）
