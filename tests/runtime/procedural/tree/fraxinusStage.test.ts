/**
 * tests/runtime/procedural/tree/fraxinusStage.test.ts —— window.__fraxinus DEV
 * 出图面句柄测试（T011.10；sophoraStage.test 同构复制——断言集逐条对应，数值面换
 * 白蜡实数）。
 *
 ...
 *   皮 + 匙形翅果帘幕簇并入**——皮拓扑恒 24178 + 翅果 2244（561 果×4）= 26422，vs
 ...
 *   24178 + 翅果簇并入皮组 2004–2712 tri（501–678 果×4，确定性账目零 rng——簇数随槽
 *   簇位平滑变化）→ 组 0 带 26182–26890 + 翅果 ≡0 mod 4 结构不变量（果 ×4 tri），vs
 ...
 * 数值锚（vs sophora 差异面）：slot-0 High 组 0 皮+翅果 **26422**（皮拓扑恒 24178 +
 *   匙形翅果 561 果×4 = 2244 并入皮组——果域 uv v∈[4,5] 冻结接口，triadica/sophora
 *   先例）/ 复叶卡 7498 tri / 3749 卡（资产预算锁定账目 triangleCount 33920 =
 *   26422 + 7498；复叶卡 = 2 tri/卡承载整枚一回奇数羽状复叶——1 卡 = 对生窗列小叶 +
 *   顶生小叶的 SDF 层表达，翅果双 quad 4 tri/果非卡面不入 leafCards）；取景视心高
 *   6.3（slot-0 冠域 2.17–10.50 冠心 ≈6.334——vs platanus 7.5 / sophora 5.9 /
 *   koelreuteria 5.9 / triadica/bischofia 5.8 / camphor 5.5 / ginkgo 5.4 / zelkova
 *   5.3 / celtis 4.3，≈10.5m 级与 ≈10m 队列同档高半档——**冠心上探成因 = 中位分歧
 *   冠底上抬（2.17 vs 国槐 1.45）非树高差**，fraxinusStage 模块头「树高参考」）。
 */
import { describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { createFraxinusHandle } from '../../../../src/runtime/procedural/tree/fraxinus/fraxinusStage';
 ...
 *  （白蜡 build 自 T011.10 起返回深度材质；不带字段的 makeFakeBuild 即回退路径） */
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（组 0 皮+翅果 26422 / 叶 7498 / 3749 卡，T011.10 探针实测 = 预算锁定账目 slot-0 High——皮 24178 恒定 + 翅果 561 果×4 并入皮组）', () => {
    const scene = new THREE.Scene();
    const handle = createFraxinusHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(26422);
    expect(stats.leafTriangles).toBe(7498);
    expect(stats.leafCards).toBe(3749);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置冠心 + update 被调', () => {
    ...
    const handle = createFraxinusHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈6.3（锚点树 10.50m 冠域 2.17–10.50 冠心 ≈6.334——中位分歧冠底上抬档，vs platanus 7.5 / sophora 5.9 / triadica/bischofia 5.8）
    expect(camera.position.x).toBeCloseTo(32, 5);
    expect(camera.position.y).toBeCloseTo(6.3, 5);
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createFraxinusHandle({ scene });
    const b = createFraxinusHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('fraxinus-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('fraxinus-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createFraxinusHandle({
    ...
    const bare = createFraxinusHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：fraxinus-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createFraxinusHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('fraxinus-dev-slots');
  ...
  it('stats（slots 模式）：8 项逐槽账目，组 0 **槽间平滑带**——皮恒 24178 + 翅果簇并入 2004–2712 tri → 带 26182–26890 + 翅果 ≡0 mod 4 结构不变量（果 ×4 tri；确定性账目零 rng——簇数随槽簇位平滑变化，vs bischofia 无花果资产组 0 恒等记档）+ 顶层合计 8 棵和；复叶卡数只断言 > 0（不锁具体值——数值归资产侧测试）', () => {
    const scene = new THREE.Scene();
    const handle = createFraxinusHandle({ scene });
    handle.mountSlots();
    const stats = handle.stats();
    ...
    for (let slot = 0; slot < 8; slot++) {
      ...
      // 组 0 = 皮拓扑恒 24178 + 匙形翅果帘幕簇并入（双 quad 4 tri/果非卡面）——翅果挂点
      // 确定性账目零 rng，簇数随槽平滑变化 → 组 0 平滑带 26182–26890（8 槽实测带 = 501–678 果）
      expect(entry.barkTriangles).toBeGreaterThanOrEqual(24178 + 2004);
      expect(entry.barkTriangles).toBeLessThanOrEqual(24178 + 2712);
      expect((entry.barkTriangles - 24178) % 4).toBe(0); // 翅果 = 果 ×4 tri 结构不变量（正反双 quad）
      expect(entry.leafCards).toBeGreaterThan(0); // 复叶卡数随簇级剔除定（8 槽实测带 2564–4394）——不锁具体值
    ...
    expect(stats.barkTriangles).toBeGreaterThanOrEqual((24178 + 2004) * 8);
    expect(stats.barkTriangles).toBeLessThanOrEqual((24178 + 2712) * 8);
    expect(stats.leafCards).toBe(stats.slots!.reduce((sum, s) => sum + s.leafCards, 0));
    handle.dispose();
  }, 300000);

  it('viewSlots：缺省机位球坐标公式复算落位 + target = 网格中心 (0, 6.3, spacing/2) + update 被调；viewSlot(5) 绕槽 5 树位；viewSlot(9/-1) 越界 warn no-op 不位移', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const handle = createFraxinusHandle({ scene, camera, controls });
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('fraxinus-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createFraxinusHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('fraxinus-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('fraxinus-dev-levels');
  ...
  it('viewLevels：缺省机位（32/35/16）球坐标公式复算 + target = 排中心 (0, 6.3, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createFraxinusHandle({ scene, camera, controls, build: makeFakeBuild(log) });
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createFraxinusHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createFraxinusHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
