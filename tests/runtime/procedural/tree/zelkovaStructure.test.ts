/**
 * tests/runtime/procedural/tree/zelkovaStructure.test.ts —— 榉树枝干结构真实性测试
 * （T011.3，对称香樟 camphorStructure 组织——方法复制，数值锚为榉树自己的实测）。
 *
 * 覆盖（零 mock——真实几何生成；直调 buildZelkovaGeometry 消费扩展 stats，与资产入口
 * 契约（asset_tree_zelkova.asset）互补——本文件锁「结构怎么长」，那边锁「契约怎么传」）：
 * - 确定性：同 rng seed + 同 shapeProfile 两次构建 → 全属性逐位相等、stats 账目全等；
 *   rng 消费恒等 150046（消费顺序即契约——次数变化 = 随机流重排）；
 ...
 *   拓扑保险）；外密内疏——内核保留率显著低于外壳（crown_fill_gradient Inferred [8]）；
 *   枝干通道——存活叶卡挂点（根边中点 = 过滤判定口径）距通道线段带内数为 0；通透不
 *   挖空整层——冠顶/冠底 1/3 高度带均有存活叶卡；
 * - 枝梢驱动叶簇（家族方法沿用）：簇-枝梢绑定（簇中心贴挂点 + 簇方向 = 挂点枝切向
 *   单位向量 + L5 末梢挂簇占主导）；簇间间隙；shellBias 外偏；每簇叶量域与卡几何域
 *   （宽 0.04–0.06m / 长宽比 1.8–2.4 落 profile 域——榉树细质密叶身份 Spec
 *   [1][2][3][4][8]）；
 * - 贴地契约：minY 精确 0（原点 = 底部中心）；
 * - 树皮起伏确定性（光滑基底微起伏语言——amplitude 0.015 / {3,5,6} / drift 11）：主干
 *   环截面起伏存在（极差/环均径带 + 毫米级微起伏量级——bark_relief 低-微 Inferred
 *   [7][8]，vs 香樟厘米级深沟断言带收窄）、沿枝级递减（主干强末梢弱）、wrap 位浮点级
 *   无缝；
...
import { ZELKOVA_SLOT0_PROFILE } from '../../../../src/runtime/procedural/tree/zelkova/zelkovaShapeProfile';

const SEED0 = morphSeedOf('asset_tree_zelkova', 0);
/** slot-0 High rng 消费快照（2026-09-20 探针实测，真实 mulberry32 流口径——任何条件
 *  跳过消费都违约；profile 冠幅校准定稿后定值；跨 seed 随保留簇数变化（通透 roll 每
 *  存活候选 1 次）——三先例同机制） */
const RNG_CALLS_LOCK = 150046;

const built: ZelkovaGeometryResult[] = [];

function buildTracked(seed: number, profile = ZELKOVA_SLOT0_PROFILE): ZelkovaGeometryResult {
  const result = buildZelkovaGeometry(mulberry32(seed), profile);
  built.push(result);
  return result;
}
...
describe('确定性（shapeProfile 路径 + rng 消费恒等）', () => {
  ...
  it('profile 缺省 = slot-0 标准组合显式传入（锚点回落路径逐位一致）', () => {
    const a = buildZelkovaGeometry(mulberry32(SEED0));
    const b = buildTracked(SEED0, ZELKOVA_SLOT0_PROFILE);
    built.push(a);
...
describe('冠内通透（显式规则：通道 / 内层密度衰减 / 局部空腔）', () => {
  ...
  function cardRootMidpoints(result: ZelkovaGeometryResult): { x: number; y: number; z: number }[] {
  ...
  it('外密内疏：内核（q<0.4）叶卡保留率显著低于外壳（q>0.6），外壳近满（crown_fill_gradient）', () => {
    ...
    expect(shellRet, '外壳保留率应近满（中-密冠壳——Spec §3 Inferred [8]）').toBeGreaterThanOrEqual(0.9);
...
describe('枝梢驱动叶簇（家族方法沿用：枝梢 → 簇空间 → 叶片分布）', () => {
  it('簇-枝梢绑定：簇数 > 0、L5 末梢挂簇占主导；簇中心贴挂点（cm 级）；簇方向 = 枝切向单位向量；簇账目守恒', () => {
    ...
    expect(l5, 'L5 末梢簇应占主导（叶互生二列、末级枝外段受光区）').toBeGreaterThan(l4);
    // 簇剔除账目守恒：保留 + 剔除 = 簇位总数（每 L5 枝 clustersL5 簇 + 每 L4 枝 clustersL4 簇）
    expect(stats.clusters.length + stats.clustersCulled).toBe(
      stats.levelBranches[4]! * ZELKOVA_SLOT0_PROFILE.clustersL5 + stats.levelBranches[3]! * ZELKOVA_SLOT0_PROFILE.clustersL4,
    );
  ...
  it('每簇叶量域 + 卡几何域：逐簇存活数 ≤ 每簇预算且总数守恒；卡宽/长宽比落 profile 域（榉树细质密叶身份 Spec Verified/Inferred）', () => {
    const result = buildTracked(SEED0);
    const { stats } = result;
    const budget = Math.max(ZELKOVA_SLOT0_PROFILE.clusterLeavesL5, ZELKOVA_SLOT0_PROFILE.clusterLeavesL4);
    ...
    for (let base = leaf.start; base < leaf.start + leaf.count; base += 6) {
      ...
      const aspect = len / w;
      if (
        w < ZELKOVA_SLOT0_PROFILE.leafWidthMin - 1e-6 ||
        w > ZELKOVA_SLOT0_PROFILE.leafWidthMin + ZELKOVA_SLOT0_PROFILE.leafWidthSpan + 1e-6 ||
        aspect < ZELKOVA_SLOT0_PROFILE.leafAspectMin - 1e-6 ||
        aspect > ZELKOVA_SLOT0_PROFILE.leafAspectMin + ZELKOVA_SLOT0_PROFILE.leafAspectSpan + 1e-6
      ) {
        bad++;
      }
    }
    expect(bad, '卡宽应 ∈ [0.04, 0.06]、长宽比 ∈ [1.8, 2.4]（profile 域）').toBe(0);
  }, 30000);
});

describe('树皮起伏确定性（光滑基底微起伏语言——amplitude 0.015 / {3,5,6} / drift 11）', () => {
  /** 皮组某管某段 ring A 的顶点索引（a0 = 每四边形首顶点；发射序 = 主干 → 底盖 → L1 → 深度优先首链） */
  function ringRadii(result: ZelkovaGeometryResult, base: number, seg: number, radial: number): number[] {
    const pos = result.geometry.getAttribute('position');
  ...
  const TRUNK_RADIAL = ZELKOVA_SLOT0_PROFILE.trunk.radial;
  const TRUNK_SEGS = ZELKOVA_SLOT0_PROFILE.trunk.segs;
  ...
  it('主干全部环截面极差/环均径 ∈ [1.5%, 4%]，最大极差 ≥ 6mm（毫米级微起伏——光滑基底 + 薄片翘曲，bark_relief 低-微 Inferred [7][8]；vs 香樟深沟 [2%,12%]/≥12mm 带收窄——第四种树皮语言分化)', () => {
    const result = buildTracked(SEED0);
    ...
    for (let seg = 0; seg < TRUNK_SEGS; seg++) {
      ...
      expect(range / mean, `主干环 ${seg} 极差/均径应 ≥ 1.5%（微起伏可辨）`).toBeGreaterThanOrEqual(0.015);
      expect(range / mean, `主干环 ${seg} 极差/均径应 ≤ 4%（低-微起伏端不夸张——光滑基底读向）`).toBeLessThanOrEqual(0.04);
    }
    expect(maxAbs * 1000, '主干最大极差应 ≥ 6mm（毫米级——基径 0.24–0.30m × flare × 幅度比 1.5%；探针实测 7.26mm）').toBeGreaterThanOrEqual(6);
  }, 30000);
  ...
  it('起伏幅度沿枝级递减：主干极差 > L1 首环极差 > L5 首环极差；L5 绝对量 < 1mm 级（亚视觉地板 → 圆管）', () => {
    ...
    const l1 = ringRadii(result, L1_BASE, 0, ZELKOVA_SLOT0_PROFILE.levels[0]!.radial);
    const l1Range = Math.max(...l1) - Math.min(...l1);
    const l5 = ringRadii(result, L5_BASE, 0, ZELKOVA_SLOT0_PROFILE.levels[4]!.radial);
    const l5Range = Math.max(...l5) - Math.min(...l5);
    expect(trunkMax, '主干极差应大于 L1').toBeGreaterThan(l1Range);
    expect(l1Range, 'L1 首环极差应 ≥ 1.5mm（粗枝端剥落斑驳微起伏——winter-a [8]；探针实测 2.67mm）').toBeGreaterThanOrEqual(0.0015);
...
describe('贴地契约与参数面驱动', () => {
  ...
  it('canopyDensity 压低 → 叶卡数显著下降（同 seed 同拓扑，密度场真实消费）', () => {
    ...
    const sparse = buildTracked(SEED0, { ...ZELKOVA_SLOT0_PROFILE, canopyDensity: 0.25 });
