/**
 * tests/runtime/procedural/tree/sophoraStructure.test.ts —— 国槐枝干结构真实性
 * 测试（T011.9，对称栾树/乌桕/重阳木 organization——方法复制，数值锚为国槐自己的
 * 2026-09-21 终测（规范种子 = morphSeedOf(id, 0)）；**一回羽叶小卡簇挂点 + 两段
 * scaffold 角带低位放射 + 念珠荚果串零 rng 账目 + 无花资产 uv 域（果域 v∈[4,5] +
 * 隔离带空）= 国槐独有断言组**）。
 *
 * 覆盖（零 mock——真实几何生成；直调 buildSophoraGeometry 消费扩展 stats，与资产
 * 入口契约（asset_tree_sophora.asset）互补——本文件锁「结构怎么长」，那边锁「契约
 * 怎么传」）：
 * - 确定性：同 rng seed + 同 shapeProfile 两次构建 → 全属性逐位相等、stats 账目全等；
 *   rng 消费恒等 89876（消费顺序即契约——次数变化 = 随机流重排；011.3 ③ 教训：包裹
 *   rng 闭包逐调用计数口径；**荚果挂点零 rng（确定性账目）——消费全在骨架/簇/通透
 *   roll**；跨槽/跨 seed 随保留簇数浮动 ±0.7%（实测 89266–90446）——通透 roll 计数
 *   机制，九先例同款记档）；
 * - 组序恰 2（D15 免组膨胀：皮 0 / 复叶卡 1——**荚果入皮组（组 0）**）；皮拓扑面数
 *   恒等 24178（6 骨架 + 1 领导拓扑）；组账守恒（组 0 = 皮拓扑 + 荚果 / 组 1 = 卡×2）；
 * - 主次分级：五级拓扑计数恒等 [7,21,63,189,378]；各级平均起径严格递减；一级骨架枝
 *   （含领导枝）≥ 2.2× 次级、≥ 12× 末梢——「所有分枝视觉权重接近即不达标」的结构化判据；
 * - **一回羽叶小卡簇挂点（国槐独有身份，家族复叶第三型——Spec §4/域扩展节 B 一回
 *   奇数羽状复叶互生散生 Verified [1][2][3][6] + 主代理 Step 1 判定）**：每簇存活叶量
 *   ≤ 10（互生节位合并抽象候选上限）且均值 ≥ 预算半（5）；簇账守恒（簇账 = 总卡数）；
 *   卡几何域（宽 0.07–0.12m / **长/宽 = 1/0.34 恒比例**（冻结接口——尺寸抖动仅缩放；
 *   15–25cm 一回羽叶 ×1.4 映射）落 profile 域）；
 * - **uv 域身份标记 + 念珠荚果账目（国槐独有断言组——荚果做（裁决 4）/ 花不做
 *   （裁决 3））**：果域 v∈[4,5] 顶点数 = 珠 × 24（八面体 8 面 × 3 顶点）；**v∈[3,4)
 *   顶点数 = 0（皮管弧长域与果域的隔离带空）**；**v∈[5,7) 顶点数 = 0（无花资产——
 *   裁决 3 的结构锁）**；皮组其余顶点 v < 2.5（皮管弧长域实测 max 1.885——10m 级
 *   最长枝弧 ×0.5）；叶组 uv 标准 0–1 四边形域（v 轴 = 复叶基部（裸轴段）0 → 顶生
 *   小叶尖 1、u = 0.5 叶轴中轴——材质 SDF 窗列的几何侧不变式）；荚果账目（串数/
 *   珠数/三角 = 珠 × 8；珠数 ∈ [3×串, 10×串]——裁决 5 域 3–10 珠的账目锁）；
 * - 冠内通透（显式规则）：候选 → 存活存在剔卡且枝干通道/密度场两规则实际命中；外密
 *   内疏——内核保留率显著低于外壳（致密相家族方向沿用）；枝干通道——存活叶卡挂点
 *   （根边中点 = 过滤判定口径）距通道线段带内数为 0；通透不挖空整层——冠顶/冠底
 *   1/3 高度带均有存活叶卡；
 ...
 *   ——终测 10.34；开展宽冠树高不宜取高端的负相关读向下锚域中带回调记档见
 *   shapeProfile）；
 * - 树皮起伏确定性（灰褐深纵裂厚脊沟中-深偏高档——amplitude 0.032 / {3,4,6} /
 *   drift 5.5）：主干环截面起伏存在（极差/环均径 3–7.5% 实测带 + 最大极差 ≥ 10mm
 *   毫米级——厘米级沟脊 [6] 的起伏量级域）；**L1/L5 环极差 = 0 级（抬档亚视觉地板
 *   3.5mm——起径 <0.109m 的管平滑发射，仅主干有效起伏，「深裂显于粗干」[4][6] 的
 *   几何侧读向，交叉网状/瘤突/色序归材质层）**；wrap 位浮点级无缝；
...
import { SOPHORA_SLOT0_PROFILE } from '../../../../src/runtime/procedural/tree/sophora/sophoraShapeProfile';

const SEED0 = morphSeedOf('asset_tree_sophora', 0);
/** slot-0 High rng 消费快照（2026-09-21 终测，包裹闭包逐调用计数——任何条件跳过消费
 *  都违约；荚果挂点零 rng（确定性账目）——消费全在骨架/簇/通透 roll；跨槽/跨 seed 随
 *  保留簇数变化 ±0.7%（实测 89266–90446；通透 roll 计数 = 存活候选数——九先例同机制） */
...
function buildTracked(seed: number, profile = SOPHORA_SLOT0_PROFILE): SophoraGeometryResult {
  const result = buildSophoraGeometry(mulberry32(seed), profile);
  built.push(result);
  return result;
}
...
describe('确定性（shapeProfile 路径 + rng 消费恒等）', () => {
  ...
  it('profile 缺省 = slot-0 标准组合显式传入（锚点回落路径逐位一致）', () => {
    const a = buildSophoraGeometry(mulberry32(SEED0));
    const b = buildTracked(SEED0, SOPHORA_SLOT0_PROFILE);
    built.push(a);
    expect(a.geometry.getAttribute('position').array).toEqual(b.geometry.getAttribute('position').array);
    expect(a.stats).toEqual(b.stats);
  }, 30000);

  it(`rng 消费恒等 ${RNG_CALLS_LOCK}（消费顺序即契约——次数变化 = 随机流重排；荚果挂点零 rng）`, () => {
...
describe('组序与皮拓扑（D15 恰 2 组 + 结构计数恒等）', () => {
  it(`五级拓扑计数恒等 [7,21,63,189,378]、皮面数恒等 ${BARK_TRIS_LOCK}（6 骨架 + 1 领导——槽间皮恒定的前提）`, () => {
    ...
    expect(geometry.groups, '应恰 2 组（皮+荚果 0 / 复叶卡 1——荚果入皮组）').toHaveLength(2);
    expect(geometry.groups[0]!.materialIndex).toBe(0);
    expect(geometry.groups[1]!.materialIndex).toBe(1);
  }, 30000);

  it('各级平均起径严格递减；L1 ≥ 2.2×L2 且 L1 ≥ 12×L5（层级对比可感知判据；终测 L1/L2 2.87 / L1/L5 31.3）', () => {
...
describe('一回羽叶小卡簇挂点（国槐独有身份——家族复叶第三型，Spec §4 一回奇数羽状复叶互生散生 Verified [1][2][3][6] + 主代理 Step 1 判定）', () => {
  it('每簇存活叶量 ≤ 10（互生节位合并抽象候选上限）且均值 ≥ 5（预算半）；簇账守恒（簇账 = 总卡数）', () => {
    const { stats } = buildTracked(SEED0);
    expect(stats.clusterLeafMax, '逐簇存活数应 ≤ 每簇候选 10').toBeLessThanOrEqual(10);
    expect(stats.clusterLeafMean, '逐簇均值应 ≥ 预算半（互生散簇存活典型 5–9）').toBeGreaterThanOrEqual(5);
    expect(stats.clusterLeaves.reduce((s, n) => s + n, 0)).toBe(stats.leafCards);
  }, 30000);

  it('卡几何域：宽 ∈ [0.07, 0.12]、长/宽 = 1/0.34 恒比例（冻结接口——尺寸抖动仅缩放不改比例；15–25cm 一回羽叶 ×1.4 映射 Spec [1][2][4]）', () => {
    ...
    for (let base = leaf.start; base < leaf.start + leaf.count; base += 6) {
      ...
      const aspect = len / w;
      if (
        w < SOPHORA_SLOT0_PROFILE.leafWidthMin - 1e-4 ||
        w > SOPHORA_SLOT0_PROFILE.leafWidthMin + SOPHORA_SLOT0_PROFILE.leafWidthSpan + 1e-4 ||
    ...
    expect(bad, '卡宽应 ∈ [0.07, 0.12]、长/宽恒 = 1/0.34（冻结比例卡；1e-4 容差 = Float32 顶点存储噪声）').toBe(0);
  }, 30000);

  it('aBend 卡内根→尖非降（复叶卡契约——根 = 复叶基部/裸轴段、尖 = 顶生小叶）；aLeafRand ∈ [0,1) 且组 0 恒 0', () => {
    const result = buildTracked(SEED0);
    ...
    // 组 0 = 皮拓扑 + 荚果：树皮语义位恒 0（荚果刚性——aLeafRand/aBend 随皮组恒 0）
    ...
    expect(badZero, '组 0 aLeafRand/aBend 应恒 0（皮 + 荚果刚性悬垂）').toBe(0);
  }, 30000);
});

describe('uv 域身份标记 + 念珠荚果账目（国槐独有断言组——荚果做（裁决 4）/ 花不做（裁决 3）；家族探针断言纪律沿用）', () => {
  it('果域 v∈[4,5] 顶点数 = 珠 × 24；v∈[3,4) 顶点数 = 0（隔离带空）；v∈[5,7) 顶点数 = 0（无花资产）；皮组其余顶点 v < 2.5（弧长域实测 max 1.885）；叶组 uv 标准 0–1 四边形域', () => {
    const result = buildTracked(SEED0);
    ...
    expect(fruitDomain, '果域顶点应 = 珠 × 24（八面体 8 面 × 3 顶点——uv (u,4.0)(u,4.0)(u,4.97)）').toBe(
      result.stats.podBeads * 24,
    );
    ...
    expect(barkMaxV, '皮管弧长域 max v 应 < 2.5（实测 1.885——10m 级最长枝弧 ×0.5）').toBeLessThan(2.5);
    ...
    expect(badLeafUv, '叶卡 uv 应落标准 0–1 四边形域（材质 SDF 窗列的几何侧不变式）').toBe(0);
  }, 30000);

  it('组账守恒：组 0 三角 = 皮拓扑 + 荚果（珠 × 8）；组 1 三角 = 复叶卡 × 2', () => {
    const result = buildTracked(SEED0);
    const { stats } = result;
    const group0Tris = result.geometry.groups[0]!.count / 3;
    const group1Tris = result.geometry.groups[1]!.count / 3;
    expect(group0Tris, '组 0 三角 = 皮拓扑 + 荚果（荚果入皮组——材质按 v 域分流）').toBe(
      stats.barkTriangles + stats.podTriangles,
    );
    expect(group1Tris, '组 1 三角 = 纯复叶卡 × 2').toBe(stats.leafCards * 2);
  }, 30000);

  it('荚果账目：珠数 ∈ [3×串, 10×串]（裁决 5 域 3–10 珠）；三角 = 珠 × 8；串数落盛挂期定值带（终测 49–63/树——8 槽带见 sophoraLod）', () => {
    const { stats } = buildTracked(SEED0);
    expect(stats.podSites, 'slot-0 应有荚果串（盛挂期身份信号——裁决 4）').toBeGreaterThanOrEqual(40);
    expect(stats.podBeads, '珠数应 ≥ 3×串（每串最少 3 珠）').toBeGreaterThanOrEqual(stats.podSites * 3);
    expect(stats.podBeads, '珠数应 ≤ 10×串（每串最多 10 珠——裁决 5 域上沿）').toBeLessThanOrEqual(stats.podSites * 10);
    expect(stats.podTriangles).toBe(stats.podBeads * 8);
  }, 30000);
});
...
describe('冠内通透（显式规则：通道 / 内层密度衰减 / 局部空腔）', () => {
   ...
   *  口径严格对齐；组 1 纯卡无附加块） */
  function cardRootMidpoints(result: SophoraGeometryResult): { x: number; y: number; z: number }[] {
    const leaf = result.geometry.groups[1]!;
    const pos = result.geometry.getAttribute('position');
  ...
  it('通透规则实际生效：存在剔卡，且枝干通道与密度场两规则均命中；账目守恒', () => {
    ...
    expect(stats.channelRejects, '枝干通道应实际剔卡（低位放射水平骨架与叶幕重叠区）').toBeGreaterThan(0);
  ...
  it('外密内疏：内核（q<0.4）叶卡保留率显著低于外壳（q>0.6），外壳高保留（致密相——空隙 10–20% 读向；终测内 0.29 / 外 0.95）', () => {
    const { stats } = buildTracked(SEED0);
    ...
    expect(shellRet, '外壳保留率应高保留（致密相——Spec §3 空隙 10–20% 带密侧）').toBeGreaterThanOrEqual(0.85);
...
describe('枝梢驱动叶簇（家族方法沿用：枝梢 → 簇空间 → 互生螺旋散排）', () => {
  it('簇-枝梢绑定：簇数 > 0、L5 末梢挂簇占主导；簇中心贴挂点（cm 级）；簇方向 = 枝切向单位向量；簇账目守恒', () => {
    ...
    expect(stats.clusters.length + stats.clustersCulled).toBe(
      stats.levelBranches[4]! * SOPHORA_SLOT0_PROFILE.clustersL5 + stats.levelBranches[3]! * SOPHORA_SLOT0_PROFILE.clustersL4,
    );
  ...
  it('shellBias 平摊：存活卡簇内归一化半径均值 ≥ 0.7（shellBias=0.44/γ=0.9 互生散布宽域平摊——叶沿簇域分布；终测 0.79）', () => {
    const { stats } = buildTracked(SEED0);
...
describe('树皮起伏确定性（灰褐深纵裂厚脊沟中-深偏高档——amplitude 0.032 / {3,4,6} / drift 5.5，终审裁决 6）', () => {
  /** 皮组某管某段 ring A 的顶点索引（a0 = 每四边形首顶点；发射序 = 主干 → 底盖 → L1 → 深度优先首链） */
  function ringRadii(result: SophoraGeometryResult, base: number, seg: number, radial: number): number[] {
    const pos = result.geometry.getAttribute('position');
  ...
  const TRUNK_RADIAL = SOPHORA_SLOT0_PROFILE.trunk.radial;
  const TRUNK_SEGS = SOPHORA_SLOT0_PROFILE.trunk.segs;
  ...
  it('主干全部环截面极差/环均径 ∈ [3%, 7.5%]，最大极差 ≥ 10mm（毫米级中-深偏高档——bark-a「脊宽厚沟深（厘米级）」双问 [6]；0.032 幅度 × 基径 0.26–0.32 × flare）', () => {
    const result = buildTracked(SEED0);
    ...
    for (let seg = 0; seg < TRUNK_SEGS; seg++) {
      ...
      expect(range / mean, `主干环 ${seg} 极差/均径应 ≥ 3%（中-深偏高档可辨）`).toBeGreaterThanOrEqual(0.03);
      expect(range / mean, `主干环 ${seg} 极差/均径应 ≤ 7.5%（中-深偏高档不夸张）`).toBeLessThanOrEqual(0.075);
    }
    expect(maxAbs * 1000, '主干最大极差应 ≥ 10mm（0.032 幅度下厘米级沟脊量级）').toBeGreaterThanOrEqual(10);
  }, 30000);

  it('抬档亚视觉地板：L1/L5 首环极差 < 1e-6（起径 < 0.109m 的管平滑发射——仅主干有效起伏；「深裂显于粗干」[4][6]，交叉网状/瘤突/色序归材质层；1e-6 容差 = Float32 顶点存储噪声）', () => {
    const result = buildTracked(SEED0);
    const l1 = ringRadii(result, L1_BASE, 0, SOPHORA_SLOT0_PROFILE.levels[0]!.radial);
    const l1Range = Math.max(...l1) - Math.min(...l1);
    const l5 = ringRadii(result, L5_BASE, 0, SOPHORA_SLOT0_PROFILE.levels[4]!.radial);
    const l5Range = Math.max(...l5) - Math.min(...l5);
    expect(l1Range, 'L1 首环极差应 = 0 级（起径 0.08–0.09 < 0.109 抬档地板——圆管）').toBeLessThan(1e-6);
...
describe('贴地契约 / 参数面驱动 / 树高锚（任务书口径）', () => {
  ...
  it('树高锚：slot-0 总高落 [9.6, 10.8]（≈10m 主代理裁定——裁决 2；终测 10.34——开展宽冠弱领导的涌现折减回调后锚域中带）', () => {
    const result = buildTracked(SEED0);
    result.geometry.computeBoundingBox();
  ...
  it('canopyDensity 压低 → 叶卡数显著下降（同 seed 同拓扑，密度场真实消费）', () => {
    ...
    const sparse = buildTracked(SEED0, { ...SOPHORA_SLOT0_PROFILE, canopyDensity: 0.25 });
