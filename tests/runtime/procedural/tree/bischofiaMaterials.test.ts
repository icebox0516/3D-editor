 ...
 *   ginkgo/platanus/koelreuteria/triadica 63 键零碰撞；叶 Mid 去小叶脉/叶团/糙度
...
const sdfOf = (fragmentShader: string): string => {
  const start = fragmentShader.indexOf('float bisLeafAlpha(vec2 bisUv, float bisRand)');
...
const subSdfOf = (fragmentShader: string): string => {
  const start = fragmentShader.indexOf('float bisLeafSDF(float bisX, float bisY, float bisRand)');
  expect(start).toBeGreaterThanOrEqual(0);
  const end = fragmentShader.indexOf('\n}', start);
  return fragmentShader.slice(start, end + 2);
};
...
const leafletOf = (fragmentShader: string): string => {
  const start = fragmentShader.indexOf('float bisLeaflet(float bisN, float bisT, float bisLen, float bisHw, float bisR)');
  expect(start).toBeGreaterThanOrEqual(0);
  const end = fragmentShader.indexOf('\n}', start);
  return fragmentShader.slice(start, end + 2);
};
...
describe('分档实装（level 参数；缺省 high = 显式 high）', () => {
  ...
  it('与 zelkova/camphor/celtis/ginkgo/platanus/koelreuteria/triadica 63 键零碰撞（bischofia 前缀不与八先例混缓存）', () => {
    ...
    expect(foreignKeys.size).toBe(63);
