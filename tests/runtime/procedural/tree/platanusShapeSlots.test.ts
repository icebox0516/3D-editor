 ...
 *   PLATANUS_SHAPE_PROFILES[slot])（几何直调）逐位一致——slot-0/3/6/7 代表位；材质
 *   （platanusMaterials 并行交付）未合并时动态 import 失败 → 优雅跳过（榉树/银杏
 *   先例，合并后自动生效）；
...
describe('路由一致（8 组向量即 config 雏形：资产路径 = 几何直调）', () => {
  it('build({seed: morphSeedOf(id, slot)}) 与 buildPlatanusGeometry(mulberry32(seed), PROFILES[slot]) 逐位一致（slot-0/3/6/7——首/中/尾代表位 + 锚点；platanusMaterials 未合并时优雅跳过；profileForSeed 为 O(8) 纯查表无槽位特判，记档）', async () => {
    const assetModule = await import('../../../../src/runtime/procedural/assets/asset_tree_platanus.asset').catch(
      () => null,
    );
    if (!assetModule) {
      // platanusMaterials.ts（park-shader-agent 并行交付）未合并——资产模块动态 import
      // 失败，路由锁延后到合并后自动生效（主代理合并验证面）；本测试不判失败
      console.warn('[platanusShapeSlots] asset_tree_platanus 入口暂不可导入（platanusMaterials 并行交付未合并）——路由一致锁延后');
