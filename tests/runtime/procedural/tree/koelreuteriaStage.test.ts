/**
 * tests/runtime/procedural/tree/koelreuteriaStage.test.ts —— window.__koelreuteria DEV
 * 出图面句柄测试（T011.6；platanusStage.test 同构复制——断言集逐条对应，数值面换栾树
 * 实数）。
 *
 ...
 *   并入花果 / 复叶卡数 = 资产恒定值）+ 幂等（重复 mount 同位重建、unmount 空挂安全）；
 ...
 *   20782 + 花序/灯笼确定性账目随槽（花 39–47 序 ×4 + 灯笼 216–251 ×8 → 合计
 *   1900–2184 面平滑变化——**零 rng 挂点法**，vs platanus 果序 rng roll 的带状浮动
 *   本例规避记档），断言带 22682–22966 + 花果 ≡0 mod 4 结构不变量（花 4 面/序 +
 *   灯笼 8 面/果）、顶层合计 8 棵和；复叶卡数只断言 > 0——不锁具体值，档间/槽间数值
 *   归资产侧测试）；viewSlots 网格中心机位（球坐标公式复算）+ viewSlot(5) 槽位特写 +
 ...
 * 数值锚（vs platanus 差异面）：slot-0 High 组 0 皮 22682（皮拓扑恒 20782 + 花序 43 序
 *   ×4 = 172 + 灯笼 216×8 = 1728 并入组 0——花果入皮组冻结接口，vs platanus 组 0 =
 *   皮 24178 + 果序 920）/ 复叶卡 1586 / 793 卡（T011.6 探针实测 = 资产预算锁定账目
 *   triangleCount 24268 = 22682 + 1586；复叶卡 = 2 tri/卡承载整枚二回羽叶）；取景视心
 *   高 5.9（slot-0 冠域 1.91–9.85 冠心 ≈5.88——vs platanus 7.5 / camphor 5.5 / ginkgo
 *   5.4 / zelkova 5.3 / celtis 4.3，族内第二高——≈10m 级中量级，koelreuteriaStage
 *   模块头「树高参考」）。
 */
import { describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { createKoelreuteriaHandle } from '../../../../src/runtime/procedural/tree/koelreuteria/koelreuteriaStage';
import { meta } from '../../../../src/runtime/procedural/assets/asset_tree_koelreuteria.asset';
 ...
 *  （栾树 build 自 T011.6 起返回深度材质；不带字段的 makeFakeBuild 即回退路径） */
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（组 0 皮 22682 / 叶 1586 / 793 卡，T011.6 探针实测 = 预算锁定账目 slot-0 High——皮 20782 + 花序 43 序×4 + 灯笼 216×8 并入组 0）', () => {
    const scene = new THREE.Scene();
    const handle = createKoelreuteriaHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(22682);
    expect(stats.leafTriangles).toBe(1586);
    expect(stats.leafCards).toBe(793);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置冠心 + update 被调', () => {
    ...
    const handle = createKoelreuteriaHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈5.9（锚点树 9.85m 冠域 1.91–9.85 冠心 ≈5.88——族内第二高，vs platanus 7.5）
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createKoelreuteriaHandle({ scene });
    const b = createKoelreuteriaHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('koelreuteria-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('koelreuteria-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createKoelreuteriaHandle({
    ...
    const bare = createKoelreuteriaHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：koelreuteria-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createKoelreuteriaHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('koelreuteria-dev-slots');
  ...
  it('stats（slots 模式）：8 项逐槽账目，组 0 平滑带——皮拓扑 20782 恒 + 花果确定性账目随槽（合计 1900–2184 面，带 22682–22966、花果 ≡0 mod 4）+ 顶层合计 8 棵和；复叶卡数只断言 > 0（不锁具体值——数值归资产侧测试）', () => {
    const scene = new THREE.Scene();
    const handle = createKoelreuteriaHandle({ scene });
    handle.mountSlots();
    const stats = handle.stats();
    ...
    for (let slot = 0; slot < 8; slot++) {
      ...
      // 组 0 = 皮拓扑恒 20782 + 花果确定性账目随槽（花 39–47 序×4 + 灯笼 216–251×8
      // → 合计 1900–2184 面平滑带，T011.6 探针实测带——零 rng 挂点法，vs platanus 果序
      // rng roll 的带状浮动本例规避）
      expect(entry.barkTriangles).toBeGreaterThanOrEqual(20782 + 1900); // 带下沿
      expect(entry.barkTriangles).toBeLessThanOrEqual(20782 + 2184); // 带上沿
      expect((entry.barkTriangles - 20782) % 4).toBe(0); // 花果 = 花 4 面/序 + 灯笼 8 面/果——结构不变量（皮拓扑恒 20782）
      expect(entry.leafCards).toBeGreaterThan(0); // 复叶卡数随簇级剔除定——不锁具体值
      expect(entry.leafTriangles).toBe(entry.leafCards * 2); // 卡 = 2 三角（整枚二回羽叶 1 卡）
    }
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('koelreuteria-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createKoelreuteriaHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('koelreuteria-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('koelreuteria-dev-levels');
  ...
  it('viewLevels：缺省机位（32/35/16）球坐标公式复算 + target = 排中心 (0, 5.9, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createKoelreuteriaHandle({ scene, camera, controls, build: makeFakeBuild(log) });
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createKoelreuteriaHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createKoelreuteriaHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
