/**
 * tests/runtime/procedural/tree/triadicaStage.test.ts —— window.__triadica DEV
 * 出图面句柄测试（T011.7；koelreuteriaStage.test 同构复制——断言集逐条对应，数值面换
 * 乌桕实数）。
 *
 * 覆盖（零 WebGL——裸 THREE.Scene + 结构桩相机/控制；window 槽装配归 bootstrap 组合根）：
 * - mount/unmount：group 挂 scene 兄弟层（scene.children 增减）+ stats 账目（组 0 皮含
 *   并入果序 / 中卡数 = 资产恒定值）+ 幂等（重复 mount 同位重建、unmount 空挂安全）；
 ...
 *   castShadow/customDepthMaterial；stats 逐槽账目（组 0 槽间**平滑带**——皮拓扑恒
 *   24178 + 果序确定性账目随槽（60–84 果 ×8 → 合计 480–672 面平滑变化——**零 rng 挂点
 *   法**，vs platanus 果序 rng roll 的带状浮动本例规避记档，栾树同款），断言带
 *   24658–24850 + 果序 ≡0 mod 8 结构不变量（八面体 8 面/果）、顶层合计 8 棵和；中卡
 *   数只断言 > 0——不锁具体值，档间/槽间数值归资产侧测试）；viewSlots 网格中心机位
 *   （球坐标公式复算）+ viewSlot(5) 槽位特写 + 越界 warn no-op；与 mount/mountWindDemo
 *   互斥重建、unmount/dispose 幂等（slots 模式 stats 归零）。
 ...
 * 数值锚（vs koelreuteria 差异面）：slot-0 High 组 0 皮 24850（皮拓扑恒 24178 + 果序
 *   84 果×8 = 672 并入组 0——果序入皮组冻结接口，vs koelreuteria 组 0 = 皮 20782 +
 *   花序/灯笼 1900）/ 中卡 7724 tri / 3862 卡（T011.7 探针实测 = 资产预算锁定账目
 *   triangleCount 32574 = 24850 + 7724；中卡 = 2 tri/卡承载单枚菱形叶）；取景视心高
 *   5.8（slot-0 冠域 2.09–9.52 冠心 ≈5.805——vs platanus 7.5 / koelreuteria 5.9 /
 *   camphor 5.5 / ginkgo 5.4 / zelkova 5.3 / celtis 4.3，族内第二高——≈9.5m 级中量级，
 *   triadicaStage 模块头「树高参考」）。
 */
import { describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { createTriadicaHandle } from '../../../../src/runtime/procedural/tree/triadica/triadicaStage';
import { meta } from '../../../../src/runtime/procedural/assets/asset_tree_triadica.asset';
 ...
 *  （乌桕 build 自 T011.7 起返回深度材质；不带字段的 makeFakeBuild 即回退路径） */
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（组 0 皮 24850 / 叶 7724 / 3862 卡，T011.7 探针实测 = 预算锁定账目 slot-0 High——皮 24178 + 果序 84 果×8 并入组 0）', () => {
    const scene = new THREE.Scene();
    const handle = createTriadicaHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(24850);
    expect(stats.leafTriangles).toBe(7724);
    expect(stats.leafCards).toBe(3862);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置冠心 + update 被调', () => {
    ...
    const handle = createTriadicaHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈5.8（锚点树 9.52m 冠域 2.09–9.52 冠心 ≈5.805——族内第二高，vs platanus 7.5 / koelreuteria 5.9）
    expect(camera.position.x).toBeCloseTo(32, 5);
    expect(camera.position.y).toBeCloseTo(5.8, 5);
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createTriadicaHandle({ scene });
    const b = createTriadicaHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('triadica-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('triadica-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createTriadicaHandle({
    ...
    const bare = createTriadicaHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：triadica-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createTriadicaHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('triadica-dev-slots');
  ...
  it('stats（slots 模式）：8 项逐槽账目，组 0 平滑带——皮拓扑 24178 恒 + 果序确定性账目随槽（合计 480–672 面，带 24658–24850、果序 ≡0 mod 8）+ 顶层合计 8 棵和；中卡数只断言 > 0（不锁具体值——数值归资产侧测试）', () => {
    const scene = new THREE.Scene();
    const handle = createTriadicaHandle({ scene });
    handle.mountSlots();
    const stats = handle.stats();
    ...
    for (let slot = 0; slot < 8; slot++) {
      ...
      // 组 0 = 皮拓扑恒 24178 + 果序确定性账目随槽（60–84 果 ×8 → 合计 480–672 面平滑带，
      // T011.7 探针实测带——零 rng 挂点法，vs platanus 果序 rng roll 的带状浮动本例规避）
      expect(entry.barkTriangles).toBeGreaterThanOrEqual(24178 + 480); // 带下沿
      expect(entry.barkTriangles).toBeLessThanOrEqual(24178 + 672); // 带上沿
      expect((entry.barkTriangles - 24178) % 8).toBe(0); // 果序 = 八面体 8 面/果——结构不变量（皮拓扑恒 24178）
      expect(entry.leafCards).toBeGreaterThan(0); // 中卡数随簇级剔除定——不锁具体值
      expect(entry.leafTriangles).toBe(entry.leafCards * 2); // 中卡 = 2 三角（单枚菱形叶 1 卡）
    }
  ...
  it('viewSlots：缺省机位球坐标公式复算落位 + target = 网格中心 (0, 5.8, spacing/2) + update 被调；viewSlot(5) 绕槽 5 树位；viewSlot(9/-1) 越界 warn no-op 不位移', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const handle = createTriadicaHandle({ scene, camera, controls });
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('triadica-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createTriadicaHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('triadica-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('triadica-dev-levels');
  ...
  it('viewLevels：缺省机位（32/35/16）球坐标公式复算 + target = 排中心 (0, 5.8, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createTriadicaHandle({ scene, camera, controls, build: makeFakeBuild(log) });
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createTriadicaHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createTriadicaHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
