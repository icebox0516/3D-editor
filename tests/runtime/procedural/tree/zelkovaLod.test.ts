/**
 * tests/runtime/procedural/tree/zelkovaLod.test.ts —— 榉树 LOD 三档几何不变量测试
 * （T011.3，对称香樟 camphorLod 组织——方法复制；快照数为榉树自己的实测锚）。
 *
 * 覆盖（零 mock——真实几何生成；直调 buildZelkovaGeometry 三档，与资产入口契约互补——
 * 本文件锁「档间不变量怎么成立」）：
 * - High 逐位不动：缺省调用（profile + level 双缺省）= 显式 'high' 逐位相等；slot-0
 *   皮面 20782 / rng 消费 150046 快照延续（快照口径 = 槽内跨档恒等；跨槽消费数随保留
 *   簇数变化——三先例同机制）；
 * - rng 三档恒等：同 seed 三档 rng 消费次数全等 150046（「发射省略不省略消费」纪律）；
 ...
 *   同簇；卡数比 ≈ 6/18 统计口径复核（探针实测 0.331–0.334）；slot-0/6/7 三槽取证
 *   （卡数最少 / 最多 + 锚点）；
 * - Low 壳卡账目：leafCards = 保留簇 × 2、leafTriangles = 保留簇 × 4（8 槽全扫）；
 * - 预算锁定（D19.8，家族行沿用）：8 槽 × 3 档总面落锁定预算 High ≤ 40000 / Mid
 *   6000–10000 / Low 1500–3000（实测带 High 32148–37568 / Mid 7662–9496 / Low
 *   2290–2914；锁定账目与依据见 asset_tree_zelkova.asset 模块头；High 下沿 ≥ 28754
 *   在 zelkovaShapeSlots 通透预算带测试锁）；
 * - 恰 2 组（皮 0 / 叶 1，D15）；档间 bbox 一致：同槽三档 XZ 跨 / 总高差 ≤ 0.4m
 *   （LOW_SHELL_MARGIN 0.10 口径——榉树小卡尺度）、minY 三档全 0；
...
/** High slot-0 皮面快照（皮拓扑恒等不动——zelkovaStructure 同款锁） */
...
function buildSlot(slot: number, level: ProceduralLevel = 'high'): ZelkovaGeometryResult {
  return track(
    buildZelkovaGeometry(
      mulberry32(morphSeedOf('asset_tree_zelkova', slot)),
      ZELKOVA_SHAPE_PROFILES[Math.min(slot, ZELKOVA_SHAPE_PROFILES.length - 1)]!,
      level,
    ),
  );
}
...
describe('High 逐位不动（缺省档回归锁）', () => {
  it('缺省调用（profile + level 双缺省）= 显式 high：position/aBend 数组与 stats 逐位全等', () => {
    const a = track(buildZelkovaGeometry(mulberry32(SEED0)));
...
describe('Mid ⊂ High（j % 3 === 1 掩码只在烘焙阶段生效）', () => {
  it('Mid 存活卡 XZ 位置块逐位 ∈ High 卡集（Y 差 = 单一贴地平移常量）、逐簇烘焙叶量 ≤ High 同簇、卡数比 ≈ 6/18（slot-0/6/7）', () => {
    for (const slot of [0, 6, 7]) {
      ...
      // 掩码统计口径：Mid 卡数 ≈ High 存活卡 × 6/18（33.3%——±10% 带内复核；探针实测 0.331–0.334）
  ...
  it('总面落锁定预算（High ≤40K / Mid 6–10K / Low 1.5–3K，D19.8）；恰 2 组；同槽三档跨度差 ≤0.4m、minY 全 0；Low 壳卡 = 簇 × 2；法线有限', () => {
    for (let slot = 0; slot < 8; slot++) {
      ...
      for (const level of LEVELS) {
        ...
        // 预算锁定（锁定账目与依据见 asset_tree_zelkova.asset 模块头）
        if (level === 'high') {
