...
function track(result: BischofiaGeometryResult): BischofiaGeometryResult {
  built.push(result);
  return result;
}
...
function spanOf(result: BischofiaGeometryResult): { xz: number; y: number; minY: number } {
 ...
 *  组 1 纯复叶卡（无花果资产——无附加块直扫） */
function leafCardXZ(result: BischofiaGeometryResult): { keys: string[]; y0: number[] } {
