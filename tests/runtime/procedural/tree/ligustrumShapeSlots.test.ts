 ...
 *   材质（ligustrumMaterials 并行交付）未就绪时动态 import 失败 → 优雅跳过
 *   （先例，合并后自动生效）；
...
describe('路由一致（8 组向量即 config 雏形：资产路径 = 几何直调）', () => {
  it('build({seed: morphSeedOf(id, slot)}) 与 buildLigustrumGeometry(mulberry32(seed), PROFILES[slot]) 逐位一致（slot-0/3/6/7——首/中/尾代表位 + 锚点；ligustrumMaterials 未就绪时优雅跳过；profileForSeed 为 O(8) 纯查表无槽位特判，记档）', async () => {
    const assetModule = await import('../../../../src/runtime/procedural/assets/asset_tree_ligustrum.asset').catch(
      () => null,
    );
    if (!assetModule) {
      // asset_tree_ligustrum.asset 入口未合并（材质工厂 ligustrumMaterials
      // park-shader-agent 并行交付未就绪——入口文件按主代理协调延后落位）——资产模块
      // 动态 import 失败，路由锁延后到合并后自动生效（主代理合并验证面）；本测试不判失败
      console.warn('[ligustrumShapeSlots] asset_tree_ligustrum 入口暂不可导入（ligustrumMaterials 并行交付未就绪）——路由一致锁延后');
