/**
 * tests/runtime/procedural/tree/bischofiaStage.test.ts —— window.__bischofia DEV
 * 出图面句柄测试（T011.8；triadicaStage.test 同构复制——断言集逐条对应，数值面换重阳木
 * 实数）。
 *
 * 覆盖（零 WebGL——裸 THREE.Scene + 结构桩相机/控制；window 槽装配归 bootstrap 组合根）：
 * - mount/unmount：group 挂 scene 兄弟层（scene.children 增减）+ stats 账目（**无花果资产
 *   组 0 = 纯皮拓扑**——皮 24178 恒定，vs triadica 组 0 皮 24178+果序 672 并入 = 24850）
 ...
 *   castShadow/customDepthMaterial；stats 逐槽账目（组 0 **槽间恒等**——无花果资产纯皮
 *   拓扑恒 24178（vs triadica 果序并入皮组 480–672 面平滑带 + 果序 ≡0 mod 8 结构不变量
 *   ——本例无附加元素账目，组 0 断言退化为恒等记档），中卡数只断言 > 0——不锁具体值，
 *   档间/槽间数值归资产侧测试）；viewSlots 网格中心机位（球坐标公式复算）+ viewSlot(5)
 ...
 * 数值锚（vs triadica 差异面）：slot-0 High 组 0 皮 **24178 恒定**（无花果资产——花果
 *   不做终审 ③-2，组 0 = 纯皮拓扑 = 主干 14 段×14 + 底盖 14 + 五级 7/21/63/189/378 枝
 *   + 领导拓扑，vs triadica 24850 = 皮 24178 + 果序 84 果×8）/ 复叶卡 5466 tri / 2733 卡
 *   （T011.8 探针实测 = 资产预算锁定账目 triangleCount 29644 = 24178 + 5466；复叶卡 =
 *   2 tri/卡承载整枚三出复叶——1 卡 = 顶生 + 两侧小叶的 SDF 层表达，vs triadica 中卡
 *   承载单枚菱形叶）；取景视心高 5.8（slot-0 冠域 1.757–9.896 冠心 ≈5.826——vs platanus
 *   7.5 / koelreuteria 5.9 / triadica 5.8 / camphor 5.5 / ginkgo 5.4 / zelkova 5.3 /
 *   celtis 4.3，与乌桕同档——≈9.9m 级中量级，bischofiaStage 模块头「树高参考」）。
 */
import { describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { createBischofiaHandle } from '../../../../src/runtime/procedural/tree/bischofia/bischofiaStage';
import { meta } from '../../../../src/runtime/procedural/assets/asset_tree_bischofia.asset';
 ...
 *  （重阳木 build 自 T011.8 起返回深度材质；不带字段的 makeFakeBuild 即回退路径） */
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（组 0 皮 24178 / 叶 5466 / 2733 卡，T011.8 探针实测 = 预算锁定账目 slot-0 High——无花果资产组 0 = 纯皮拓扑恒定）', () => {
    const scene = new THREE.Scene();
    const handle = createBischofiaHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(24178);
    expect(stats.leafTriangles).toBe(5466);
    expect(stats.leafCards).toBe(2733);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置冠心 + update 被调', () => {
    ...
    const handle = createBischofiaHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈5.8（锚点树 9.90m 冠域 1.76–9.90 冠心 ≈5.826——与乌桕同档，vs platanus 7.5 / koelreuteria 5.9 / triadica 5.8）
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createBischofiaHandle({ scene });
    const b = createBischofiaHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('bischofia-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('bischofia-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createBischofiaHandle({
    ...
    const bare = createBischofiaHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：bischofia-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createBischofiaHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('bischofia-dev-slots');
  ...
  it('stats（slots 模式）：8 项逐槽账目，组 0 **槽间恒等**——无花果资产纯皮拓扑恒 24178（vs triadica 果序并入皮组的槽间平滑带 24658–24850 + 果序 ≡0 mod 8 结构不变量——本例无附加元素账目，断言退化为恒等记档）+ 顶层合计 8 棵和；中卡数只断言 > 0（不锁具体值——数值归资产侧测试）', () => {
    const scene = new THREE.Scene();
    const handle = createBischofiaHandle({ scene });
    handle.mountSlots();
    const stats = handle.stats();
    ...
    for (let slot = 0; slot < 8; slot++) {
      ...
      // 组 0 = 纯皮拓扑恒 24178（主干 14 段×14 + 底盖 14 + 五级 7/21/63/189/378 枝 + 领导
      // 拓扑——结构计数类跨槽恒等；无花果资产无附加元素并入，vs triadica 果序带 480–672）
      expect(entry.barkTriangles).toBe(24178);
      expect(entry.leafCards).toBeGreaterThan(0); // 复叶卡数随簇级剔除定（8 槽实测带 2052–3097）——不锁具体值
      expect(entry.leafTriangles).toBe(entry.leafCards * 2); // 复叶卡 = 2 三角（1 卡承载整枚三出复叶）
    }
    expect(stats.barkTriangles).toBe(24178 * 8); // 顶层保持「总量」语义 = 8 棵合计（皮恒 → 合计恒）
    expect(stats.barkTriangles).toBe(stats.slots!.reduce((sum, s) => sum + s.barkTriangles, 0));
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('bischofia-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createBischofiaHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('bischofia-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('bischofia-dev-levels');
  ...
  it('viewLevels：缺省机位（32/35/16）球坐标公式复算 + target = 排中心 (0, 5.8, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    ...
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createBischofiaHandle({ scene, camera, controls, build: makeFakeBuild(log) });
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createBischofiaHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createBischofiaHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
