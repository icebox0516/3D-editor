 ...
 *   逐位相等）；3 工厂 × 3 档 = 9 键互异 + 与 camphor/celtis 18 键零碰撞（zelkova 前缀
 *   不混缓存）；叶 Mid 去叶脉两件/叶团（SDF 全形**含齿**保留——中距锯齿读向、透光/叶背/
...
const sdfOf = (fragmentShader: string): string => {
  const start = fragmentShader.indexOf('float zlkLeafAlpha(vec2 zlkUv, float zlkRand)');
...
describe('分档实装（level 参数；缺省 high = 显式 high）', () => {
  ...
  it('与 camphor/celtis 18 键零碰撞（zelkova 前缀不与先例混缓存）', () => {
