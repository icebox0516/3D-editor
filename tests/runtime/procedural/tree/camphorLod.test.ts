...
function track(result: CamphorGeometryResult): CamphorGeometryResult {
  built.push(result);
  return result;
}
...
function spanOf(result: CamphorGeometryResult): { xz: number; y: number; minY: number } {
...
function leafCardXZ(result: CamphorGeometryResult): { keys: string[]; y0: number[] } {
