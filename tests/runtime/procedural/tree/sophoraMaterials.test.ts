...
describe('物种配方锚定（Spec sophora-reference 1.0；生产口径 = 终审记档 ④ 裁决十项：一回奇数羽状复叶 / 树皮第 10 语言 / 荚果做 / 花不做）', () => {
  ...
  it('复叶数值锚（JS 镜像）：有效对数 4–7（run 计数——「小叶 4-7 对」统计实现）+ 顶生小叶 reach ≥ 0.030 + 叶轴连续 + 裸区零越界（减法门控回归锁）+ 顶栏残余压灭', () => {
  ...
  it('中距细碎自检剖面（本树反证 011.8 宽卵法——form-a「叶片细碎均匀」+ NC 细质）：maxW 全宽细碎质域 + 中段缢缩行 ≥6 + 卡缘恒裁 + Low 单包络档间剪影一致', () => {
