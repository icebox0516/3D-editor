...
const sdfOf = (fragmentShader: string): string =>
  extractSdf(fragmentShader, 'float sopLeafAlpha(vec2 sopUv, float sopRand)');

/** 提取注入后的 sopColumn 子函数全文（窗列子函数单一来源比对用——FXC 拆分形态） */
const columnOf = (fragmentShader: string): string =>
  extractSdf(fragmentShader, 'float sopColumn(float sopX, float sopY, float sopEnv, float sopRand)');

/** 提取注入后的 sopTerminal 子函数全文（顶生窗子函数单一来源比对用） */
const terminalOf = (fragmentShader: string): string =>
  extractSdf(fragmentShader, 'float sopTerminal(float sopX, float sopY, float sopRand)');
