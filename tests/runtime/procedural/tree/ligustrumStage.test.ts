/**
 * tests/runtime/procedural/tree/ligustrumStage.test.ts —— window.__ligustrum DEV
 * 出图面句柄测试（T011.11；fraxinusStage.test 同构复制——断言集逐条对应，数值面换
 * 女贞实数）。
 *
 ...
 *   皮 + 肾形核果满冠下垂密簇并入**——皮拓扑恒 24236 + 核果 1880（470 果×4）= 26116，
 *   vs bischofia 无花果资产组 0 纯皮 24178）+ 幂等（重复 mount 同位重建、unmount 空挂安全）；
 * - dispose：幂等；unmount 后资源已 dispose（stats 归零、group 摘除）；
 * - view：结构桩相机/控制——position 按球坐标落位、target 置冠心、update 被调；
 *   targetY 覆盖目标高（树皮干段取证低目标机位——密冠近距下冠心机位叶幕遮挡主干）+
 *   缺省不传 = 冠心 5.6 零回归锚；
 ...
 *   24236 + 核果簇并入皮组 1596–2360 tri（399–590 果×4，确定性账目零 rng——簇数随槽
 *   簇位平滑变化）→ 组 0 带 25832–26596 + 核果 ≡0 mod 4 结构不变量（果 ×4 tri），vs
 *   bischofia 无花果资产组 0 恒等记档），单叶卡数只断言 > 0——不锁具体值，档间/槽间
 ...
 * 数值锚（vs fraxinus 差异面）：slot-0 High 组 0 皮+核果 **26116**（皮拓扑恒 24236 +
 *   肾形核果 470 果×4 = 1880 并入皮组——果域 uv u∈[4,5] 冻结接口，triadica/sophora/
 *   fraxinus 先例）/ 单叶卡 7654 tri / 3827 卡（资产预算锁定账目 triangleCount 33770 =
 *   26116 + 7654；单叶卡 = 2 tri/卡承载单枚全缘单叶——vs fraxinus 复叶卡同 2 tri 承载
 *   整枚一回奇数羽状复叶、卡语义不同；核果扁平 quad 4 tri/果卡非卡面不入 leafCards）；
 *   取景视心高 5.6（slot-0 冠域 2.7165–8.4064 冠心 ≈5.5614——T011.11 Stage 探针经
 *   正式 build() 路径实测：树高 **8.4064** / 冠幅 6.9130（x 跨 6.9130 / z 跨 4.8957）/
 *   叶卡最低 Y 质心 **2.7165**（fraxinusShapeSlots.leafMetrics 同款家族口径；顶点最低
 *   2.5341 = 几何代理初报 2.53 口径）——vs fraxinus 6.3 / sophora 5.9 / koelreuteria
 *   5.9 / triadica/bischofia 5.8 / camphor 5.5 / ginkgo 5.4 / zelkova 5.3 / celtis
 *   4.3，≈8.4m 常绿密冠与 camphor（≈8.6m 常绿同类）5.5 同档高半档，ligustrumStage
 *   模块头「树高参考」）。
 */
import { describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { createLigustrumHandle } from '../../../../src/runtime/procedural/tree/ligustrum/ligustrumStage';
 ...
 *  （女贞 build 自 T011.11 起返回深度材质；不带字段的 makeFakeBuild 即回退路径） */
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（组 0 皮+核果 26116 / 叶 7654 / 3827 卡，T011.11 Stage 探针实测 = 预算锁定账目 slot-0 High——皮 24236 恒定 + 核果 470 果×4 并入皮组）', () => {
    const scene = new THREE.Scene();
    const handle = createLigustrumHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(26116);
    expect(stats.leafTriangles).toBe(7654);
    expect(stats.leafCards).toBe(3827);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置冠心 + update 被调', () => {
    ...
    const handle = createLigustrumHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈5.6（锚点树 8.4064m 冠域 2.7165–8.4064 冠心 ≈5.5614——中低分枝 0.30 档，vs fraxinus 6.3 / sophora 5.9 / triadica/bischofia 5.8）
    expect(camera.position.x).toBeCloseTo(32, 5);
    expect(camera.position.y).toBeCloseTo(5.6, 5);
  ...
  it('view targetY 覆盖：低目标近距机位（树皮干段取证）——相机 y = targetY + d·sin(el)、target.y = targetY', () => {
    ...
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createLigustrumHandle({ scene, camera, controls, build: makeFakeBuild(log) });
    handle.mount(); // 树位原点——view 机位断言不依赖几何内容，fake build 即可
    // 密冠近距下冠心机位叶幕遮挡主干（白蜡同机位中通透成立、女贞 5–15% 空隙密档失效）
    // → targetY 1.6 落裸干段取证（视觉冠底 2.72 以下干段，T011.11 Step 4 复拍机位）
    handle.view({ distance: 1.2, azimuthDeg: 0, elevationDeg: 4, targetY: 1.6 });
    const el = (4 * Math.PI) / 180;
    expect(camera.position.x).toBeCloseTo(1.2 * Math.cos(el), 5); // az 0° → 正 +X（树位原点）
    expect(camera.position.y).toBeCloseTo(1.6 + 1.2 * Math.sin(el), 5); // 目标高随参落位（非冠心 5.6）
    expect(camera.position.z).toBeCloseTo(0, 5);
    expect(controls.target.y).toBeCloseTo(1.6, 5);
    expect(controls.updates).toBe(1);
    handle.dispose();
  });

  it('view 缺省不传 targetY：目标高 = 冠心 5.6 现行为（零回归锚）', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createLigustrumHandle({ scene, camera, controls, build: makeFakeBuild(log) });
    handle.mount();
    handle.view({ distance: 1.2, azimuthDeg: 0, elevationDeg: 4 }); // 不传 targetY
    const el = (4 * Math.PI) / 180;
    expect(camera.position.y).toBeCloseTo(5.6 + 1.2 * Math.sin(el), 5); // 冠心缺省口径不变
    expect(controls.target.y).toBeCloseTo(5.6, 5);
    expect(controls.updates).toBe(1);
    handle.dispose();
  });
});
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createLigustrumHandle({ scene });
    const b = createLigustrumHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ligustrum-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ligustrum-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createLigustrumHandle({
    ...
    const bare = createLigustrumHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：ligustrum-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createLigustrumHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('ligustrum-dev-slots');
  ...
  it('stats（slots 模式）：8 项逐槽账目，组 0 **槽间平滑带**——皮恒 24236 + 核果簇并入 1596–2360 tri → 带 25832–26596 + 核果 ≡0 mod 4 结构不变量（果 ×4 tri；确定性账目零 rng——簇数随槽簇位平滑变化，vs bischofia 无花果资产组 0 恒等记档）+ 顶层合计 8 棵和；单叶卡数只断言 > 0（不锁具体值——数值归资产侧测试）', () => {
    const scene = new THREE.Scene();
    const handle = createLigustrumHandle({ scene });
    handle.mountSlots();
    const stats = handle.stats();
    ...
    for (let slot = 0; slot < 8; slot++) {
      ...
      // 组 0 = 皮拓扑恒 24236 + 肾形核果满冠下垂密簇并入（扁平 quad 4 tri/果卡非卡面）
      // ——核果挂点确定性账目零 rng，簇数随槽平滑变化 → 组 0 平滑带 25832–26596
      // （8 槽实测带 = 399–590 果）
      expect(entry.barkTriangles).toBeGreaterThanOrEqual(24236 + 1596);
      expect(entry.barkTriangles).toBeLessThanOrEqual(24236 + 2360);
      expect((entry.barkTriangles - 24236) % 4).toBe(0); // 核果 = 果 ×4 tri 结构不变量（正反双 quad）
      expect(entry.leafCards).toBeGreaterThan(0); // 单叶卡数随簇级剔除定（8 槽实测带 2600–4931）——不锁具体值
      expect(entry.leafTriangles).toBe(entry.leafCards * 2); // 单叶卡 = 2 三角（1 卡承载单枚全缘单叶）
    }
    expect(stats.barkTriangles).toBe(stats.slots!.reduce((sum, s) => sum + s.barkTriangles, 0)); // 顶层保持「总量」语义 = 8 棵合计
    expect(stats.barkTriangles).toBeGreaterThanOrEqual((24236 + 1596) * 8);
    expect(stats.barkTriangles).toBeLessThanOrEqual((24236 + 2360) * 8);
    expect(stats.leafCards).toBe(stats.slots!.reduce((sum, s) => sum + s.leafCards, 0));
    handle.dispose();
  }, 30000);

  it('viewSlots：缺省机位球坐标公式复算落位 + target = 网格中心 (0, 5.6, spacing/2) + update 被调；viewSlot(5) 绕槽 5 树位；viewSlot(9/-1) 越界 warn no-op 不位移', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const handle = createLigustrumHandle({ scene, camera, controls });
    ...
    expect(camera.position.y).toBeCloseTo(5.6 + d * Math.sin(el), 5);
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ligustrum-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createLigustrumHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('ligustrum-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ligustrum-dev-levels');
  ...
  it('viewLevels：缺省机位（32/35/16）球坐标公式复算 + target = 排中心 (0, 5.6, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createLigustrumHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createLigustrumHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
