/**
 * tests/runtime/procedural/tree/camphorShapeSlots.test.ts —— 香樟 8 槽形态向量表测试
 * （T011.2，对称朴树 celtisShapeSlots 组织——方法复制；数值锚为香樟自己的实测）。
 *
 * 覆盖（零 mock——真实几何生成；CAMPHOR_SHAPE_PROFILES 八槽完整组合 = 「8 组向量即
 * config 雏形」的契约锁，与 camphorStructure.test.ts（slot-0 结构语义）互补）：
 ...
 *   （香樟叶身份 Spec [1][2][5][6]）与 barkRelief（槽间恒等——非形态差异维度）逐位同
 *   slot-0，canopyDensity ≤ 1（消费端 Math.min(1,·) 截断，>1 无效）；
 * - 确定性：同 seed 同槽两次构建逐位相等（代表槽 0/3/7 全属性 + stats——全 8 槽双建
 *   为全量套件并行负载裁剪让位：camphorStructure 锁 slot-0 双建 + 结构计数恒等间接覆盖）；
 * - 路由一致（D19 契约锁）：build({seed: morphSeedOf(id, slot)})（资产路径，profileForSeed
 *   查表）与 buildCamphorGeometry(mulberry32(seed), CAMPHOR_SHAPE_PROFILES[slot])（几何
 *   直调）逐位一致——slot-0/3/6/7 代表位；材质（camphorMaterials 并行交付）未合并时
 *   动态 import 失败 → 优雅跳过（朴树先例，合并后自动生效）；
 * - 槽间真实差异（方向性断言，同 seed = SEED0 下跨槽对比——隔离 profile 效应与
 *   morphSeed 随机流，差异只能来自向量本身，杜绝「seed 运气冒充形态差异」）：挺拔 vs
 *   展开 XZ 包围盒宽度比 ≥ 1.2（实测 ≈1.71）；低冠 vs 高冠叶卡最低 Y（视觉冠底——
 *   Spec trunk_height_ratio 域 0.30–0.35 的槽维度展开）差 ≥ 0.9m（实测 ≈1.61m）；疏松
 *   vs 丰满叶卡数比 ≤ 0.85（实测 ≈0.51——香樟密度个体差异弱于朴树组但槽维度仍可辨，
 *   Spec §6 Inferred [5][6]）；偏冠 vs 标准——6 种子面板口径：存活叶卡质心对树干轴
 *   偏移的面板均值比 ≥ 1.4（实测 ≈2.74；单种子波动 = 骨架相位 rng 运气——面板均值是
 *   形态向量的稳健度量，朴树先例记档）；规范种子下偏冠槽质心绝对 ≥ 0.9m 且 ≥ 1.6×
 *   标准槽（实测 1.00m / ×5.07）；8 槽叶卡数（规范种子）不全相同且各落预算卡域
 *   [(28754−20782)/2, (40000−20782)/2]；
 * - 物种锚点（规范种子 slot-0）：总高落 7.5–8.8m 锚域、视觉冠底/实高落 Spec 干高占比
 *   域 0.30–0.35（±0.02 容差）、bbox 冠幅落对标域 5.5–7.0m；
...
const ASSET_ID = 'asset_tree_camphor';
...
const built: CamphorGeometryResult[] = [];
const sources: InstanceSource[] = [];

/** 几何直调（规范种子 = 各槽自己的 morphSeed） */
function buildSlot(slot: number): CamphorGeometryResult {
  const result = buildCamphorGeometry(mulberry32(SEEDS[slot]!), CAMPHOR_SHAPE_PROFILES[slot]!);
  built.push(result);
  return result;
}

/** 几何直调（指定种子 = 同 seed 跨槽对比——隔离 profile 效应） */
function buildSlotAt(slot: number, seed: number): CamphorGeometryResult {
  const result = buildCamphorGeometry(mulberry32(seed), CAMPHOR_SHAPE_PROFILES[slot]!);
  built.push(result);
  return result;
}
...
function leafMetrics(result: CamphorGeometryResult): LeafMetrics {
...
describe('结构计数恒等（8 槽皮面数与 rng 消费恒等的前提）', () => {
  ...
  it('配置侧锁：结构计数类字段 / 叶卡尺寸域 / barkRelief 逐位同 slot-0；canopyDensity ≤ 1', () => {
    const anchor = CAMPHOR_SHAPE_PROFILES[0]!;
    expect(CAMPHOR_SHAPE_PROFILES).toHaveLength(8);
    for (let slot = 1; slot < 8; slot++) {
      const p = CAMPHOR_SHAPE_PROFILES[slot]!;
...
describe('确定性（同 seed 同槽逐位复现）', () => {
  it('代表槽（0/3/7）同 seed 两次构建 position 逐位相等 + 全属性 + stats 全等（全 8 槽确定性由 camphorStructure slot-0 锁 + 结构计数恒等 + 本文件路由/差异测试间接覆盖——全量 16 次构建裁剪为 6 次，控制全量套件并行负载，记档）', () => {
...
describe('路由一致（8 组向量即 config 雏形：资产路径 = 几何直调）', () => {
  it('build({seed: morphSeedOf(id, slot)}) 与 buildCamphorGeometry(mulberry32(seed), PROFILES[slot]) 逐位一致（slot-0/3/6/7——首/中/尾代表位 + 锚点；camphorMaterials 未合并时优雅跳过；profileForSeed 为 O(8) 纯查表无槽位特判，记档）', async () => {
    const assetModule = await import('../../../../src/runtime/procedural/assets/asset_tree_camphor.asset').catch(
      () => null,
    );
    if (!assetModule) {
      // camphorMaterials.ts（park-shader-agent 并行交付）未合并——资产模块动态 import
      // 失败，路由锁延后到合并后自动生效（主代理合并验证面）；本测试不判失败
      console.warn('[camphorShapeSlots] asset_tree_camphor 入口暂不可导入（camphorMaterials 并行交付未合并）——路由一致锁延后');
...
describe('槽间真实差异（同 seed = SEED0 跨槽对比——差异纯来自形态向量，非 seed 运气）', () => {
  ...
  it('挺拔 vs 展开：XZ 包围盒宽度比 ≥ 1.2（挺拔显著窄——密林端 vs 开阔宽展端）', () => {
    ...
    expect(broad.xzWidth / narrow.xzWidth, '展开/挺拔 宽度比（实测 ≈1.71）').toBeGreaterThanOrEqual(1.2);
  }, 120000);

  it('低冠 vs 高冠：叶卡最低 Y（视觉冠底）差 ≥ 0.9m（实测 ≈1.61m——Spec 干高占比域 0.30–0.35 的槽维度展开）', () => {
    const low = m(4);
    const high = m(5);
    expect(high.minLeafY - low.minLeafY, '高冠-低冠 视觉冠底差').toBeGreaterThanOrEqual(0.9);
  }, 120000);

  it('疏松 vs 丰满：叶卡数比 ≤ 0.85（疏松显著少——crown_transparency 个体差异维度；香樟密度差异幅度弱于朴树但槽维度可辨）', () => {
    const sparse = m(6);
    const full = m(7);
    expect(sparse.cards / full.cards, '疏松/丰满 卡数比（实测 ≈0.51）').toBeLessThanOrEqual(0.85);
  }, 120000);

  it('偏冠 vs 标准：6 种子面板质心均值比 ≥ 1.4（实测 ≈2.74；单种子波动 = 相位 rng 运气，面板均值是稳健口径）；规范种子偏冠槽绝对 ≥ 0.9m 且 ≥ 1.6× 标准槽（实测 1.00m / ×5.07）', () => {
...
describe('物种锚点（规范种子 slot-0——Spec 尺度与比例域）', () => {
  it('总高 ∈ [7.5, 8.8]、bbox 冠幅 ∈ [5.5, 7.0]、视觉冠底/实高 ∈ [0.28, 0.36]（干高占比域 0.30–0.35 ±0.02 容差）', () => {
    const r = buildSlot(0);
    ...
    expect(width, 'slot-0 bbox 冠幅应落 8m 个体对标域（冠幅/树高比 ≈0.7–0.85 的 bbox 口径）').toBeGreaterThanOrEqual(5.5);
    expect(width).toBeLessThanOrEqual(7.0);
    const crownBase = metrics.minLeafY / height;
    expect(crownBase, 'slot-0 视觉冠底/实高应落 Spec 干高占比域（两样木 0.35/0.30 ±0.02）').toBeGreaterThanOrEqual(0.28);
    expect(crownBase).toBeLessThanOrEqual(0.36);
  }, 120000);
});
...
describe('通透不变量与预算带（规范种子逐槽）', () => {
  it('外壳（q>0.6）保留率 ≥ canopyDensity×0.85（槽自适应）且 ≥ 内核（q<0.4）保留率×1.8（外密内疏）；总面数落 [28754, 40000]', () => {
    for (let slot = 0; slot < 8; slot++) {
      const { stats } = buildSlot(slot);
      const profile = CAMPHOR_SHAPE_PROFILES[slot]!;
      ...
      // 自适应（疏松槽 0.79 下 ≈0.79 为设计预期；固定 0.85 对密度缩放槽不适用）
