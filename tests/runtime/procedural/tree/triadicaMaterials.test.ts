 ...
 *   GLSL 全文逐位相等）；3 工厂 × 3 档 = 9 键互异 + 与 camphor/ginkgo/zelkova/
 *   platanus/koelreuteria 45 键零碰撞；叶 Mid 去叶脉三件/腺点/叶团/糙度叶团项
...
const sdfOf = (fragmentShader: string): string => {
  const start = fragmentShader.indexOf('float triLeafAlpha(vec2 triUv, float triRand)');
...
describe('分档实装（level 参数；缺省 high = 显式 high）', () => {
  ...
  it('与 camphor/ginkgo/zelkova/platanus/koelreuteria 45 键零碰撞（triadica 前缀不与先例混缓存）', () => {
    ...
    expect(foreignKeys.size).toBe(45);
