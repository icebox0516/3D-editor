/**
 * tests/runtime/procedural/tree/ginkgoLod.test.ts —— 银杏 LOD 三档几何不变量测试
 * （T011.4，对称榉树 zelkovaLod 组织——方法复制；快照数为银杏自己的实测锚）。
 *
 * 覆盖（零 mock——真实几何生成；直调 buildGinkgoGeometry 三档，与资产入口契约互补——
 * 本文件锁「档间不变量怎么成立」）：
 * - High 逐位不动：缺省调用（profile + level 双缺省）= 显式 'high' 逐位相等；slot-0
 *   皮面 20782 / rng 消费 82756 快照延续（快照口径 = 槽内跨档恒等；跨槽消费数随保留
 *   簇数变化——四先例同机制）；
 * - rng 三档恒等：同 seed 三档 rng 消费次数全等 82756（「发射省略不省略消费」纪律）；
 ...
 *   恒为单一贴地平移常量；j % 3 === 1 掩码只作用于烘焙阶段——莲座 8 选 3 / 散生
 *   L5 5 选 2 两类同掩码位）；Mid 逐簇烘焙叶量 ≤ High 同簇；卡数比 ≈ 0.376 统计口径
 *   复核（莲座 3/8 + 散生混合掩码率；探针实测 0.367–0.381）；slot-0/6/7 三槽取证
 *   （卡数最少 / 最多 + 锚点）；
 * - Low 壳卡账目：leafCards = 保留簇 × 2、leafTriangles = 保留簇 × 4（8 槽全扫）；
 * - 预算锁定（D19.8，家族行沿用）：8 槽 × 3 档总面落锁定预算 High ≤ 40000 且
 *   ≥ 27000（银杏参考下沿——扇形大卡覆盖率高、卡数天然低于小卡资产，低于夏栎参考
 *   28754 不阻塞，记档见资产模块头）/ Mid 6000–10000 / Low 1500–3000（实测带 High
 *   27892–31050 / Mid 6622–7774 / Low 1858–2262）；
 * - 恰 2 组（皮 0 / 叶 1，D15）；档间 bbox 一致：同槽三档 XZ 跨 / 总高差 ≤ 0.4m
 *   （LOW_SHELL_MARGIN 0.12 口径——银杏大卡尺度）、minY 三档全 0；
...
/** High slot-0 皮面快照（皮拓扑恒等不动——ginkgoStructure 同款锁） */
const BARK_TRIS_LOCK = 20782;
/** 档间 bbox 容差（LOW_SHELL_MARGIN 口径——见 ginkgoGeometry 常量注释） */
const SPAN_TOLERANCE = 0.4;

const built: GinkgoGeometryResult[] = [];

function track(result: GinkgoGeometryResult): GinkgoGeometryResult {
  built.push(result);
  return result;
}

/** 槽位构建（seed + 槽 profile + 档位——与资产路径 profileForSeed 同路由口径） */
function buildSlot(slot: number, level: ProceduralLevel = 'high'): GinkgoGeometryResult {
  return track(
    buildGinkgoGeometry(
      mulberry32(morphSeedOf('asset_tree_ginkgo', slot)),
      GINKGO_SHAPE_PROFILES[Math.min(slot, GINKGO_SHAPE_PROFILES.length - 1)]!,
      level,
    ),
  );
}
...
function spanOf(result: GinkgoGeometryResult): { xz: number; y: number; minY: number } {
...
function leafCardXZ(result: GinkgoGeometryResult): { keys: string[]; y0: number[] } {
...
describe('High 逐位不动（缺省档回归锁）', () => {
  it('缺省调用（profile + level 双缺省）= 显式 high：position/aBend 数组与 stats 逐位全等', () => {
    const a = track(buildGinkgoGeometry(mulberry32(SEED0)));
...
describe('Mid ⊂ High（j % 3 === 1 掩码只在烘焙阶段生效——莲座 8 选 3 / 散生 5 选 2 同掩码位）', () => {
  it('Mid 存活卡 XZ 位置块逐位 ∈ High 卡集（Y 差 = 单一贴地平移常量）、逐簇烘焙叶量 ≤ High 同簇、卡数比 ≈ 0.376（slot-0/6/7）', () => {
    for (const slot of [0, 6, 7]) {
      const high = buildSlot(slot);
      const mid = buildSlot(slot, 'mid');
      // 掩码统计口径：Mid 卡数 ≈ High 存活卡 × 0.376（莲座 3/8 + 散生 L5 2/5 混合——
      // ±20% 带内复核；探针实测 0.367–0.381）
...
describe('8 槽 × 3 档横扫（预算锁定 / 恰 2 组 / Low 壳卡账目 / 档间 bbox 一致 / 法线健康）', () => {
  it('总面落锁定预算（High 27–40K / Mid 6–10K / Low 1.5–3K，D19.8）；恰 2 组；同槽三档跨度差 ≤0.4m、minY 全 0；Low 壳卡 = 簇 × 2；法线有限', () => {
    for (let slot = 0; slot < 8; slot++) {
      ...
      for (const level of LEVELS) {
        ...
        // 预算锁定（锁定账目与依据见 asset_tree_ginkgo.asset 模块头）
        if (level === 'high') {
          expect(total, `slot-${slot} High 总面应 ≤ 40000（上限预算）`).toBeLessThanOrEqual(40000);
          // 银杏参考下沿 27000（扇形大卡覆盖率语义——低于夏栎参考 28754 不阻塞，记档）
          expect(total, `slot-${slot} High 总面应 ≥ 27000（银杏参考下沿）`).toBeGreaterThanOrEqual(27000);
        } else if (level === 'mid') {
