...
describe('物种配方锚定（Spec fraxinus-reference 1.0；生产口径 = 终审记档 ③ 裁决八项：一回奇数羽状对生严格 / 缘锐锯齿载波回归 / 树皮第 11 语言 / 翅果做 / 花不做）', () => {
  ...
  it('复叶数值锚（JS 镜像）：有效带数 2–3 对（run 高度 ≥0.040 计数——「小叶5-7枚」= 2–3 对 + 顶生统计实现）+ 顶生 reach ≥0.030 + 顶生/中位 1.01–1.16 + 跨轴错位 ≤0.012 + 叶轴连续 + 裸区零越界（减法门控回归锁）+ 顶栏残余压灭', () => {
  ...
  it('中距细碎自检剖面（Spec §3「细碎均质中绿 + airy 开放 空隙 25–40%」Inferred——对生羽叶层叠）：maxW 全宽细碎质域 + 中段缢缩行 ≥6 + 卡缘恒裁 + Low 单包络档间剪影一致', () => {
