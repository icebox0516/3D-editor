/**
 * tests/runtime/procedural/tree/ginkgoStage.test.ts —— window.__ginkgo DEV 出图面句柄
 * 测试（T011.4；榉树 zelkovaStage.test 同构复制——断言集逐条对应，数值面换银杏实数）。
 *
 ...
 * 数值锚（vs zelkova 差异面）：slot-0 High 皮 20782 / 叶 9676 / 4838 卡（T011.4 探针
 *   实测 = 资产预算锁定账目 triangleCount 30458 = 20782 + 4838×2，莲座+散生双挂点
 *   合计）；取景视心高 5.4（slot-0 冠域 2.68–8.17 冠心 ≈5.42——vs zelkova 5.3 /
 *   camphor 5.5 / celtis 4.3，ginkgoStage 模块头「树高参考」）。
 */
import { describe, expect, it, vi } from 'vitest';
import * as THREE from 'three';
import { createGinkgoHandle } from '../../../../src/runtime/procedural/tree/ginkgo/ginkgoStage';
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（皮 20782 / 叶 9676 / 4838 卡，T011.4 探针实测 = 预算锁定账目 slot-0 High）', () => {
    ...
    const handle = createGinkgoHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(20782);
    expect(stats.leafTriangles).toBe(9676);
    expect(stats.leafCards).toBe(4838);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置冠心 + update 被调', () => {
    ...
    const handle = createGinkgoHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈5.4（锚点树 8.17m 冠域 2.68–8.17 冠心 ≈5.42——vs zelkova 5.3）
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createGinkgoHandle({ scene });
    const b = createGinkgoHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ginkgo-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ginkgo-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createGinkgoHandle({
    ...
    const bare = createGinkgoHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：ginkgo-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createGinkgoHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('ginkgo-dev-slots');
  ...
  it('viewSlots：缺省机位球坐标公式复算落位 + target = 网格中心 (0, 5.4, spacing/2) + update 被调；viewSlot(5) 绕槽 5 树位；viewSlot(9/-1) 越界 warn no-op 不位移', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const handle = createGinkgoHandle({ scene, camera, controls });
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ginkgo-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createGinkgoHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('ginkgo-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('ginkgo-dev-levels');
  ...
  it('viewLevels：缺省机位（29/35/16）球坐标公式复算 + target = 排中心 (0, 5.4, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createGinkgoHandle({ scene, camera, controls, build: makeFakeBuild(log) });
    handle.mountLevels(); // spacing 11 → 排中心 = 组位原点
    handle.viewLevels(); // 缺省 29 / 35° / 16°——公式复算断言
    const d = 29;
    const az = (35 * Math.PI) / 180;
    const el = (16 * Math.PI) / 180;
    expect(camera.position.x).toBeCloseTo(d * Math.cos(el) * Math.cos(az), 5);
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createGinkgoHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createGinkgoHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
