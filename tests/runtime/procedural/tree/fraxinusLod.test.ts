 ...
 *   前用 build 实测核对的锁；资产入口未就绪（fraxinusMaterials 并行交付）时优雅跳过）**；
...
function track(result: FraxinusGeometryResult): FraxinusGeometryResult {
  built.push(result);
  return result;
}
...
function spanOf(result: FraxinusGeometryResult): { xz: number; y: number; minY: number } {
...
function leafCardXZ(result: FraxinusGeometryResult): { keys: string[]; y0: number[] } {
...
describe('High 逐位不动（缺省档回归锁）', () => {
  ...
  it('meta.triangleCount = slot-0 High 实数（皮 24178 + 复叶卡 ×2 + 翅果果 ×4 三账合计——写死 meta 前用 build 实测核对的锁；资产入口未就绪（fraxinusMaterials 并行交付）时优雅跳过，同 fraxinusShapeSlots 路由锁口径）', async () => {
    const assetModule = await import('../../../../src/runtime/procedural/assets/asset_tree_fraxinus.asset').catch(
      () => null,
    );
    if (!assetModule) {
      // asset_tree_fraxinus.asset 入口暂不可导入（fraxinusMaterials 并行交付未就绪）——
      // 实数锁延后到合并后自动生效（主代理合并验证面）；本测试不判失败
      console.warn('[fraxinusLod] asset_tree_fraxinus 入口暂不可导入（fraxinusMaterials 并行交付未就绪）——triangleCount 实数锁延后');
