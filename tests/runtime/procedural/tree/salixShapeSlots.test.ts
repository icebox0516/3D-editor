 ...
 *   crownWidthRatio 0.52 探针定档 1.140 的实测验收锁（喷泉链外泄系数 ×2.2 的回推，
 *   见 profile 注释））、视觉冠底/实高落带 [0.12, 0.30]（帘缘止于地上 1–2m [6]
 *   的标准槽读向；终测 0.197）；
 * - 8 槽 w/h 涌现带落 [1.0, 1.3]（Spec 域直读——slot-1 深垂帘窄端 1.046 / slot-6
 *   斜弯宽端 1.269 = 域两端身份；终测带 1.046–1.269）与 8 槽树高带 [8.3, 11.4]
 *   （Spec §2 生产锚 8–12m 带内；终测 8.43–11.22）；**垂幕段（腰口径）工程带
 *   [0.28, 0.45]**（喷泉腰（半径 ≥ 0.85×rmax 叶卡带）→ 帘缘的垂帘纵跨——照片
 *   判读域 2/5–2/3 含拱背垂索的保守口径记档；终测 0.326–0.400）；
...
describe('槽间真实差异（差异来自形态向量——垂幕幕缘方差记档下双口径断言）', () => {
  ...
  it('深垂帘 vs 浅垂帘（双口径——垂幕长度主轴）：①同种子面板帘缘差 slot-2 − slot-1 ≥ 0.4m（浅垂端帘缘更高的主方向）②规范种子帘缘绝对差 ≥ 0.4m + 深垂帘卡数 ≥ 浅垂帘 × 1.05（长链挂点多；终测帘缘 1.24 vs 2.10 / 卡数 5176 vs 4335）', () => {
  ...
  it('宽冠 vs 窄冠（双口径——冠幅比主轴）：①同种子面板冠幕宽度差（slot-3 − slot-4 面板均差 ≥ 0.3m）②规范种子 w/h 比差 ≥ 0.05（终测 slot-3 1.149 / slot-4 1.077；域两端身份另由 8 槽带断言覆盖）', () => {
    const panelSeeds = [SEEDS[0]!, SEEDS[1]!, SEEDS[2]!, SEEDS[3]!, 12345, 777777];
    let broad = 0;
    let narrow = 0;
    for (const seed of panelSeeds) {
...
describe('物种锚点（规范种子 slot-0——Spec 尺度与比例域）', () => {
  it('总高 ∈ [8.8, 10.8]、bbox 冠幅比 w/h ∈ [1.0, 1.3]、视觉冠底/实高 ∈ [0.12, 0.30]（帘缘止于地上 1–2m 标准槽读向；终测 h 9.83 / w/h 1.140 / 冠底 0.197——2026-09-23 终测）', () => {
    const r = buildSlot(0);
    const b = boxOf(r);
  ...
  it('8 槽 w/h 涌现带落 [1.0, 1.3]（Spec 终审修正域直读——slot-1 深垂帘窄端 / slot-6 斜弯宽端 = 域两端身份；终测 1.046–1.269）', () => {
    for (let slot = 0; slot < 8; slot++) {
  ...
  it('8 槽树高带 [8.3, 11.4]（Spec §2 生产锚 8–12m 带内；终测 8.43–11.22——slot-6 斜弯矮端 / slot-1 深垂帘高端）+ 垂幕段（腰口径）工程带 [0.28, 0.45]（照片判读域 2/5–2/3 含拱背垂索的保守口径记档；终测 0.326–0.400）', () => {
    for (let slot = 0; slot < 8; slot++) {
