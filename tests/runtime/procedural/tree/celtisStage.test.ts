/**
 * tests/runtime/procedural/tree/celtisStage.test.ts —— window.__celtis DEV 出图面句柄
 * 测试（T011.1；夏栎 tree3aStage.test 同构复制——断言集逐条对应，数值面换朴树实数）。
 *
 ...
 *   castShadow/customDepthMaterial；stats 逐槽账目（皮面数槽间恒等 24178 = 拓扑不变量、
...
import { createCeltisHandle } from '../../../../src/runtime/procedural/tree/celtis/celtisStage';
import { meta } from '../../../../src/runtime/procedural/assets/asset_tree_celtis.asset';
 ...
 *  （朴树 build 自 T011.1 起返回深度材质；不带字段的 makeFakeBuild 即回退路径） */
...
describe('mount / unmount / stats', () => {
  it('mount：group 挂 scene + stats 账目 = slot-0 锚点实数（皮 24178 / 叶 15188 / 7594 卡，T011.1 预算锁定账目 slot-0 High）', () => {
    const scene = new THREE.Scene();
    const handle = createCeltisHandle({ scene });
    handle.mount();
    expect(scene.children).toHaveLength(1);
    const stats = handle.stats();
    expect(stats.mounted).toBe(true);
    expect(stats.barkTriangles).toBe(24178);
    expect(stats.leafTriangles).toBe(15188);
    expect(stats.leafCards).toBe(7594);
    handle.dispose();
...
describe('view 取景（结构桩相机/控制）', () => {
  it('view：相机球坐标落位 + target 置树心 + update 被调', () => {
    ...
    const handle = createCeltisHandle({ scene, camera, controls });
    handle.mount({ x: 2, z: 4 });
    handle.view({ distance: 30, azimuthDeg: 0, elevationDeg: 0 });
    // az 0°/el 0°：正 +X 方向距离 30，视心高 ≈4.3（锚点树 8.57m 半高）
    expect(camera.position.x).toBeCloseTo(32, 5);
    expect(camera.position.y).toBeCloseTo(4.3, 5);
...
describe('StrictMode 双挂载安全（dispose 只摘自己的）', () => {
  it('同 scene 两句柄：dispose(A) 后 B 仍挂载', () => {
    const scene = new THREE.Scene();
    const a = createCeltisHandle({ scene });
    const b = createCeltisHandle({ scene });
    a.mount();
    b.mount({ x: 10 });
    a.dispose();
    expect(scene.children).toHaveLength(1);
...
describe('mountWindDemo / freezeTime（风动验收载体）', () => {
  it('默认 3 实例 InstancedMesh 一排：aSeed InstancedBufferAttribute 值互异 ∈[0,1)；间距 8m；customDepthMaterial 叶影裁切', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('celtis-dev-wind');
  ...
  it('mountWindDemo(count) 自定义实例数；与 mount 互斥（重建先摘）；unmount 清理幂等', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('celtis-dev-stage');
  ...
  it('freezeTime/unfreezeTime 转调注入 time deps；未注入 no-op 不抛', () => {
    const calls: string[] = [];
    const handle = createCeltisHandle({
    ...
    const bare = createCeltisHandle({ scene: new THREE.Scene() });
...
describe('mountSlots / viewSlots / viewSlot（8 槽批量出图面）', () => {
  it('mountSlots：celtis-dev-slots 组内 8 独立 Mesh 4×2 行主序网格（spacing 11）；castShadow + customDepthMaterial 与单树同待遇；转台对 slots 组无 rAF 安全', () => {
    const scene = new THREE.Scene();
    const handle = createCeltisHandle({ scene });
    handle.mountSlots();
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('celtis-dev-slots');
  ...
  it('stats（slots 模式）：8 项逐槽账目，皮面数槽间恒等 24178（拓扑不变量）+ 顶层合计 8×；叶卡数只断言 > 0（不锁具体值——数值归资产侧测试）', () => {
    const scene = new THREE.Scene();
    const handle = createCeltisHandle({ scene });
    handle.mountSlots();
    const stats = handle.stats();
    ...
    for (let slot = 0; slot < 8; slot++) {
      ...
      expect(entry.barkTriangles).toBe(24178); // 皮拓扑槽间恒定
      expect(entry.leafCards).toBeGreaterThan(0); // 叶卡数随簇级剔除定——不锁具体值
      expect(entry.leafTriangles).toBe(entry.leafCards * 2); // 卡 = 2 三角
    }
    expect(stats.barkTriangles).toBe(8 * 24178); // 顶层保持「总量」语义 = 8 棵合计
    expect(stats.leafCards).toBe(stats.slots!.reduce((sum, s) => sum + s.leafCards, 0));
    handle.dispose();
  }, 60000);

  it('viewSlots：缺省机位球坐标公式复算落位 + target = 网格中心 (0, 4.3, spacing/2) + update 被调；viewSlot(5) 绕槽 5 树位；viewSlot(9/-1) 越界 warn no-op 不位移', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const handle = createCeltisHandle({ scene, camera, controls });
  ...
  it('互斥与幂等：mountSlots ↔ mount/mountWindDemo 重建 scene.children 恒 1；自定义 spacing 生效；unmount/dispose 幂等、slots 模式 stats 归零', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('celtis-dev-slots');
...
describe('mountLevels / viewLevels / viewLevel（档位强制出图面——deps.build 注入 fake）', () => {
  it('mountLevels：build 恰调 3 次、同 seed + level 依次 high/mid/low（透传）；3 独立 Mesh 一字排开（−11/0/+11）castShadow + 逐档深度材质互异；slot/spacing 自定义；slot 越界 warn no-op 不拆现场', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createCeltisHandle({ scene, build: makeFakeBuild(log) });
    ...
    expect(scene.children).toHaveLength(1);
    const group = scene.children[0]!;
    expect(group.name).toBe('celtis-dev-levels');
    const meshes = group.children as THREE.Mesh[];
  ...
  it('互斥：mountLevels 先释放 mountSlots 既有 8 份几何（dispose 计数）；反向 mount 释放 levels 3 份；账目字段让位', () => {
    ...
    expect(scene.children).toHaveLength(1);
    expect(scene.children[0]!.name).toBe('celtis-dev-levels');
  ...
  it('viewLevels：缺省机位（32/35/16）球坐标公式复算 + target = 排中心 (0, 4.3, 0)；viewLevel 三档特写绕档树位；未知档位 warn no-op 不动相机', () => {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera();
    const controls = makeControlsStub();
    const log = makeLog();
    const handle = createCeltisHandle({ scene, camera, controls, build: makeFakeBuild(log) });
...
describe('customDepthMaterial 同源消费（SOP §1.4「DEV 同源」：源带字段消费之，fake 不带回退自建）', () => {
  it('mount：源带深度材质 → mesh.customDepthMaterial 与 source 同引用（不自建）；unmount 随 disposeSource 释放恰一次，幂等不重复', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createCeltisHandle({ scene, build: makeFakeBuildWithDepth(log) });
    handle.mount();
  ...
  it('回退路径对账：fake build 不带字段 → 回退自建深度材质挂载并随 unmount 释放恰一次（既有行为不回归）', () => {
    const scene = new THREE.Scene();
    const log = makeLog();
    const handle = createCeltisHandle({ scene, build: makeFakeBuild(log) }); // 不带深度字段
    handle.mount();
    const mesh = scene.children[0]!.children[0] as THREE.Mesh;
