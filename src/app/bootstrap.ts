...
import { createGinkgoHandle } from '../runtime/procedural/tree/ginkgo/ginkgoStage';
import type { GinkgoHandle } from '../runtime/procedural/tree/ginkgo/ginkgoStage';
import { createPlatanusHandle } from '../runtime/procedural/tree/platanus/platanusStage';
...
export function createEditor(canvas: HTMLCanvasElement | null, opts: CreateEditorOptions = {}): EditorHandle {
  ...
  // T011.4 DEV 出图面：window.__ginkgo（银杏 slot-0 锚点树直挂渲染场景——夏栎 __tree3a /
  // 朴树 __celtis / 香樟 __camphor / 榉树 __zelkova 同构装配：独立 group 挂 scene 兄弟组
  // 不参与拾取；mount/mountSlots 8 槽批量 / mountLevels 三档对照 / 风动 / freezeTime /
  // 固定机位 view 系供视觉取证与档位生成验证。实现全在
  // runtime/procedural/tree/ginkgo/ginkgoStage——组合根只装配，dispose 只摘自己的实例）。
  let ginkgo: GinkgoHandle | null = null;
  if (import.meta.env.DEV && renderer && typeof window !== 'undefined') {
    ginkgo = createGinkgoHandle({
      scene: renderer.scene,
      camera: renderer.camera,
      controls: renderer.controls,
      time: renderer.uTime,
    });
    window.__ginkgo = ginkgo;
  }
  // T011.5 DEV 出图面：window.__platanus（悬铃木 slot-0 锚点树直挂渲染场景——夏栎 __tree3a /
  // 朴树 __celtis / 香樟 __camphor / 榉树 __zelkova / 银杏 __ginkgo 同构装配：独立 group 挂
  // scene 兄弟组不参与拾取；mount/mountSlots 8 槽批量 / mountLevels 三档对照 / 风动 /
  // freezeTime / 固定机位 view 系供视觉取证与档位生成验证。实现全在
  // runtime/procedural/tree/platanus/platanusStage——组合根只装配，dispose 只摘自己的实例）。
  let platanus: PlatanusHandle | null = null;
  if (import.meta.env.DEV && renderer && typeof window !== 'undefined') {
    platanus = createPlatanusHandle({
      scene: renderer.scene,
      camera: renderer.camera,
      controls: renderer.controls,
      time: renderer.uTime,
    });
    window.__platanus = platanus;
  }
  // T011.6 DEV 出图面：window.__koelreuteria（栾树 slot-0 锚点树直挂渲染场景——夏栎
  // __tree3a / 朴树 __celtis / 香樟 __camphor / 榉树 __zelkova / 银杏 __ginkgo / 悬铃木
  // __platanus 同构装配：独立 group 挂 scene 兄弟组不参与拾取；mount/mountSlots 8 槽
  // 批量 / mountLevels 三档对照 / 风动 / freezeTime / 固定机位 view 系供视觉取证与
  // 档位生成验证。实现全在
  // runtime/procedural/tree/koelreuteria/koelreuteriaStage——组合根只装配，dispose 只摘
  // 自己的实例）。
  let koelreuteria: KoelreuteriaHandle | null = null;
  if (import.meta.env.DEV && renderer && typeof window !== 'undefined') {
    koelreuteria = createKoelreuteriaHandle({
      scene: renderer.scene,
      camera: renderer.camera,
      controls: renderer.controls,
      time: renderer.uTime,
  ...
  // T011.8 DEV 出图面：window.__bischofia（重阳木 slot-0 锚点树直挂渲染场景——夏栎
  // __tree3a / 朴树 __celtis / 香樟 __camphor / 榉树 __zelkova / 银杏 __ginkgo / 悬铃木
  // __platanus / 栾树 __koelreuteria / 乌桕 __triadica 同构装配：独立 group 挂 scene 兄弟
  // 组不参与拾取；mount/mountSlots 8 槽批量 / mountLevels 三档对照 / 风动 / freezeTime /
  // 固定机位 view 系供视觉取证与档位生成验证。实现全在
  // runtime/procedural/tree/bischofia/bischofiaStage——组合根只装配，dispose 只摘自己的
  // 实例）。
  let bischofia: BischofiaHandle | null = null;
  if (import.meta.env.DEV && renderer && typeof window !== 'undefined') {
    bischofia = createBischofiaHandle({
      scene: renderer.scene,
      camera: renderer.camera,
      controls: renderer.controls,
      time: renderer.uTime,
  ...
  // T011.11 DEV 出图面：window.__ligustrum（女贞 slot-0 锚点树直挂渲染场景——夏栎
  // __tree3a / 朴树 __celtis / 香樟 __camphor / 榉树 __zelkova / 银杏 __ginkgo / 悬铃木
  // __platanus / 栾树 __koelreuteria / 乌桕 __triadica / 重阳木 __bischofia / 国槐
  // __sophora / 白蜡 __fraxinus 同构装配：独立 group 挂 scene 兄弟组不参与拾取；
  // mount/mountSlots 8 槽批量 / mountLevels 三档对照 / 风动 / freezeTime / 固定机位
  // view 系供视觉取证与档位生成验证。实现全在
  // runtime/procedural/tree/ligustrum/ligustrumStage——组合根只装配，dispose 只摘自己
  // 的实例）。
  let ligustrum: LigustrumHandle | null = null;
  if (import.meta.env.DEV && renderer && typeof window !== 'undefined') {
    ligustrum = createLigustrumHandle({
      scene: renderer.scene,
      camera: renderer.camera,
      controls: renderer.controls,
      time: renderer.uTime,
  ...
  const facade: EditorHandle = {
    ...
    dispose(): void {
      ...
      // T011.4 DEV 出图面成对拆除（同上：仅摘自己的树与 window 槽）
      if (ginkgo && typeof window !== 'undefined' && window.__ginkgo === ginkgo) {
        ginkgo.dispose();
        delete window.__ginkgo;
      }
      // T011.5 DEV 出图面成对拆除（同上：仅摘自己的树与 window 槽）
      if (platanus && typeof window !== 'undefined' && window.__platanus === platanus) {
        platanus.dispose();
        delete window.__platanus;
      }
      // T011.6 DEV 出图面成对拆除（同上：仅摘自己的树与 window 槽）
      ...
      // T011.8 DEV 出图面成对拆除（同上：仅摘自己的树与 window 槽）
      ...
      // T011.11 DEV 出图面成对拆除（同上：仅摘自己的树与 window 槽）
      if (ligustrum && typeof window !== 'undefined' && window.__ligustrum === ligustrum) {
        ligustrum.dispose();
