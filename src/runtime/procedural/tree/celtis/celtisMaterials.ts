 ...
 *   - 齿限上半部（身份核心）：门控 smoothstep(0.42, 0.56, v) 只放叶上半段齿，下半部近全缘
 ...
 * smoothstep(0.42,0.56,v)，下半部近全缘——四源一致核心辨识 Verified [1][3][5][6]；圆钝齿
...
float t3cLeafAlpha(vec2 t3cUv, float t3cRand) {
  ...
  float t3cGate = smoothstep(0.42, 0.56, t3cP.y); // 齿限上半部（下半部近全缘——四源一致核心辨识）
...
// 弱对比 0.18）；浅黄绿脉色（脉比叶肉亮）；纯 ALU 零采样。Step 4 校准：带宽/权重/脉色
// 上调（albedo 域亮度差 中脉 16% / 三出脉 12.4%——宏距可辨下限 ≥12%）
vec2 t3cP = vec2(vUv.x - 0.5, vUv.y);
t3cP.x -= 0.012 * t3cP.y; // 与 SDF 同款中心线漂移（叶脉跟随叶轴）
float t3cVeinMid = 1.0 - smoothstep(0.008, 0.030, abs(t3cP.x)); // Step 4：带半宽 ×1.5（1.24→1.86px@100px 叶——窄带 AA 后消失）
float t3cTriPath = 0.30 * pow(t3cP.y, 0.45) * (1.0 - 0.45 * t3cP.y); // 基出侧脉轨迹：自叶基急升后近叶缘平行内行
float t3cVeinTri = (1.0 - t3cVeinMid)
  * (1.0 - smoothstep(0.010, 0.040, abs(abs(t3cP.x) - t3cTriPath))) // Step 4：带半宽 ×1.33（1.86→2.5px）
...
t3cMul = mix(t3cMul, t3cMul * vec3(1.28, 1.19, 0.74), t3cVeinMid * 0.85 + t3cVeinTri * 0.65 + t3cVeinLat * 0.18); // Step 4：脉对比校准（中脉 6.5%→16%、三出脉 4.4%→12.4% albedo 亮度差）
