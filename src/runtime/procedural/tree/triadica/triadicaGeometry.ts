/**
 * runtime/procedural/tree/triadica/triadicaGeometry —— 乌桕（Triadica sebifera
 * (Linnaeus) Small，大戟科乌桕属落叶乔木——长江流域城市公园夏绿单干中龄个体 ≈9.5m；
 * 旧口径 Sapium sebiferum 记档见 Spec §1）CPU 几何生成器（T011.7，阔叶家族第八实例
 * ——方法复制自夏栎第一实例经朴树/香樟/榉树/银杏/悬铃木（T011.5 单叶直接先例）/栾树
 * （T011.6 最新模板——零 rng 花果账目 + uv 域隔离断言）七次验证的通路
 * ../platanus/platanusGeometry，契约字段语义不变：五级递归分枝拓扑 / 锥度管状枝干 /
 * 枝梢驱动叶簇 / 冠内通透三规则 / 树皮近景微起伏 / LOD 三档同流派生全部沿用；乌桕
 * 数值与算法细节差异点见下）。
 *
 * 职责：morphRng（mulberry32 流，消费顺序即契约——同 seed 逐位同结果）驱动的五级递归
 *      分枝拓扑 → 锥度管状枝干（平行传输标架）→ 枝梢驱动**菱形中卡**烘焙 + **绿闭
 *      蒴果序弯垂轴**（夏果），产出树皮/叶两层非索引几何；层间 mergeGeometries
 *      (useGroups=true) 恰 2 组（D15 免组膨胀：皮 0 / 叶 1——**果序入皮组（组 0）**，
 *      材质接口约定见下）。形态参数类型 = 阔叶家族契约 ../broadleaf/broadleafShapeProfile
 *      （乌桕为第八实例），数值与槽组合见 ./triadicaShapeProfile（全部数值依据
 *      docs/research/triadica-reference.md Spec 1.0 生产取用版（正文 + 终审记档：
 *      树高锚 ≈9.5m 主代理裁定、外缘微垂不建下垂语言、冠幅比 0.8–1.0 维持））；
 *      build 只做 slot → profile 路由，不进 ProceduralBuild 公共签名。
 * 乌桕算法细节差异点（vs platanus 模板，逐条 Spec 引用见 triadicaShapeProfile 内联
 * 注释；主代理 Step 1 判定 3–10 为执行依据）：
 *   - 尺度锚：长江流域公园夏绿单干中龄个体 ≈9.5m（主代理裁定——终审 9–10m 带内中值；
 *     与栾树 ≈10m 相当、低于悬铃木 12m、高于夏栎 8m 的速生开展等幅冠）——形态参数域
 *     树高 9.2–9.8m；净干参数域 0.17–0.26（Spec 域扩展节 A 0.15–0.25 单源 Inferred
 *     + 主代理带内推 0.15–0.30）；根径 0.26–0.32m（照片 20–40cm 无标尺推断——工程
 *     设定，速生比例瘦干读向）。
 *   - 干形：近直立单干（多干/萌生变体现象 Verified 频率 Unknown——**记档不建模**：
 *     结构差异不落连续参数，platanus 低位双主枝/栾树多干同款纪律）→ 主干倾轴幅度
 *     ≤2.9°（jitter 0.05）；根部 flare 轻度 1.18×（basal_flare Unknown——家族常量）。
 *   - 树形：**开展圆头（rounded-spreading）**（NC Rounded Verified [5] + form-a
 *     「开展-圆形树冠、顶部微圆穹、轮廓不规则圆滑」Inferred [7]）——几何侧驱动链
 *     （T009.3 消费语义）：**广角骨架**（50–68° 对铅垂——FRPS「枝广展」Verified [1]
 *     + 照片 45–90° Inferred [7]，七实例最开展起角带）+ **干向挂点高度分级**
 *     （SCAFFOLD_LENGTH_TAPER ×1.06→×0.74 低枝长高枝短 + SCAFFOLD_ANGLE_TAPER 低枝
 *     +12…14° 平展（冠基放张）/ 高枝 −6…−8° 收角（圆头闭合）——确定性零 rng，platanus
 *     先例方法复制）+ **upturn 链低正递减**（[0.20,…,0.08]——末级细枝上举 20–30°
 *     （bark-c 判读 [7]）由低正 upturn + 游走复合，**冠缘微垂读向由末级低值 + 冠缘
 *     平展承载（终审裁决 2——不建负链下垂枝语言**，区别栾树 L5 −0.02））+ 散布方位
 *     （螺旋互生散布——骨架排列 Unknown [7]）+ 领导中庸偏弱 0.48（apical_dominance
 *     中庸-弱 [7]——开展圆头顶由骨架链共构、领导不翻转树顶）+ 冠幅比工程域 0.8–1.0
 *     （crownWidthRatio 0.62 探针定档涌现 0.861——外泄 ×1.39：广角骨架水平伸长但
 *     中卡簇内收于冠参数域，开展等幅读向由冠参数直推；8 槽带 0.700–0.986，slot-1
 *     幼相窄端 0.700 微出下沿记档）。
 *   - **互生单叶「中卡簇」挂点语言（乌桕独有身份，Spec §4/域扩展节 B
 *     leaf_attachment_rule：单叶互生、散生于枝非簇生——FRPS「叶互生」Verified [1][3]
 *     + 照片三源 [7]）**：家族「叶簇」槽位 = 末级枝互生散布中卡簇——簇内叶方位 **黄金
 *     角互生螺旋**（φ_j = j×137.5° + 抖动——互生叶序方位语言继承 platanus）+ **卡面
 *     平展摊开略前倾**（PLANE_LIFT 0.13——长柄叶多向摊开、层叠轻盈（域扩展节 B
 *     leaf_orientation_dist Inferred [5][7]——柄 ≈ 等叶长的飘逸感取向）+ FORWARD_TILT
 *     0.26）+ 弱抖动（WOBBLE 0.16）；每簇 8 候选（真叶互生密节距 × 卡宽 0.08–0.14 的
 *     连续节位合并抽象——介于朴/樟密簇 18 与悬铃木疏簇 5 之间「叶中细质、量大轻盈」
 *     [5][7]），通透过滤后逐簇存活典型 4–7 片；**卡宽 0.08–0.14 × 长宽比（长/宽）
 *     0.80–1.20 近等宽**（真叶 3–9(13)×3–9cm ×≈2 工程映射 + FOC 检索表 "nearly as
 *     long as wide" Verified [4]——菱形本质，家族第七种叶形语言）；无长短枝二型；
 *     中簇（0.11–0.16）+ 中间距抑制（minSeparation 0.54×径和——散布平摊读向
 *     platanus 同向）。
 *   - **绿闭蒴果序（Spec 判定做——§4 果序姿态与账目重点问题 Verified [1][3][5][7]
 *     + 终审 ③-5 复核链：夏季相树上果 = 绿色闭合小蒴果（四月绿幼果 → 七月末绿闭果
 *     簇挂枝顶三源））**：径 1–1.5cm 梨状球形三室 ×2 工程映射 → 径 ≈2.4–3.0cm（半径
 *     0.012–0.015m）；**沿弯垂总状轴散挂枝顶/冠缘外段**（顶生花序语言 [1][3] + 果期
 *     轴弯垂 [7]——果-c「拱弯-下垂果序轴、绿闭蒴果沿轴排布」；区别 platanus 长梗
 *     垂挂冠下缘点缀与栾树冠缘灯笼串）；每轴 2–4 果、步距 0.09–0.13m 沿轴散布非密串
 *     （果序至 28cm [3] 的散布读向）；总量「显著性低-中」克制（中距可辨但不主导——
 *     悬铃木 84–131 球量级下调，探针定档：实测 60–84 果/树）；**确定性挂点账目法零
 *     rng**（栾树先例优先——保留 L5 簇位冠外域判据（q ≥ 0.40——「枝顶/冠缘外段」
 *     的径向滤；顶生语义 = 仅 L5 簇位资格）+ 位置散列排序 + 固定步长抽选（STRIDE 8）
 *     ——组 0 带随槽仅随保留簇数平滑变化，规避 platanus 果序 rng roll 组 0 带状浮动
 *     先例）；表达 = **八面体最小面数 8 tri/果**（platanus 果球方法复制，flat 法线）；
 *     **账目入皮组（组 0）**——triadicaMaterials（park-shader-agent 并行交付、签名
 *     冻结：createTriadicaBarkMaterial / createTriadicaLeafMaterial /
 *     createTriadicaLeafDepthMaterial，均 (level?) => 材质）按 uv v 域分流绿果配方
 *     （**果序顶点 uv v∈[4,5] 果域 + u = 逐果熟度 roll ∈ [0,1)**（位置散列、果内
 *     24 顶点同值——材质绿果变奏通道，冻结接口）+ aLeafRand/aBend 随皮组恒 0（刚性
 *     悬垂——风动语义归材质层）+ 深度材质皮组 aLeafRand=0 实心守卫天然覆盖果影）；
 *     **皮管弧长域不得侵入 [4,5)**（9.5m 级最长枝弧 ×0.5 实测 ≤2.31——与叶卡 0–1 四边形
 *     域双重隔离的几何身份标记；隔离带 [2.5,4) 空 + 果域顶点数断言在 triadicaStructure
 *     测试锁）；Mid 保留全量（身份信号）/ **Low 省略**（远距 ≈3cm 果亚像素——省略
 *     记档，档间连续同 platanus/栾树果序先例）。
 *   - **花：不做**（顶生总状黄绿穗 6–20cm——色近叶弱显著、牺牲顺位第 3，主代理判定；
 *     记档 triadicaShapeProfile 模块头不建模清单）；**白蜡种子相不做**（非夏季主相
 *     ——同秋色纪律记档）；**秋色红不建模**；**腺体几何不做**（毫米级归材质层——
 *     triadicaMaterials 承载，几何侧只保证叶卡 uv 域/顶点属性契约不动）。
 *   - 骨架：**6 骨架枝 + 1 领导枝**（scaffold_branch_count Unknown + 开展等幅冠多枝
 *     平展读向——拓扑 L1=7 → 枝数 [7,21,63,189,378]、簇位 945（L4 189×1 + L5 378×2）、
 *     皮面 24178——9.5m 开展冠的中卡中量预算余量）。
 *   - 密度语言：中-疏轻盈（空隙 40–55% form-b 单源 Inferred [7] 带中带——七实例最疏
 *     带）：壳带 0.50 + 芯层地板 0.06（开放空隙 [7]）+ 大空腔 0.52–0.92 + 散布平摊
 *     浅壳（clusterShellBias 0.42——互生散布平摊）。
 *   - 树皮近景微起伏：**暗灰-灰褐窄纵裂 + 窄条翘皮中-深浮雕**（第 7 树皮语言——FRPS
 *     「树皮暗灰色，有纵裂纹」Verified [1] + NC "peel off in vertical, narrow strips"
 *     [5] + 照片四源 [7]；中龄档灰褐浅-中纵裂）——amplitudeRatio 0.027（浅-中裂到
 *     中裂带——朴 0.016/悬铃木-榉 0.014–0.015 光滑族与银杏 0.030/夏栎 0.033/樟
 *     0.036 深裂族之间；**暗色调/窄条翘皮/幼干灰绿/地衣银斑主体在 triadicaMaterials
 *     材质层**，geometry 只管纵裂浮雕起伏）+ 谐波 {3,4,5}（窄纵脊族——脊少而窄长）
 *     + drift 2.8 rad/m（纵裂连续族低值——轴向去相关 ≈2.24m 纵脊长程连续，区别
 *     悬铃木/榉断裂拼贴 11）。机制不变：纯确定性函数零 rng；幅度 ∝ 局部半径（0.027
 *     下起径 <0.056m 的管平滑发射——主干 + L1 骨架（起径 0.09–0.11）有效起伏、L2
 *     以下细枝光滑的读向自然涌现）；主干 radial 14（奈奎斯特域 7 容纳 k=5 留 2 档
 *     边际）。
 * 不建模（Spec 有事实、几何不表达——记档见 triadicaShapeProfile 模块头）：菱形轮廓
 *   细节/骤尖尾头/全缘/基部阔楔-浅心/侧脉弯拱网结（归 triadicaMaterials SDF）/ 叶柄
 *   与柄顶 2 腺体（毫米级——归材质层）/ 托叶 / 花（判定不做）/ 白蜡种子相 / 秋色红 /
 *   新叶铜红 / 小枝亮绿皮孔色序（归材质）/ 地衣银斑（归材质候选）/ 多干萌生（记档不
 *   建模）/ 乳状汁液毒性（非形态）。
 ...
 *      消费次数恒定），**果序挂点零 rng**（确定性账目——无消费口径问题），保留簇数
 *      与实际叶卡数随 seed 由簇级距离抑制 + 通透规则确定（同槽同 seed 恒等——确定性
 *      不破）。
 * 确定性纪律（家族纪律原样沿用）：簇生成与叶片候选的 rng 消费均为无条件固定次数（每簇
 *      2 次 + 每中卡 8 次），被簇级距离抑制丢弃的簇位足额消费后丢弃；通透 roll 每卡
 ...
 *      候选数——簇级剔除的簇位不产候选，七先例同机制记档）——槽内三档恒等（同 seed
 *      同保留簇集）、跨槽/跨 seed 随保留簇数浮动。
 * 叶卡属性契约（008.3 起冻结，本任务只消费不修改）：
 *      - aLeafRand：Float32 itemSize 1，逐叶 ∈ [0,1)，同一叶卡 6 顶点同值；皮组
 *        （含果序）顶点树皮语义位恒 0（皮组全 0——果序随组恒 0）；
 *      - aBend：Float32 itemSize 1 ∈ [0,1]，卡内根→尖非降（根 0.12·hw / 尖 0.52+
 *        0.44·hw，hw = 冠内高度权重）；树皮层同名属性写恒等值 0；**果序恒 0**（随
 *        皮组——刚性悬垂，弯垂轴摆动语义归 triadicaMaterials 风动层，几何侧不预置
 *        摆幅）。
 * 边界：零外部模型（D13）；每次调用全部数据现算（无模块级缓存对象，D17 所有权随调用
 *      移交）；原点 = 底部中心（整体 Y 平移使 minY 精确 = 0）；叶卡 UV 标准 0–1 四边形
 *      域（菱形叶 SDF alpha 消费归 triadicaMaterials——并行交付，签名冻结）；**果序
 *      UV v∈[4,5] 果域 + u = 逐果熟度**（几何身份标记——果序入皮组接口记档，见上）；
 *      叶法线取卡面单侧（cross(side, dir)）。
 * LOD 三档（T011.7，家族方法逐位复制：level 为 Runtime 可选参数——不参与
 *      shapeSlot/morphSeed/sourceKey 形态身份计算；档位缓存维度 = sourceKey + level 归
 *      ProceduralSourceCache）：三档共用**同一条 rng 消费流**与同一套骨架/簇位/果序
 *      决策路径，Mid/Low 只在「发射」阶段降密度/降段数——被省略发射的站点/叶候选/果序
 *      照常决策（果序零 rng，决策 = 同一簇位表 + 同一区域判据 + 同一散列抽选——逐位
 *      同源），枝路径/簇位/果序挂点/冠形包络/通透过滤决策逐位同源：
 ...
 *        j % 3 === 1（中卡簇 8 候选中 3 张（37.5%）——被弃候选足额消费 rng 后不进
 *        烘焙；通透过滤对全候选照常执行 → Mid 存活卡 ⊂ High 存活卡逐位同位）+ 树皮
 *        微起伏保留 + **果序全量保留**（身份信号——绿闭果中距可读 [7]，8 tri/果预算
 *        轻）；皮 4514；
 *      - Low：主干 + L1 骨架极简管（径向 6/4、隔 1 抽 1；递归照常走完消费 rng）+
 *        冠 = High 簇位表驱动的壳层卡（每保留簇 2 张交叉竖卡，半幅 = 簇半径 + 0.10m
 *        余量（中卡尺度：典型卡长 ≈0.08–0.18m，margin 取小卡档覆盖中卡常态伸出，
 *        极值叶尖仍由 High 决定冠包络）+ **果序不发射**（Low 观距 ≈3cm 果亚像素——
 *        省略记档，rng 照常消费）；皮 370。
 *      预算锁定账目（8 槽 × 3 档实测带 + 锁定依据）见
 *      ../assets/asset_tree_triadica.asset 模块头（预算制 D19.8，家族行沿用）。
 */
...
interface LeafCard {
  ...
  dir: THREE.Vector3; // 卡长方向（根→尖；散簇叶恒指簇外平展——卡根朝簇心）
...
interface FruitCapsule {
  center: THREE.Vector3;
  radius: number;
  /** uv u = 逐果熟度 roll ∈ [0,1)（位置散列——材质绿果变奏通道：绿闭果熟度梯度
   *  （绿 → 转黑前夜 [1][5]）的近似通道，冻结接口） */
  maturityRoll: number;
}

/** 果序挂点记录：1 条弯垂总状轴（2–4 果散布） */
interface FruitSite {
  capsules: FruitCapsule[];
}
...
interface BuildCtx {
  ...
  fruit: FruitSite[]; // 果序挂点表（保留 L5 簇位确定性抽选——档间同源）
...
export interface TriadicaGeometryResult {
  ...
  stats: {
    ...
    leafCards: number;
    /** 绿闭果序账目：挂点数（1 条弯垂轴/点）/ 烘焙果数 / 果序三角（果 × 8；Low = 0
     *  省略）。守恒：组 0 三角 = 皮拓扑（barkTriangles）+ fruitTriangles（果序入皮组
     *  记档——材质接口约定）、组 1 三角 = leafTriangles（纯叶卡） */
    fruitSites: number;
    fruitBalls: number;
...
// ── 乌桕互生单叶「中卡簇」挂点语言常量（结构常数：槽间恒等——改值即改面数与 rng 消费）──

/** 黄金角（互生叶序螺旋步进——FRPS「叶互生」Verified [1][3] 的簇内方位语言：φ_j =
 *  j×137.5° + 抖动，语言继承 platanus（vs 银杏莲座均分 2π/n——互生 vs 轮生的方位分化） */
const GOLDEN_ANGLE = 2.39996;
/** 散簇中卡取向常量（互生平展语言——连续形态方法常量，非槽差异维度）：长柄叶多向摊开、
 *  层叠轻盈（域扩展节 B leaf_orientation_dist Inferred [5][7]——柄 ≈ 等叶长「纤细
 *  飘逸」[1] 的飘逸感取向：平展摊开 + 略前倾 + 弱抖动） */
const ALT_FORWARD_TILT = 0.26; // 沿枝向前倾分量（× 簇方向 T）
const ALT_WOBBLE = 0.16; // 弱球面抖动（互生自然不齐）
const PLANE_LIFT = 0.13; // 叶尖上举分量（平展摊开——长柄叶多向摊开 [5][7]，略高于悬铃木 0.12）

/** 干向挂点高度分级锥度（开展圆头剪影驱动：挂点低枝 ×1.06 → 高枝 ×0.74 的确定性线性
 *  分级，零 rng——低枝略长（冠基放张）/ 高枝短（圆头收拢）；与 rank 环内势差正交；
 *  广角 + 本分级 + upturn 低正链 = 开展圆头冠的骨架侧驱动链，Spec §3 开展圆头
 *  Inferred+Verified [5][7]。**探针记档（2026-09-21）**：初值角度 54–72° + 分级 [1.10, 0.72] 下 8 槽 w/h
 *  0.50–1.05 参差（低枝平展 + 长枝双放大的槽间离散），回调角度 50–68° + 分级
 *  [1.06, 0.74] 后 8 槽带 0.700–0.986 收进开展域（slot-1 幼相端微出记档）。幅度工程
 *  设定 */
const SCAFFOLD_LENGTH_TAPER = [1.06, 0.74] as const;
/** 干向挂点高度角度梯度（开展圆头第二驱动——低枝 +12…+14° 平展（冠基放张——「枝广展」
 *  [1][7] 低枝外段可达 60–80° 近平展）× 高枝 −6…−8° 收角（圆头闭合——弱于银杏锥顶
 *  −8…−10°：圆头 vs 锥顶的分化）。确定性零 rng（作用于 profile 角域两端，槽间角域
 *  差异保持）。幅度工程设定（探针回调见上） */
const SCAFFOLD_ANGLE_TAPER = { minLow: 12, minHigh: -6, maxLow: 14, maxHigh: -8 } as const;

// ── 绿闭蒴果序常量（Spec 判定做——§4 果序姿态与账目 Verified [1][3][5][7] + 终审
//    ③-5；确定性挂点账目法：区域判据 + 位置散列排序 + 固定步长抽选，零 rng 消费——
//    挂点口径槽间恒等（结构常数）──

/** 果带判据（枝顶/冠缘外段）：**保留 L5 簇位（末级枝顶——顶生花序语义 [1][3]）∧
 *  q ≥ 0.40 冠外滤**（「散挂枝顶/冠缘外段」的径向读位；探针定档：q 0.50 带回果数
 *  偏稀、0.30 带内层果过多主导冠面——0.40 中带配合 STRIDE 14 实测 60–84 果/树，
 *  「显著性低-中」中距可辨不主导的量级定档） */
const FRUIT_ZONE_Q_MIN = 0.4;
/** 果序抽选步长（散列序每 14 取 1——顶生果序「显著性低-中」的工程定档：实测
 *  19–28 轴/树 × 2–4 果 ≈ 60–84 果（悬铃木 84–131 球量级下调——绿闭果小（径
 *  ≈2.4–3.0cm）显著性低于悬铃木 5–6.4cm 果球，同可见性下数量下调；无 rng roll——
 *  组 0 带不随 roll 浮动（platanus 果序 rng roll 组 0 带状浮动先例的规避记档，栾树
 *  先例方法） */
const FRUIT_STRIDE = 14;
/** 蒴果参数（platanus 果球方法复制 + 弯垂轴散布挂点）：真蒴果梨状球形径 1–1.5cm
 *  （FRPS/FOC Verified [1][3]）× 2 工程映射 → 径 2.4–3.0cm ⇒ 半径 0.012–0.015m
 *  （中距 15–30m 冠缘小绿果剪影可读、近景沿轴散布可辨）；轴 = 弯垂外向弧链 2–4 果
 *  （n = 2 + ⌊散列×3⌋ ∈ {2,3,4}——「每轴 2–4 果」主代理判定）× 步距 0.09–0.13m
 *  （「步距沿轴散布非密串」——果序轴至 28cm [3] 的散布读向；轴长 ≈0.18–0.39m） */
const CAPSULE_R_MIN = 0.012;
const CAPSULE_R_SPAN = 0.003;
const CAPSULE_COUNT_BASE = 2;
const CAPSULE_COUNT_SPAN = 3;
const CAPSULE_SPACING_MIN = 0.09;
const CAPSULE_SPACING_SPAN = 0.04;
/** 果域 uv 常量：v ∈ [4.0, 4.97]（果域 v∈[4,5] 冻结口径——platanus 终版；面基 4.0 /
 *  面尖 4.97 < 5）；u = 逐果熟度随机（位置散列，果内 24 顶点同值——材质绿果变奏
 *  通道：绿 → 转黑前夜的熟度梯度 [1][5]，冻结接口） */
...
interface LodEmissionPlan {
  ...
  /** Low 壳卡模式：不烘焙簇内叶卡与果序（候选/过滤/果序决策照常），逐保留簇发射交叉壳卡 */
  shellCards: boolean;
}

/**
 * 档位 → 发射计划（结构依据见模块头 LOD 段；预算记账 = 8 槽实测口径，正式预算带
 * 锁定见 ../assets/asset_tree_triadica.asset 模块头）：
 ...
 * - Mid 叶 ≈ High 存活卡 × 掩码率（中卡簇 8 选 3 = 37.5%）+ 果序全量；8 槽实测落
 *   6–10K 带。
...
function lodPlanFor(profile: BroadleafShapeProfile, level: ProceduralLevel): LodEmissionPlan {
  if (level === 'high') {
    ...
    // 24178 / rng 消费快照（triadicaLod 测试锁）延续
    return {
  ...
  if (level === 'mid') {
    ...
    // Mid 总面落 6–10K 预算带的档率）+ 果序全量保留（身份信号 [7]）
    return {
      radial: [6, 5, 4, 3, 3, 3],
...
/** Low 壳卡半幅余量（米）：High 散簇中卡自簇心的最大伸出 ≈ 簇半径 + 卡长（≤0.18m
 *  斜向——中卡 0.08–0.14 宽 × 长宽比 ≤1.26（叶形相轴阔卵端 slot-2 上限）⇒ 卡长
 *  ≈0.08–0.18m），典型伸出（r̂≤1 壳位 + 互生平摊）中位 ≈ 半卡长 0.04–0.09m——取
 *  0.10（小卡档：悬铃木 0.16 大卡 / 栾树 0.58 复叶大卡的缩放档）使壳卡吞并簇半径的
 *  常态视觉域，且极值叶尖仍由 High 决定冠包络（Low 不涨出，档间 bbox 一致性测试
 *  容差依据：实测跨档 XZ 跨度/总高差 ≤ 0.5m） */
...
// ── 绿闭蒴果发射（八面体最小面数——8 面/果 flat 法线；果域 uv v∈[4.0,4.97]）──
...
/**
 * 单蒴果发射：八面体 8 面 × 3 顶点（24 顶点/果）；uv 三角 (u,4.0)(u,4.0)(u,4.97)——
 * **果域 v∈[4,5]**（几何身份标记：与叶卡 0–1 四边形域（组 1）及皮管弧长域（v = 累计
 * 弧长 × 0.5，9.5m 级最长枝弧实测 v ≤ 2.31）双重隔离）。**u = 逐果熟度随机
 * ∈ [0,1)**（位置散列、果内 24 顶点同值——材质绿果变奏通道：绿 → 转黑前夜熟度梯度
 * [1][5]，冻结接口）；**账目入皮组（组 0）**——triadicaMaterials（park-shader-agent
 * 并行交付、签名冻结）按 v 域分流绿闭果配方；aLeafRand/aBend 随皮组恒 0（果序刚性
 * 悬垂——弯垂轴摆动语义归材质层）。
 */
function emitCapsule(
  ...
  for (const face of OCTA_FACES) {
    ...
    const uvs: [number, number][] = [
      [maturityRoll, FRUIT_UV_V_BASE],
      [maturityRoll, FRUIT_UV_V_BASE],
      [maturityRoll, FRUIT_UV_V_TIP],
    ];
    const idxs = [ia!, ib!, ic!];
    for (let k = 0; k < 3; k++) {
...
/** 位置确定性散列（salt 分通道——果熟度/抽选排序共用，零 rng） */
...
//    全在 shapeProfile.barkRelief 三字段（乌桕幅度/谐波/游走率见 triadicaShapeProfile）──
...
/** 脊深呼吸（权重沿轴调制）频率（rad/m）/幅度——起伏深浅沿干交错（乌桕「暗灰窄纵裂」
 *  的脊沟深浅交错 [1][7]——窄纵裂的长程连续读向由低 drift 2.8 承载，呼吸调制脊沟
 *  深浅缓变；幅度工程设定） */
const BARK_RELIEF_BREATHE_FREQ = 2.4;
const BARK_RELIEF_BREATHE_AMP = 0.3;
/** 亚视觉幅度地板（米，峰值）：管起径 × amplitudeRatio < 此值 → 该管整体跳过起伏
 *  （近景不可辨的亚毫米层，兼免低径向段细枝上高次谐波混叠成无意义计算——乌桕
 *  amplitudeRatio 0.027 下起径 < 0.056m 的管（L2 以下细枝典型域）平滑发射，主干 +
 *  L1 骨架（起径 0.09–0.11）有效起伏（窄纵裂显于粗干 [1][5]）、细枝光滑的读向自然
 *  涌现） */
...
interface ReliefHarmonic {
  ...
  /** 相位原点 */
  psi: number;
  /** 轴向游走率（±drift 域内确定性取值——乌桕 drift 2.8：窄纵裂长程连续（轴向去相关
   *  ≈2π/2.8 ≈ 2.24m——纵裂脊沿轴连续伸展读向 [1][7]）；银杏 ±2.6 纵脊连续同族 /
   *  栾树 ±5 局部细裂 / 悬铃木-榉 ±11 断裂拼贴。速率值 = profile.barkRelief.drift） */
  driftRate: number;
  /** 游走正弦相位 */
  chi: number;
  /** 呼吸正弦相位 */
 ...
 * 半径；d_θ/d_s 为解析导数（法线修正源）。φₘ(s) = ψₘ + driftₘ·s + wander 正弦项（乌桕
 * driftₘ ±2.8 rad/m——窄纵裂长程连续）、wₘ(s) = baseWₘ·(1 + breathe 正弦项)（脊沟
 * 深浅呼吸）——均弧长 s 的缓变函数。amplitudeRatio ≤ 0（亚视觉地板跳过路径）时
...
function emitTube(
  ...
  // 亚视觉地板：起径 × 幅度比 < 1.5mm 的管（乌桕 0.027 幅度下 L2 以下细枝典型域）
 ...
 * L5×2 末梢）；枝角/长度/粗度/曲率 rng 驱动（骨架枝横展 50–68° 对铅垂广展 + 干向挂点
 * 高度分级（长度 ×1.06→×0.74 / 角度 +12…14°→−6…−8° 低枝平展高枝收角）——开展圆头
 * 冠的结构成因，Spec §3 + FRPS「枝广展」Verified [1]；领导枝 0.48 中庸偏弱续顶——
 * 开展圆头顶共构）。互生单叶中卡簇挂点：末两级枝梢散簇（中簇 + 8 候选中卡黄金角
 * 互生螺旋）+ 保留 L5 簇位果序确定性抽选（弯垂轴绿闭果，见模块头）。主次分级与冠内
 * 通透规则见模块头。
 * profile 缺省 = slot-0 标准组合（锚点回落——单测直调便捷路径，资产路径显式传槽
 * profile）。level 缺省 = 'high'（三档同流派生——档位只改发射密度，不改骨架决策/
 * rng 消费序，见 lodPlanFor）。
 */
export function buildTriadicaGeometry(
  rng: () => number,
  profile: BroadleafShapeProfile = TRIADICA_SLOT0_PROFILE,
  level: ProceduralLevel = 'high',
): TriadicaGeometryResult {
  ...
  const totalHeight = 9.2 + rng() * 0.6; // 形态参数域 9.2–9.8m（≈9.5m 主代理裁定——终审 9–10m 带内中值；实测涌现带见 asset 模块头——9.5m 级领导链复利外伸 ≈×1.8 的种子实现差异，各槽领导比回调研 profile 注释）
  const crownRadius = (totalHeight * profile.crownWidthRatio * 0.5) * (0.94 + rng() * 0.12); // 冠幅半径 = 冠幅比驱动（含 ±6% 抖动）——工程域 0.8–1.0 涌现（探针定档见 profile）
  const trunkBaseR = 0.26 + rng() * 0.06; // 根径 0.26–0.32m（DBH 照片 20–40cm 无标尺推断 [7]——工程设定，速生比例瘦干读向）
  const trunkH = totalHeight * (0.17 + rng() * 0.09); // 净干 17–26%（Spec 域扩展节 A 0.15–0.25 单源 Inferred [7] + 主代理带内推 0.15–0.30 的工程域中带）
  const lean = randUnit(rng).multiplyScalar(jitter(rng, 0.05)); // 主干倾轴（≤2.9°——单干主导；多干/萌生变体记档不建模）
  ...
  // ── L1 骨架枝 ×6（横展 50–68° 对铅垂广展——FRPS「枝广展」Verified [1] + 照片
  //    45–90° Inferred [7]，七实例最开展起角带；方位 60° 均分 + asymmetry 0.46 散布
  //    抖动 = 螺旋互生散布（骨架排列 Unknown [7]）；挂高 0.50–0.70 干高段 span 0.20
  //    （净干 0.17–0.26 低分枝方向）；rank 乘子承担环内势差 + 干向挂点高度分级
  //    SCAFFOLD_LENGTH_TAPER ×1.06→×0.74 + 角度梯度低枝 +12…14° 平展（冠基放张——
  //    低枝外段可达 60–80° 近平展）/ 高枝 −6…−8° 圆头闭合）──
  ...
  for (let i = 0; i < scaffoldCount; i++) {
    ...
    // （+12…+14°——冠基放张）/ 高端收角（−6…−8°——圆头闭合），确定性随 attachNorm 插值
    const tiltMin =
  ...
  // ── L1 领导枝 ×1（apical_dominance 中庸-弱 Inferred [7]——开展圆头顶由骨架链共构、
  //    领导不翻转树顶；leaderLengthRatio 表达强度，直立系数 0.92 为家族方法常量）──
  ...
  // ── 果序挂点确定性抽选（零 rng——消费顺序契约不含果序；簇位表与档位无关 ⇒ 决策
  //    逐位同源；platanus 果序 rng roll 组 0 带状浮动先例的规避记档——栾树先例方法）：
  //    资格 = 保留 L5 簇位（末级枝顶——顶生花序/果序语义 [1][3]）∧ q ≥ FRUIT_ZONE_Q_MIN
  //    （「枝顶/冠缘外段」径向滤）；散列排序 + 固定步长抽选（每 FRUIT_STRIDE 取 1）→
  //    每位点 1 条弯垂外向弧轴（外向随 k 衰减 + 下垂随 k 增强——果期轴弯垂 [7]）×
  //    2–4 果沿轴散布（步距 0.09–0.13 非密串）──
  ...
  for (const c of l5Kept) {
    if (qOf(c.center) >= FRUIT_ZONE_Q_MIN) {
      fruitCands.push({ h: posHash(c.center.x, c.center.y, c.center.z, 1.7), c });
    }
  }
  fruitCands.sort((a, b) => a.h - b.h);
  for (let i = 0; i < fruitCands.length; i += FRUIT_STRIDE) {
    const { c } = fruitCands[i]!;
    // 果轴原料：冠缘径向外向（树轴水平径向——近干簇的确定性回退）+ 弯垂弧链
    const outward = new THREE.Vector3(c.center.x, 0, c.center.z);
    if (outward.lengthSq() < 1e-6) outward.set(1, 0, 0);
    outward.normalize();
    const count = CAPSULE_COUNT_BASE + Math.floor(posHash(c.center.x, c.center.y, c.center.z, 9.1) * CAPSULE_COUNT_SPAN);
    const spacing = CAPSULE_SPACING_MIN + CAPSULE_SPACING_SPAN * posHash(c.center.x, c.center.y, c.center.z, 11.3);
    let fp = c.center
      .clone()
      .addScaledVector(outward, c.radius * 0.7)
      .addScaledVector(UP, -0.03);
    const capsules: FruitCapsule[] = [];
    for (let k = 0; k < count; k++) {
      if (k > 0) {
        // 弯垂弧链方向：外向随 k 衰减 + 下垂随 k 增强（果期轴弯垂 [7]——「拱弯-下垂
        // 果序轴、绿闭蒴果沿轴排布」果-c 读向）
        const step = outward
          .clone()
          .multiplyScalar(0.58 - 0.09 * k)
          .add(UP.clone().multiplyScalar(-(0.55 + 0.12 * k)))
          .normalize();
        fp = fp.clone().addScaledVector(step, spacing);
      }
      capsules.push({
        center: fp.clone(),
        radius: CAPSULE_R_MIN + CAPSULE_R_SPAN * posHash(fp.x, fp.y, fp.z, 17.9),
        maturityRoll: posHash(fp.x, fp.y, fp.z, 19.3),
      });
    }
    ctx.fruit.push({ capsules });
  }

  // ── 烘焙名单：High = 全存活卡；Mid = 簇内候选序掩码子集（j % every === phase——
  //    中卡簇 8 候选 3 张；通透过滤对全候选照常执行 → Mid 存活卡 ⊂ High 存活卡逐位
  //    同位）；Low 壳卡模式不烘焙簇内叶卡与果序（候选生成/通透过滤/果序决策已照常
  //    走完——果序零 rng、决策表档间同源，消费序列档间恒等）──
  ...
  const bakedLeafCount = lod.shellCards
    ...
    : bakedCards.length;
  /** 烘焙蒴果数（High/Mid 全量保留——身份信号；Low 省略记档） */
  const bakedFruitBalls = lod.shellCards ? 0 : ctx.fruit.reduce((s, f) => s + f.capsules.length, 0);
  ...
  // ── 果序烘焙（High/Mid 全量：逐果八面体 8 面——**入皮组（组 0）**：uv v∈[4,5]
  //    果域 + u = 逐果熟度身份标记 + aLeafRand/aBend 随皮组恒 0（材质侧冻结接口——
  //    按 v 域分流绿闭果配方，见 emitCapsule 注释）；「绿闭蒴果沿弯垂轴散挂枝顶/
  //    冠缘外段」Spec 判定做的几何侧表达；Low 省略记档 ──
  if (!lod.shellCards) {
    for (const site of ctx.fruit) {
      for (const capsule of site.capsules) {
        emitCapsule(bark, capsule.center, capsule.radius, capsule.maturityRoll);
      }
    }
  }
  ...
  leafGeo.dispose();
  if (!geometry) throw new Error('程序化资产 asset_tree_triadica 层合并不兼容（属性集应一致：position/normal/uv/aLeafRand/aBend）');

  const groups = geometry.groups;
  /** 组 0 = 皮拓扑 + 果序（果序入皮组——材质接口约定，见 emitCapsule 注释）；
...
/**
 * 分枝递归：level 0–4（L1–L5）；路径 = 起点方向 + 每步游走 + 上举偏置（乌桕 upturn
 * [0.20,0.17,0.14,0.11,0.08] 单调低正链——**大枝广展平展 + 末级细枝上举 20–30° +
 * 冠缘微垂读向**（FRPS「枝广展」Verified [1] + bark-c 判读 [7] + 终审裁决 2：末级
 * 低值 + 冠缘平展承载微垂，不建负链下垂语言——区别栾树 L5 −0.02））；起径 = 父径 ×
 * profile.radiusRatio[level]、末径 = 起径 × profile.endRatio[level]；子枝挂点内埋父径
 * 内（起点回退 2.5×子径，杜绝接缝黑洞——树皮微起伏幅度 ≈ 2.7% 局部半径 ≪ 内埋余量，
 * 接缝安全不变）；末两级 = **互生中卡挂点**（散簇挂枝梢簇位——簇位表同时为果序承载
 * 位表）；通透过滤在收冠后统一执行，内层稀疏由密度场接管。
 * LOD：站点序列全分辨率计算（游走 rng 全消费——档间逐位同源），管发射按 lod 计划抽稀
 * （径向降段 + 站点隔 1 抽 1 + 未发射级跳过 emitTube）；簇决策/簇内候选生成/果序抽选
 * 与档位无关（果序零 rng、簇位表驱动——发射省略不省略消费）。
 */
function growBranch(
  ctx: BuildCtx,
  rng: () => number,
  ...
  for (let i = 0; i <= spec.segs; i++) {
    ...
    if (i < spec.segs) {
      ...
      // 上举偏置随 t 增强（横枝末段姿态——乌桕末级低值 0.08 + 游走 = 冠缘平展/微垂
      // 读向；开展圆头主剪影由骨架侧驱动链承载：干向分级 + 广角 + 散布方位）
  ...
  // ── 互生中卡挂点（末两级 L4/L5）──
  // 散簇（家族「枝梢驱动叶簇」的乌桕语义）：枝梢 → 簇空间（⊥ 枝切向平面）→ 互生
  // 螺旋散排中卡——L4 外段 1 簇 / L5 沿途外段 + 枝端共 2 簇（单叶互生散生于枝 [1][3][7]
  // ——簇 = 末级枝叶组的卡布置球）；簇间间隙 = 「簇只挂枝梢离散位 + 间距抑制」（拓扑
  // 相关处的父子/兄弟共位由簇级距离抑制保险，见下）；L5 簇位表同时为果序承载位表
  // （冠外域判据 + 散列抽选——主生成段，零 rng）
  ...
  if (clusterCount > 0) {
    ...
    for (let i = clusterCount - 1; i >= 0; i--) {
      ...
      // 散簇平面基（⊥ 簇方向 = 挂点枝切向——「叶对枝轴近垂直、平展摊开」的簇平面）
      ...
      for (let j = 0; j < leavesPerCluster; j++) {
        // 互生螺旋方位：黄金角步进 + 抖动（「叶互生」[1][3][7] 的簇内方位语言——语言
        ...
        // 互生偏移方向：簇平面辐射 + 沿枝向前倾（FORWARD_TILT——长柄叶略前倾）+ 弱抖动
        const offsetDir = u1
          .clone()
        ...
        // 叶尖方向：辐射外向 + 平展上举（长柄叶多向摊开、层叠轻盈 [5][7]——PLANE_LIFT）
  ...
  if (level < profile.levels.length - 1) {
    ...
    for (let i = 0; i < plan.ts.length; i++) {
      ...
      if (isTip) {
        ...
        // branch_angle 度数 Unknown（「大枝广展 + 末级上举」定性 Verified/Inferred
        // [1][7]，读向由 upturn 链承载），家族方法常量承载）
