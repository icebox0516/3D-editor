/**
 * runtime/procedural/tree/platanus/platanusStage —— window.__platanus DEV 出图面（T011.5；
 * 银杏 ginkgoStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/T011.1/T011.2/T011.3/
 * T011.4 能力面一次交付；方法零新面，数值面换悬铃木实数）。
 *
 ...
 *      celtis/camphor/zelkova/ginkgo Stage 同款 = docs/procedural-assets/shadow-visual-sop.md §3：
 ...
 *      相位摆动，D19.7；悬铃木叶/皮材质同款消费实例 aSeed——整树缓摆同公式同相位）；
 *      mesh 挂 customDepthMaterial（叶影 SDF 裁切——build 产物自带
 ...
 *      SOP §3 P42 口径；展开槽冠幅最大 ≈8.79m 防交叠）挂 slots 组直挂 scene 兄弟层；
 *      viewSlots 全景 / viewSlot(i) 单槽特写固定机位（球坐标同 view 语义——「批量
 *      出图 → 分轮裁定」的取证载体）；stats 扩逐槽账目（**组 0 槽间不恒等**——皮拓扑
 *      24178 恒等但宿存果序并入皮组随槽 672–1048（84–131 球 ×8），组 0 带
 *      24850–25226；叶卡数随槽形态向量各异——**本模块不感知槽内容**，morphSeed → slot →
 ...
 *      createPlatanusLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。
 *      deps.build 注入位 = 测试 seam（缺省悬铃木 asset build，产品路径不变——档位透传
 *      断言不依赖真实 level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__platanus，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor/zelkova/ginkgo Stage 同为纯工厂，window 装配归
 *      组合根）；资源所有权归本句柄——unmount/dispose 摘自己的 group 并 dispose source
 *      资源（geometry/material/customDepthMaterial——深度材质随 source 释放）与自建回退
 ...
 * 树高参考（机位 target y 基准——与 ginkgo 的差异点）：悬铃木 slot-0 锚 ≈11.63m 高
 *      （涌现 bbox 实测，T011.5 探针——≈12m 中龄公园个体工程锚 = 主代理裁定保守低端，
 *      12–14m 弱 Inferred per 终审记档；六实例首个 Inferred 支撑干高域），视觉冠底/实高
 *      0.296（platanusShapeProfile 挂高段探针回推——六实例首个实测落 Inferred 域
 *      0.25–0.35 中带）→ 冠域 3.44–11.63 → 视心取**冠心 ≈7.5**（(3.44+11.63)/2 ≈ 7.54），
 *      vs ginkgo 5.4（银杏冠域 2.68–8.17 冠心 5.42）/ zelkova 5.3 / camphor 5.5 /
 *      celtis 4.3——悬铃木为同语境混植的上层大乔（≈1.5× 五先例树高量级，Spec §2），
 *      冠心较五先例上移 ≈2m 为族内最高；stats 锚点实数 = slot-0 High 组 0 皮 25098
 *      （皮拓扑恒 24178 + 宿存果序 115 球 ×8 = 920 并入皮组——果序账目入皮组，
 *      platanusGeometry/platanusMaterials 冻结接口）/ 叶 3368 / 1684 卡（资产预算锁定
 *      账目：triangleCount 28466 = 25098 + 3368；叶卡 = 疏簇叶卡双三角，果序球面非卡
 *      不入 leafCards——本模块不感知组内果序/皮分离，归资产侧测试）。
 * 011.3 缺口 D 评估结论（T011.5 Step 3 记档，归 011.13）：悬铃木树皮 = **满干型**——
 *      材质侧已判（platanusMaterials 三色带拼贴「剥落型全干拼贴」整干有效、无「未剥落
 *      基底」概念，幼树基段先起斑不建模），本步复核一致：唯一高度项 smoothstep(3.0, 5.5)
 *      为干上部/细枝红褐收敛的**连续过渡**（斑径 < 枝径不可辨 + FRPS「老枝秃净，红褐色」
 *      ——非榉树「满斑区 <2.2m」式限带特征）+ 几何浅起伏主干整段有效（0.014 起径门控
 *      <0.11m 的管平滑——起径门控非高度门控）→ 冠心取景机位可拍全干斑块拼贴身份，
 *      **Stage 低目标机位能力本轮不补**（缺口 D 维持榉树单例，归族门 011.13 收口）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_platanus.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createPlatanusLeafDepthMaterial } from './platanusMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface PlatanusStageDeps {
...
export interface PlatanusLevelStats {
...
/** window.__platanus 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface PlatanusHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'platanus-dev-slots' 组直挂
   *  scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；展开槽冠幅最大
   *  ≈8.79m 防交叠）；每 Mesh castShadow + customDepthMaterial（与 mount 单树同待遇）；
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：同槽冠幅 XZ 最大 ≈8.79m 防交叠）；每档
   ...
   *  自建 createPlatanusLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
  ...
  /** 8 槽全景固定机位：目标 = 网格中心（组位 + (0, 7.5, spacing/2)）；缺省 distance 42
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+8.79 ≈ 41.8m、
   *  纵深 ≈20m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨且留边（垂直半角需求 ≈11° <
   *  典型 fov 25° 余量；P42 为全族固定口径，沿 tree3a/celtis/camphor/zelkova/ginkgo
   *  ——42m 全景机位与网格几何绑定，不随冠幅缩距；celtis 横跨 ≈42.9m 先例同沿 42） */
  viewSlots(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单槽特写机位：球坐标绕该槽树位（复用 view 公式，目标 = 该槽 x/z、视心高 ≈7.5）；
  ...
  /** 三树全景固定机位：目标 = 排中心（组位，三树 −s/0/+s 几何中心即组位）视心高 ≈7.5；
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 8.79
   *  ≈ 30.8m（同槽冠幅 XZ 最大 ≈8.79m，T011.5 探针实测带 widthRange max），32m 距离
   *  35° 斜视 16° 俯角下三棵全入画可辨且留边（对齐 viewSlots「distance ≈ 实宽」换算
   *  口径上取留边——沿 zelkova 30.5→32 / celtis 32 同族定档法；垂直半角需求 ≈7°
   *  同口径余量内） */
  viewLevels(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单档特写机位（viewSlot 的档位版）：球坐标绕该档树位（复用 view 公式，目标 = 该档
   *  x/z、视心高 ≈7.5）；缺省 distance 25 / azimuth 35 / elevation 8（同 view/viewSlot）；
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；展开槽冠幅最大 ≈8.79m（widthRange max
 *  8.79，T011.5 探针实测带），11m 留 ≈2.2m 防交叠（P42 全族固定口径沿 tree3a/celtis/
 *  camphor/zelkova/ginkgo——celtis 9.9m 冠幅同距 ≈1m 先例；42m 全景机位与网格几何
 *  绑定，不随冠幅缩距） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（同槽冠幅 XZ 最大 ≈8.79m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 8.79 ≈ 30.8m，「distance ≈ 实宽」口径
 *  上取留边（沿 zelkova 30.5→32 / celtis 32 同族定档法，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 3.44–11.63 → ≈7.54 取 7.5；vs ginkgo 5.4 /
 *  camphor 5.5 / zelkova 5.3 / celtis 4.3，族内最高——上层大乔量级，见模块头「树高参考」） */
const VIEW_TARGET_Y = 7.5;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createPlatanusHandle(deps: PlatanusStageDeps): PlatanusHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省悬铃木 asset build 行为不变） */
  ...
  /** 球坐标取景落位（view/viewSlots/viewSlot/viewLevels/viewLevel 共用语义，与 tree3a/celtis/
   *  camphor/zelkova/ginkgo Stage 度→弧度换算同款）：绕 (tx, targetY, tz) 以方位/仰角定距放相机 */
  const placeCamera = (
    tx: number,
    tz: number,
    targetY: number,
  ...
  return {
    mount(opts = {}) {
      ...
      if (sourceDepth) {
        ...
        leafDepth = createPlatanusLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createPlatanusLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'platanus-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'platanus-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createPlatanusLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[platanusStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'platanus-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createPlatanusLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（11.63m 锚点树冠域 3.44–11.63 → ~7.5m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[platanusStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[platanusStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: PlatanusLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: PlatanusSlotStats[] = slotsSources.map((tree, slot) => {
