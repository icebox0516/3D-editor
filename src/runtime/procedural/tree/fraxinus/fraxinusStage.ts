/**
 * runtime/procedural/tree/fraxinus/fraxinusStage —— window.__fraxinus DEV
 * 出图面（T011.10；sophoraStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/
 * T011.1–T011.9 能力面一次交付；方法零新面，数值面换白蜡实数）。
 *
 ...
 *      sophora Stage 同款 = docs/procedural-assets/shadow-visual-sop.md §3：主方位
 *      az35 / 逆光 az215–275 / 侧方位 az110–125，缺省 25m/35°/8° = M25 机位）。
 *      mountWindDemo(count) 风动验收载体——build() 一次锚点源 → InstancedMesh ×N
 *      间距 8m 一排，桶几何挂 aSeed InstancedBufferAttribute（值各异 → 同槽树不同
 *      相位摆动，D19.7；白蜡叶/皮材质同款消费实例 aSeed——整树缓摆同公式同相位 +
 *      aBend 复叶卡快颤双层（一回羽叶卡长轴飘逸感）；皮/果组 aBend 恒 0 只随整树
 ...
 *      网格（间距缺省 11m——SOP §3 P42 口径；8 槽冠幅实测带最大 ≈9.79m 与邻槽窄端
 *      互补防交叠）挂 slots 组直挂 scene 兄弟层；viewSlots 全景 / viewSlot(i) 单槽
 *      特写固定机位（球坐标同 view 语义——「批量出图 → 分轮裁定」的取证载体）；
 *      stats 扩逐槽账目（**组 0 槽间平滑带**——皮拓扑恒 24178，匙形翅果帘幕簇并入
 *      皮组随槽 2004–2712 tri 平滑带（triadica/sophora 果序并入同款；翅果挂点 =
 *      确定性账目零 rng——簇数随槽簇位平滑变化），复叶卡数随槽形态向量各异（8 槽
 *      实测带 2564–4394 卡——**本模块不感知槽内容**，morphSeed → slot →
 ...
 *      createFraxinusLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。deps.build 注入位 =
 *      测试 seam（缺省白蜡 asset build，产品路径不变——档位透传断言不依赖真实
 *      level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__fraxinus，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor/zelkova/ginkgo/platanus/
 *      koelreuteria/triadica/bischofia/sophora Stage 同为纯工厂，window 装配归组合根）；
 ...
 * 树高参考（机位 target y 基准——与 sophora 的差异点）：白蜡 slot-0 锚 ≈10.50m 高
 *      （涌现 bbox 实测 10.4973，T011.10 探针——≈10m 长江流域公园中龄个体锚 = 志书
 *      「高10-12米」直给 per 终审裁决 2（卵圆开展冠不取高端；8 槽带 8.57–10.50、
 *      slot-0 锚点即全槽最高端），w/h 0.763 落 Spec 域 0.7–1.0 下中段），视觉冠底/
 *      实高 **0.207**（叶卡最低 Y ≈2.17m，T009.3 口径——fraxinusShapeSlots 同款
 *      测量：中位分歧挂高段 0.58 下缘 + 单段角域直读（无国槐两段带低位放射带）→
 *      冠底上抬一档）→ 冠域 2.17–10.50 → 视心取**冠心 ≈6.3**
 *      （(2.17+10.50)/2 ≈ 6.334；vs platanus 7.5（12m 级上层大乔）/ sophora 5.9 /
 *      koelreuteria 5.9 / triadica 5.8 / bischofia 5.8 / camphor 5.5 / ginkgo 5.4 /
 *      zelkova 5.3 / celtis 4.3——白蜡 ≈10.5m 级与 ≈10m 队列同档高半档，**冠心
 *      上探至 6.3 的成因是中位分歧的冠底上抬（2.17 vs 国槐 1.45）非树高差**；Stage
 *      实际渲染 M25 观感复核（卵圆-圆头冠 + 干基裸段更长 + 满冠翅果帘幕，6.3 视心
 *      上下留边均衡）；stats 锚点实数 = slot-0 High 组 0 皮+翅果 26422（皮拓扑恒
 *      24178 + 匙形翅果 561 果 ×4 = 2244 并入皮组——果域 uv v∈[4,5] 冻结接口，
 *      triadica/sophora 先例）/ 复叶卡 7498 tri / 3749 卡（资产预算锁定账目：
 *      triangleCount 33920 = 26422 + 7498；复叶卡 = 2 tri/卡承载**整枚一回奇数
 *      羽状复叶**——vs 国槐同为一回羽叶卡 2 tri、乌桕中卡承载单枚菱形叶，卡语义
 *      不同、tri/卡同 2；翅果双 quad 4 tri/果非卡面不入 leafCards——本模块不感知
 *      组内果/皮分离，归资产侧测试）。
 * 011.3 缺口 D 评估结论（T011.10 DEV Step 记档，归 011.13）：白蜡树皮 = **灰褐
 *      浅-中纵裂（浅细脊沟、无剥落无碎翘）+ 幼干-大枝近光滑 + 皮孔小不明显**（第
 *      11 树皮语言，终审裁决 6——FRPS「树皮灰褐色，纵裂」+ 照片多源）——材质侧
 *      8 细脊/周浅宽坡剖面 + 沟深 0.68 浅档 + 沟内弱 AO + 干基暗化弱档 + 细枝黄褐
 *      两档（「小枝黄褐色」FRPS 原句）；上部弱化门控 smoothstep(3.2,5.6)（幼干-
 *      大枝近光滑）为**连续过渡**（沟深连续弱化至近光滑——3.2/5.6 为材质侧待同步
 *      的占位锚〔材质注释自记缺口候选⑤，冠基 2.8m 同批〕，实测冠底 2.17/顶 10.50，
 *      同步编辑归 park-shader-agent，判定只依赖连续 vs 限带的结构性事实、不依赖
 *      常数值）；皮孔场（High 专属近景身份层）= 高度门 smoothstep(2.6,3.8) × 中低
 *      弧长 v 门 (1−smoothstep(0.90,1.70)) 的**限带门控**，但该带（2.6–3.8m 干上
 *      段 + 皮管中低弧长）在冠心机位（视心 6.3、M25 8° 俯角 25m）画面下部覆盖内
 *      （与 sophora 同几何口径：画面含干基 0–3.4m 带，皮孔带上沿 3.8m 在画幅内）
 *      拍得到；细枝黄褐-灰褐两档 4.8–8.2m 为高位**连续过渡**（冠缘、机位天然覆盖）；
 *      果域色序/边缘暗线为 v∈[4,5] 域门控（末级枝梢冠带——机位天然覆盖）；几何
 *      浅浮雕 = 亚视觉地板抬档 3.5mm 的**起径门控**（起径×幅度比 < 0.0035 的管
 *      平滑发射——半径门控非高度门控，0.020 下**仅主干有效起伏、全部分枝管光滑**
 *      （L1 骨架起径 0.08–0.11 在地板下），主干 + 底盖满段有效起伏 → 冠心取景机位
 *      （视心 6.3）可拍全灰褐浅纵裂浅细脊沟身份（下半干脊沟最强 + 干基暗化 + 上
 *      半干近光滑过渡 + 幼干带皮孔小点），**Stage 低目标机位能力本轮不补**（缺口 D
 *      维持榉树单例，归族门 011.13 收口）。
 * Stage 观察项（T011.10 Step 4 裁定输入——本 Stage 只记档不修数值）：
 *      ①中距覆盖/通透感（中通透空隙 25–40% vs 国槐 10–20% 致密端——族门密度横向
 *      对照第二对样本的 M25 读向）；②翅果帘幕中距可辨性（50–67 簇满冠分布的 M25
 *      剪影读向——裁决 5「显著高于国槐冠缘散点」的画面验证）；③对生挂点近读
 *      （decussate 对生簇 vs 国槐黄金角互生的横向对照）；④冠底裸干带读向（视觉
 *      冠底比 0.207 中位分歧 vs 国槐低位放射）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_fraxinus.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createFraxinusLeafDepthMaterial } from './fraxinusMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface FraxinusStageDeps {
  ...
  /** 构建函数注入位（缺省 = 白蜡 asset build；测试注桩 fake build——透传断言不依赖真实
   *  level 路由落地时序；产品路径 bootstrap 不传，行为不变） */
  build?: ProceduralBuild;
}

/** mountSlots 模式逐槽账目（slot 序号 + 该槽组 0 皮+翅果/叶三角与叶卡数——组 0 含翅果簇） */
export interface FraxinusSlotStats {
  slot: number;
...
/** mountLevels 模式逐档账目（LOD 档位 + 该档组 0 皮+翅果/叶三角与叶卡数——档间可比的取证面） */
export interface FraxinusLevelStats {
...
/** window.__fraxinus 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface FraxinusHandle {
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：8 槽冠幅 XZ 最大 ≈9.79m 防交叠）；每档
   ...
   *  自建 createFraxinusLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
  ...
  /** 8 槽全景固定机位：目标 = 网格中心（组位 + (0, 6.3, spacing/2)）；缺省 distance 42
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+9.79 ≈
   *  42.8m、纵深 ≈20m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨（P42 为全族固定口径，
   *  沿 tree3a/celtis/camphor/zelkova/ginkgo/platanus/koelreuteria/triadica/
   *  bischofia/sophora——42m 全景机位与网格几何绑定，不随冠幅缩距；celtis 横跨
   *  ≈42.9 / 栾 42.89 / 乌桕 42.57 / 重阳木 42.64 / 国槐 44.01 先例同沿 42，白蜡
   *  42.79 冠幅同位续沿） */
  viewSlots(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单槽特写机位：球坐标绕该槽树位（复用 view 公式，目标 = 该槽 x/z、视心高 ≈6.3）；
  ...
  /** 三树全景固定机位：目标 = 排中心（组位，三树 −s/0/+s 几何中心即组位）视心高 ≈6.3；
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 9.79
   *  ≈ 31.8m（8 槽冠幅 XZ 实测带 max ≈9.79m = slot-2 开张宽端，T011.10 探针），32m
   *  距离 35° 斜视 16° 俯角下三棵全入画可辨且留边（对齐 viewSlots「distance ≈ 实宽」
   *  换算口径上取留边——沿 zelkova 30.5→32 / celtis 32 / platanus 32 / koelreuteria
   *  31.9→32 / triadica 31.6→32 / bischofia 31.64→32 / sophora 33.0→34 同族定档法；
   *  白蜡树高带 8.57–10.50 不超 platanus 需求，31.8 跨 → 32 同法定档；垂直半角需求
   *  ≈11° 同口径余量内） */
  viewLevels(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单档特写机位（viewSlot 的档位版）：球坐标绕该档树位（复用 view 公式，目标 = 该档
   *  x/z、视心高 ≈6.3）；缺省 distance 25 / azimuth 35 / elevation 8（同 view/viewSlot）；
  ...
  /** 账目：挂载态 + 面数（组 0 皮+翅果/叶三角 + 叶卡数——顶层恒「总量」语义，slots 模式
   *  = 8 棵合计、levels 模式 = 三档合计；组 0 皮含并入翅果簇） */
  stats(): {
    mounted: boolean;
    barkTriangles: number;
    leafTriangles: number;
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；8 槽冠幅实测带最大 ≈9.79m（T011.10
 *  探针实测 slot-2 开张宽端 w/h 1.037；meta widthRange max 9.9 为声明带），11m 网格下
 *  宽端与邻槽窄端互补（slot-2 邻槽 1/3 窄端 ≈7m 级，半幅和 ≈8.4 < 11 不交叠——P42
 *  全族固定口径沿 tree3a/celtis/camphor/zelkova/ginkgo/platanus/koelreuteria/
 *  triadica/bischofia/sophora；42m 全景机位与网格几何绑定，不随冠幅缩距） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（8 槽冠幅 XZ 最大 ≈9.79m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 9.79 ≈ 31.8m，「distance ≈ 实宽」
 *  口径上取留边（沿 zelkova 30.5→32 / celtis 32 / platanus 32 / koelreuteria 31.9→32 /
 *  triadica 31.6→32 / bischofia 31.64→32 / sophora 33.0→34 同族定档法，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 2.17–10.50 → ≈6.334 取 6.3；vs platanus 7.5 /
 *  sophora 5.9 / koelreuteria 5.9 / triadica 5.8 / bischofia 5.8 / camphor 5.5 /
 *  ginkgo 5.4 / zelkova 5.3 / celtis 4.3——≈10.5m 级与 ≈10m 队列同档高半档，冠心
 *  上探成因 = 中位分歧冠底上抬（2.17 vs 国槐 1.45），见模块头「树高参考」） */
const VIEW_TARGET_Y = 6.3;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createFraxinusHandle(deps: FraxinusStageDeps): FraxinusHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省白蜡 asset build 行为不变） */
   ...
   *  Stage 度→弧度换算同款）：绕 (tx, targetY, tz) 以方位/仰角定距放相机 */
  const placeCamera = (
    tx: number,
    tz: number,
    targetY: number,
  ...
  return {
    mount(opts = {}) {
      ...
      if (sourceDepth) {
        ...
        leafDepth = createFraxinusLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createFraxinusLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'fraxinus-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'fraxinus-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createFraxinusLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[fraxinusStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'fraxinus-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createFraxinusLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（10.50m 锚点树冠域 2.17–10.50 → ~6.3m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[fraxinusStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[fraxinusStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: FraxinusLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: FraxinusSlotStats[] = slotsSources.map((tree, slot) => {
