/**
 * runtime/procedural/tree/ligustrum/ligustrumGeometry —— 女贞（Ligustrum lucidum
 * W. T. Aiton f. lucidum 女贞原变型本尊——种定名三源裁定见 ligustrumShapeProfile 模块
 * 头，木犀科女贞属常绿乔木——长江流域城市公园/行道中龄单干个体 ≈8m，卵圆-广卵满密
 * 常绿冠 + 中低分枝 0.30 + 中角骨架）CPU 几何生成器（T011.11 Step 3a，阔叶家族第十二
 * 实例——方法复制自夏栎第一实例经朴树/香樟（**常绿密冠 + 广卵冠骨架法参数面先例**）/
 * 榉树/银杏/悬铃木/栾树/乌桕/重阳木/国槐/白蜡（**decussate 对生挂点机制 + 满冠带
 * 果簇账目 + 果簇卡入皮组 uv 果域——最直接模板**）十一次验证的通路
 * ../fraxinus/fraxinusGeometry，契约字段语义不变：五级递归分枝拓扑 / 锥度管状枝干 /
 * 枝梢驱动叶簇 / 冠内通透三规则 / 树皮近景微起伏 / LOD 三档同流派生全部沿用；
 * 女贞数值与算法细节差异点见下）。
 *
 * 职责：morphRng（mulberry32 流，消费顺序即契约——同 seed 逐位同结果）驱动的五级
 *      递归分枝拓扑 → 锥度管状枝干（平行传输标架）→ 枝梢驱动**单叶卡（decussate
 *      对生挂点——家族首例对生单叶挂点语言）**烘焙 + **肾形核果满冠下垂密簇**（果期
 *      7 月至翌年 5 月覆盖 9–10 月主语境——任务书裁决 1 做），产出树皮/叶两层非索引
 *      几何；层间 mergeGeometries(useGroups=true) 恰 2 组（D15 免组膨胀：皮 0 / 叶 1
 *      ——**核果卡入皮组（组 0）**，材质接口约定见下）。形态参数类型 = 阔叶家族契约
 *      ../broadleaf/broadleafShapeProfile（女贞为第十二实例，零修改实例化），数值与
 *      槽组合见 ./ligustrumShapeProfile（全部数值依据 docs/research/
 *      ligustrum-reference.md Spec 1.0——**生产一律以文末「主代理终审记档」修正后
 *      口径为准**：form-cn 整树主张否证、整树锚 = form-d 单样木）；build 只做 slot →
 *      profile 路由，不进 ProceduralBuild 公共签名。
 * 女贞算法细节差异点（vs fraxinus 模板，逐条 Spec 引用见 ligustrumShapeProfile 内联
 *      注释；主代理「待裁决位」10 条为执行依据）：
 *   - 尺度锚：城市公园/行道单干中龄个体 ≈8m（终审裁决 2 维持——form-d 单整树样木
 *     ≈8m 双系统一致 + NC 栽培域 4.6–15.2m + 志书 25m 上限口径，中档证据如实）——
 *     形态参数域树高 7.3–8.1m（**Step 3 探针回调定档**：初值 7.5–8.3 实测 slot-0
 *     涌现 8.82 偏 ≈8m 锚 +0.8——广卵上收链 + 中庸领导的纵向外泄，回调 −0.2 后
 *     终测带见 asset 模块头）；干高参数域 0.36–0.42（干高占比
 *     ≈0.30 form-d + bark-b 双证 Inferred [10] 的带内映射）；根径 0.22–0.28m
 *     （DBH Unknown——工程设定，8m 中龄家族带内偏细档）。
 *   - 干形：近直立单干（丛生多干相 form-b [10] 为变体端——**记档不建模**：结构差异
 *     不落连续参数，家族同款纪律）→ 主干倾轴幅度 ≤2.9°（jitter 0.05）；根部 flare
 *     轻度 1.18×（basal_flare Unknown——家族常量）。
 *   - 树形：**卵圆-广卵满密冠（camphor 广卵法端稍紧）+ 中低分枝 scaffold + 中角骨架
 *     + 领导枝中庸-弱**（Spec §3/§4/域扩展节 A：form-d broad-ovate 0.75–0.85/分枝
 *     0.30 双系统一致 + bark-b 分枝 0.3 侧证 + scaffold_angle 中角三源双问 [10] +
 *     apical_dominance「中庸-弱」Inferred [10]）——几何侧驱动链（T009.3 消费语义）：
 *     **单段角域直读**（scaffoldAngleMin/Max 36–58° 中角中带直接消费——无国槐两段
 *     角带、无白蜡修正史，槽间角域差异整体平移单段域）+ **挂高段中低**（0.56–0.74
 *     span 0.18——中低分枝：冠下压、干较矮读向）+ rank 乘子承担环内势差 + **upturn
 *     链中正广卵上收**（[0.24,0.20,0.14,0.08,0.04] 单调递减——沿樟 [0.28,…,0.05]
 *     端稍紧一档：外层末段上收使冠缘向上收口、顶部圆钝（卵圆读向上收快于樟广卵），
 *     同时抑制中角横展的冠幅外泄——探针校准依据）+ 领导枝中庸-弱 0.50 续顶（圆钝顶
 *     由上段骨架链共构）+ 散布方位（骨架排列 Unknown [10]）。
 *   - **单叶卡 decussate 对生挂点（本树核心分化位，家族首例对生单叶挂点语言——任务书
 *     裁决 5：沿 fraxinus 机制直接继承，单叶卡替代复叶卡）**：「叶对生，单叶……全缘」
 *     属级 Verified [3][4][7] 的几何落地——簇内单叶卡方位 = **对生对 + 相邻对交互
 *     90°**（φ_j = ⌊j/2⌋×90° + (j mod 2)×180° + 微抖动——vs 黄金角互生螺旋；
 *     与白蜡（对生复叶）共享「对生」位、叶层面为单叶）+ **对内共享簇内径向位 r̂**
 *     （同节对生语义：一对单叶自同一节位向相反方向展开；对首卡消费 rng、对次卡复用
 *     ——消费次数由 j 奇偶确定性决定，固定次数纪律不破）+ 对生抖动 ±0.20 rad 小档
 *     （leaf-a「opposite pairs + 顶芽单叶」/leaf-winter「数出 4–5 组对生叶对」双问
 *     [10]——对生规整读向排列可读优先）+ 平展取向（PLANE_LIFT 0.11）+ 略前倾
 *     （FORWARD_TILT 0.24）+ 弱抖动（WOBBLE 0.15）——家族常量沿用；每簇 10 候选 =
 *     5 个对生对（对生叶节位合并抽象——真叶沿枝对生节距数 cm 级 × 卡长 0.12–0.34
 *     ⇒ 连续节位合并，通透过滤后逐簇存活典型 6–10 枚）；大簇（0.13–0.18——**女贞
 *     团块略大于樟 0.11–0.15**，团块 1/6–1/4 冠宽读向 [10] 的簇参数承载）+ 中间距
 *     抑制（minSeparation 0.52×径和）；**卡尺寸 = 真叶 6–17 × 3–8cm（FRPS Verified
 *     [1][2]）× 2 家族工程映射 → 卡宽 0.06–0.16 / 长宽比 1.7–2.8 变比例域**（单叶系
 *     卡——真叶长宽比本身有域即如实映射，vs 白蜡复叶卡恒比例冻结；材质单叶 SDF 按
 *     此比例域设计包络——卵形-长卵形-椭圆-宽椭圆形/全缘平坦/先端锐尖-渐尖归
 *     ligustrumMaterials，并行交付）。
 *   - **肾形核果满冠下垂密簇（Spec 判定做——任务书裁决 1：属相内显著性（FRPS 裁定
 *     句以果形定种——肾形 = 种级身份）+ 生长季主语境全覆盖（果期 7 月至翌年 5 月
 *     [1][2] + 照片四点色序链 [10]）+ 家族独有性（肾形白粉浆果核果独立位））**：
 *     **果簇沿末级枝梢下垂挂、满冠分布**（fruit-b/c「满冠下垂密簇」[10]——资格门槛
 *     = **满冠带 q ≥ 0.30**（fraxinus 帘幕型同判据直接沿用——满冠外-中部带，vs 国槐
 *     0.42 冠缘散点））——**零 rng 确定性挂点账目法**（011.6/011.7/011.9/011.10
 *     先例：保留 L5 簇位冠带判据 + 位置散列排序 + 固定步长抽选（DRUPE_STRIDE）——
 *     账目恒等可断言，组 0 带不随 roll 浮动）；**真果肾形 7–10 × 4–6mm（FRPS
 *     Verified [1][2]）× 2 家族工程映射 → 果长 0.014–0.020m / 果宽 0.008–0.012m**；
 *     每簇 8–12 果卡（真实「每簇 20–80 果/序」[10] 的视觉密度档映射——**绝对数不
 *     承重**（终审记档 5 口径））；果序轴下垂 0.12–0.26m + 果沿轴散布（散布半径
 *     0.03–0.08m）、果长轴斜下外姿态（「熟果自枝端下垂悬挂」[10]——**熟果下垂读向
 *     由簇卡朝向弱表达**：果卡长轴自果序轴斜下外伸，果尖朝下）；几何 = **扁平 quad
 *     果卡正反双 quad = 4 tri/果卡**（fraxinus 翅果选型沿用——双面由几何侧承担
 *     〔材质组 0 FrontSide〕：冠下仰观（fruit-b/c 主视角）背面可见；正面绕序
 *     (p0,p1,p2)/(p0,p2,p3) + 法线 n、反面顶点序反转 + 法线 −n，同 uv 同账）；
 * *     uv 果域标记 v∈[4.0,4.97]（族冻结接口——triadica/sophora/fraxinus 三重一致
 *     口径：**果域在 v 轴**：果梗端 v=4.0 / 果尖 v=4.97（材质按 (v−4.0)/0.97 归一
 *     长轴分流核果配方——肾形轮廓的长轴坐标；面尖 4.97 < 5）+ **u = 逐果色档随机
 *     ∈ [0,1)**（位置散列，果内 12 顶点同值——材质果色档通道：绿-白绿未熟→紫黑蓝黑
 *     被白粉色序 [10]，冻结接口——fraxinus 果域模式（轴 + 色档双编码）同轴同位）；
 *     **2026-09-22 主代理终审裁定：派遣简报 u 轴系误写，对齐 triadica/sophora/
 *     fraxinus 族冻结口径 v 轴**（族内不存无设计依据的轴向变体——翻转仅 uv 写入轴
 *     互换，顶点位置/法线/顶点色/挂点账目零变化）；**入皮组（组 0）**——
 *     ligustrumMaterials（park-shader-agent 并行交付）按 v 域分流核果配方；
 *     aLeafRand/aBend 随皮组恒 0（果簇刚性悬垂——摆动语义归材质层/缺口候选）；
 *     数量级定档 **DRUPE_STRIDE 6（探针回调定档——初值 5 实测 Mid 越预算带上沿，
 *     见 DRUPE_STRIDE 常量注释）实测落带见 asset 模块头终测记档**（Mid 预算带
 *     （10000 − 皮 4514 − Mid 叶面）下 4 tri/果卡的簇数-果卡数平衡）；**Low 档核果
 *     省略**（triadica/sophora/koelreuteria/fraxinus Low 果省略先例——远距亚像素，
 *     档间连续记档）。
 *   - **uv 域身份标记双重隔离（家族探针断言纪律沿用——platanus 域碰撞先例教训）**：
 *     皮管 v = 累计弧长 × 0.5（8m 级最长枝弧实测 ≤ ≈1.7，slot-0 终测 max 1.609
 *     ——**皮管弧长域 ≤≈2.9 族纪律内，无需 fraxinus 式压缩**）｜核果 v∈[4.0, 4.97]
 *     ——**皮管弧长域不得侵入 [3,4) 隔离带与 [4,5] 果域**：测试探针断言「v∈[3,4)
 *     顶点数 = 0（隔离带空）+ 果域顶点 = 果卡 × 12（双 quad 4 tri × 3 顶点）+
 *     v∈[5,7) 顶点数 = 0（无花资产——任务书裁决 2）+ 皮组其余顶点 v < 2.5（皮管
 *     弧长域实测上界——8m 级最长枝弧 ×0.5）且 u ≤ 1（环向 θ/2π 与底盖 0.5+0.5cosθ
 *     域）」（ligustrumStructure 测试锁）。
 *   - 骨架：**6 骨架枝 + 1 领导枝**（scaffold_branch_count Unknown——任务书裁决 9
 *     【工程设定无现实基准】族典型域取值；拓扑 L1=7 → 枝数 [7,21,63,189,378]、簇位
 *     945（L4 189×1 + L5 378×2）、皮面 24236：主干 464（14 段 ×16 + 底盖 16——
 *     **径向 16 承载细窄脊谐波 {5,7} 的奈奎斯特需求**）+ L1 1008（7 枝 ×9 段 ×8）+
 *     L2 2352（21 ×8 段 ×7）+ L3 3780（63 ×5 段 ×6）+ L4 7560（189 ×4 段 ×5）+
 *     L5 9072（378 ×3 段 ×4）；大卡中数量 + 核果簇组 0 预算留带内余量）。
 *   - 密度语言：**常绿密档（沿香樟——任务书裁决 4）**——空隙 ≈5–15%（canopy-a
 *     5–10% / canopy-b 10–15% 双问 [10]；与樟 10–15% 同密档密端更深一档）：
 *     canopyDensity 0.95 + 厚壳 0.46 + 芯层地板 0.08（内膛不空透）+ 空腔 3 个收窄域
 *     0.38–0.66 + 大簇（0.13–0.18）+ shellBias 0.46（团块整块感——樟 0.66 壳聚与
 *     白蜡 0.42 平摊之间）；疏密差异归槽 6/7（0.78 ↔ 0.97 展开空隙 5–15% 带两端）。
 *   - 树皮近景微起伏：**灰褐细窄纵脊浅沟低浮雕（第 12 树皮语言，任务书裁决 7；「树皮
 *     灰褐色」FRPS Verified [1] + bark-a「细窄脊 + 浅沟、纵行大体平行（少量横向连
 *     接）、fine maple-like」+ bark-b「细鳞微翘、纵裂轻微」双问 [10]——建模取中龄
 *     细浅端；沟深低于樟深纵裂、脊细于白蜡）**——amplitudeRatio 0.016（低浮雕档——
 *     先例谱系（樟 0.036/夏栎 0.033/银杏 0.030/重阳 0.028/乌桕 0.027/白蜡 0.020）
 *     低带，与朴 0.016 同档）+ 谐波 {5,7}（细窄脊——**主干 radial 16 的奈奎斯特域
 *     8 ≥ 7+1 留 1 档边际**；比白蜡 {4,6} 细一档、比樟宽脊 {3,4,6} 细两档）+ drift
 *     3.4 rad/m（**顺直档**——「纵行大体平行」[10]：轴向去相关 ≈2π/3.4 ≈ 1.85m
 *     纵脊长程连续；「少量横向连接」由呼吸项承载）。机制不变：纯确定性函数零 rng；
 *     幅度 ∝ 局部半径（**亚视觉地板 3.0mm（LIGUSTRUM_BARK_RELIEF_MIN_EFFECTIVE）**：
 *     0.016 下起径 < 0.1875m 的管平滑发射 → **仅主干有效起伏**、全部分枝管光滑
 *     （「皮孔在枝不在干」FRPS [1] + 细浅纹显于主干的读向——灰褐色调/细纹浅色质感/
 *     细鳞微翘归材质层））；主干 radial 16（奈奎斯特域 8 容纳 k=7 留 1 档边际）。
 *   - **小枝两档不建模于几何**（任务书裁决 8：当年生枝黄褐-红调 vs 老枝灰褐——
 *     「枝黄褐色、灰色或紫红色」FRPS Verified [1] + 照片红铜新梢 [10]；**几何不区分
 *     枝色**——两档表达归 ligustrumMaterials 皮配方层（并行交付），此处记档）。
 * 不建模（Spec 有事实、几何不表达——记档见 ligustrumShapeProfile 模块头）：单叶
 *   内部结构（卵形-长卵形-椭圆-宽椭圆包络/全缘平坦/先端锐尖-渐尖/侧脉 4–9 对细弱
 *   不显/叶柄 1–3cm 具沟/两面色差——归材质 SDF）；**花**（圆锥花序 8–20 × 8–25cm
 *   顶生白 5–7 月——Verified [1][2][3][10]，任务书裁决 2 不做：时窗错位；无花果域
 *   v∈[5,7) 空）；果色序细档与宿存冬态（归材质 u 通道（翻转后自由轴）/风格层）；芽（Spec Unknown）；
 *   丛生多干相（form-b [10]）；f. latifolium 落叶变型；新叶红铜调（归材质变体轴）；
 *   树皮灰褐色调/细纹浅色质感/细鳞微翘/枝上皮孔（归材质层）；小枝两档色（归材质层，
 *   见上）。
 * 结构计数：皮拓扑（枝数/环数/径向段）槽间恒定 → 皮面数恒等（High 24236 账目见
 ...
 *      189×1 + L5 378×2）/每簇 10 卡为计数类（rng 消费次数恒定），**核果挂点零
 *      rng**（确定性账目——无消费口径问题），保留簇数与实际叶卡数随 seed 由簇级
 *      距离抑制 + 通透规则确定（同槽同 seed 恒等——确定性不破）。
 * 确定性纪律（家族纪律原样沿用）：簇生成与叶片候选的 rng 消费均为无条件固定次数
 *      （每簇：簇位 2 次（t 抖动/半径）+ 每卡 8 次（对首卡）/7 次（对次卡，r̂ 复用
 *      ——对生对共享节位的确定性消费结构，见 growBranch 实现注释）），被簇级距离
 *      抑制丢弃的簇位足额消费后丢弃；通透 roll 每卡无条件 1 次——任何条件跳过都
 *      禁止（档间同理：Mid/Low 的发射省略不省略消费）。**探针闭包教训（011.3 ③）**：
 *      rng 消费计数必须用包裹 rng 闭包实测（每次调用即计数），不得事后重建流复算
 *      ——消费次数以测试锁定快照为准。结构消费全部固定次数；总消费数随保留簇数
 *      变化（通透 roll 计数 = 存活候选数）——槽内三档恒等（同 seed 同保留簇集）、
 *      跨槽/跨 seed 随保留簇数浮动（实测带见 ligustrumStructure 测试快照注释）。
 ...
 *        0.44·hw，hw = 冠内高度权重）；树皮层同名属性写恒等值 0；**核果卡恒 0**（随
 *        皮组——果簇刚性悬垂，摆动语义归材质层；下垂摆动未表达归缺口候选）。
 * 边界：零外部模型（D13）；每次调用全部数据现算（无模块级缓存对象，D17 所有权随调用
 *      移交）；原点 = 底部中心（整体 Y 平移使 minY 精确 = 0）；叶卡 UV 标准 0–1 四边形
 *      域（v 轴 = 单叶长轴（叶基 0 → 叶尖 1）、u = 叶宽方向 0–1——单叶 SDF 消费归
 *      ligustrumMaterials——并行交付，签名冻结：createLigustrumBarkMaterial /
 *      createLigustrumLeafMaterial / createLigustrumLeafDepthMaterial，均
 *      (level?) => 材质〔导出约定沿 fraxinusMaterials〕）；**核果卡 uv v = 长轴
 *      ∈[4.0,4.97] / u = 逐果色档 ∈ [0,1)**（几何身份标记——核果入皮组接口记档，
 *      见上）；叶法线取卡面单侧（cross(side, dir)）。
 * LOD 三档（T011.11，家族方法逐位复制：level 为 Runtime 可选参数——不参与
 *      shapeSlot/morphSeed/sourceKey 形态身份计算；档位缓存维度 = sourceKey + level 归
 *      ProceduralSourceCache）：三档共用**同一条 rng 消费流**与同一套骨架/簇位/核果
 *      决策路径，Mid/Low 只在「发射」阶段降密度/降段数——被省略发射的站点/叶候选/果簇
 *      照常决策（果簇零 rng，决策 = 同一簇位表 + 同一区域判据 + 同一散列抽选——逐位
 *      同源），枝路径/簇位/核果挂点/冠形包络/通透过滤决策逐位同源：
 *      - High：全发射（缺省档；皮面数 24236）；树皮微起伏保留；核果簇全量烘焙；
 *      - Mid：径向段数降（主干/五级 16/8/7/6/5/4 → 6/5/4/3/3/3——粗枝保圆度、细枝
 *        三边管）+ 轴向站点隔 1 抽 1 发射（站点全算·游走 rng 全消费）+ L5 末梢管不
 *        发射（末梢径亚厘米，Mid 观距亚像素；簇位/果簇挂点照常派生）+ 簇内叶卡掩码
 *        j % 3 === 1（单叶卡簇 10 候选中 3 张（30%）——被弃候选足额消费 rng 后不进
 *        烘焙；通透过滤对全候选照常执行 → Mid 存活卡 ⊂ High 存活卡逐位同位）+ 树皮
 *        微起伏保留 + **核果簇全量保留**（身份信号——满冠下垂密簇中距可读 [10]，
 *        4 tri/果卡 预算轻）；皮 4514；
 *      - Low：主干 + L1 骨架极简管（径向 6/4、隔 1 抽 1；递归照常走完消费 rng）+
 *        冠 = High 簇位表驱动的壳层卡（每保留簇 2 张交叉竖卡，半幅 = 簇半径 + 0.22m
 *        余量（单叶大卡尺度：卡长 0.12–0.34 的常态伸出中位 ≈ 半卡长——同家族口径；
 *        极值叶尖仍由 High 决定冠包络——Low 不涨出，档间 bbox 一致性测试容差依据
 *        见 ligustrumLod）；宽轴 = 0.44 × 半幅——**单叶卡中位长宽比（1/2.25）在
 *        Low 壳卡的延续**（变比例域的档间剪影取中位比例））+ **核果簇不发射**（Low
 *        观距果簇亚像素——省略记档，档间连续记档同先例 Low 果处理）；皮 370。
 *      预算锁定账目（8 槽 × 3 档实测带 + 锁定依据）见
 *      ../assets/asset_tree_ligustrum.asset 模块头（预算制 D19.8，家族行沿用）。
 */
...
interface LeafCard {
  ...
  dir: THREE.Vector3; // 卡长方向（根→尖 = 叶基 → 叶尖；散簇叶恒指簇外平展——卡根朝簇心）
...
interface DrupeCard {
  /** 果梗端基点（果轴起点——uv v=4.0 端） */
  base: THREE.Vector3;
  /** 果长轴单位向量（果梗端 → 果尖，斜下外姿态——熟果下垂读向的弱表达） */
  axis: THREE.Vector3;
  /** 果长（0.014–0.020m——真果 7–10mm ×2 家族映射） */
  length: number;
  /** 果宽（0.008–0.012m——真果 4–6mm ×2） */
  width: number;
  /** uv u = 逐果色档随机 ∈ [0,1)（位置散列——材质果色档通道：绿-白绿未熟→紫黑蓝黑
   *  被白粉色序 [10]，冻结接口） */
  colorRoll: number;
}

/** 核果簇挂点记录：1 簇下垂果序（8–12 果卡——「数十果/序」的视觉密度档，绝对数不承重） */
interface DrupeSite {
  cards: DrupeCard[];
}
...
/** 构建上下文：发射槽 + 簇表 + 核果簇表 + 候选叶卡 + 通道表 + 逐级统计（全树共享，逐枝累加） */
interface BuildCtx {
  bark: BarkSink;
  clusters: ClusterRecord[];
  clustersCulled: number; // 簇级距离抑制丢弃的簇位数（工程账目）
  drupes: DrupeSite[]; // 核果簇挂点表（保留 L5 簇位确定性抽选——档间同源）
...
export interface LigustrumGeometryResult {
  geometry: THREE.BufferGeometry;
  stats: {
    barkTriangles: number;
    /** 叶卡三角（仅卡——leafCards × 2；核果三角单列） */
    leafTriangles: number;
    leafCards: number;
    /** 核果簇账目：挂点数（1 簇/点）/ 果卡数 / 核果三角（果卡 × 4 = 正反双 quad；
     *  Low = 0 省略）。入皮组（组 0）——组 0 三角 = 皮拓扑 + 核果（守恒） */
    drupeClusters: number;
    drupeCards: number;
    drupeTriangles: number;
...
// ── 女贞单叶卡 decussate 对生挂点语言常量（结构常数：槽间恒等——改值即改面数与 rng 消费）──

/** decussate 对生对步进角（π/2 = 90°——**对生叶序的交互对生标准式**（decussate：
 *  相邻叶对交互 ±90°），「叶对生」属级 Verified [3][4][7] 的几何落地——家族首例
 *  对生单叶挂点语言（与白蜡对生复叶共享「对生」位、叶层面为单叶）；vs 黄金角互生
 *  螺旋 137.5°：先例单叶系（朴/樟/榉/栎/槐/栾/桕/重阳）全部互生——女贞为单叶系
 *  首例对生（任务书裁决 5：沿 fraxinus decussate 机制直接继承，单叶卡替代复叶卡） */
const DECUSSATE_PAIR_STEP = Math.PI / 2;
/** 对生对方位抖动 ±0.20 rad（≈±11.5°——「对生规整」读向的排列可读优先小档抖动
 *  （leaf-a「opposite pairs + 顶芽单叶」/leaf-winter「数出 4–5 组对生叶对」双问
 *  [10]）；对生对结构是身份信号，抖动只提供自然不齐——fraxinus 同档沿用） */
const DECUSSATE_JITTER = 0.2;
/** 散簇单叶卡取向常量（平展语言——连续形态方法常量，非槽差异维度）：革质大叶平展
 *  层叠（冠面光泽斑驳暗示叶面多向受光 [10]——平展摊开 + 略前倾 + 弱抖动取向；
 *  fraxinus 同款常量沿用） */
const ALT_FORWARD_TILT = 0.24; // 沿枝向前倾分量（× 簇方向 T）
const ALT_WOBBLE = 0.15; // 弱球面抖动（自然不齐）
const PLANE_LIFT = 0.11; // 叶尖上举分量（平展摊开——家族常量）

// ── 肾形核果簇常量（Spec 判定做——任务书裁决 1；确定性挂点账目法：区域判据 + 位置
//    散列排序 + 固定步长抽选，零 rng 消费——挂点口径槽间恒等（结构常数）──

/** 果带判据（**满冠下垂密簇带**——fruit-b/c「满冠下垂密簇」[10]：**q ≥ 0.30 覆盖冠
 *  外-中部带**（fraxinus 帘幕型同判据直接沿用——满冠显著 vs 国槐 0.42 冠缘散点）；
 *  保留 L5 簇位（末级枝梢——果序顶生于当年生枝端 [1][2][3]，熟果下垂 [10]） */
const DRUPE_ZONE_Q_MIN = 0.3;
/** 果簇抽选步长（散列序每 6 取 1——满冠下垂密簇数量级工程定档：**Step 3 探针定档
 *  （2026-09-22）**：STRIDE 5 实测 Mid 总面 slot-2/7 越预算带上沿（10076/10260 > 10000
 *  ——果卡 4 tri/果 × Mid 全量保留的簇数-果卡数平衡），回调 6 终测落带见 asset 模块头
 *  （任务书数量级按 Mid 预算带（10000 − 皮 4514 − Mid 叶面）下定档；中距簇信号可读
 *  [10]、远距省略（Low 档）；无 rng roll——组 0 带不随 roll 浮动（011.6/011.7/
 *  011.9/011.10 方法复制） */
const DRUPE_STRIDE = 6;
/** 核果参数（fraxinus 确定性账目方法 + 肾形 quad 选型）：真果肾形 7–10 × 4–6mm
 *  （「果肾形或近肾形，长7-10毫米，径4-6毫米」FRPS Verified [1][2]）× 2 家族工程
 *  映射（platanus 果球/栾灯笼/国槐珠串/白蜡翅果同款 ×2 口径）⇒ 果长 0.014–0.020m
 *  / 果宽 0.008–0.012m；每簇果卡数 n = 8 + ⌊散列×5⌋ ∈ {8,…,12}（真实「每簇
 *  20–80 果/序」[10] 的**视觉密度档映射——绝对数不承重**（终审记档 5 口径））；
 *  果序轴下垂长 0.12–0.26m（真圆锥果序 8–20cm 的下垂主轴工程档）+ 果沿轴散布
 *  （簇内散布半径 0.03–0.08m——果序宽 8–25cm 的簇内散布工程档）、果长轴斜下外
 *  姿态（「熟果自枝端下垂悬挂」[10]——熟果下垂读向由簇卡朝向弱表达：果卡自轴斜下
 *  外伸，果尖朝下） */
const DRUPE_LEN_MIN = 0.014;
const DRUPE_LEN_SPAN = 0.006;
const DRUPE_WID_MIN = 0.008;
const DRUPE_WID_SPAN = 0.004;
const DRUPE_COUNT_BASE = 8;
const DRUPE_COUNT_SPAN = 5;
const DRUPE_AXIS_MIN = 0.12;
const DRUPE_AXIS_SPAN = 0.14;
const DRUPE_SPREAD_R_MIN = 0.03;
const DRUPE_SPREAD_R_SPAN = 0.05;
/** 果域 uv 常量：v ∈ [4.0, 4.97]（**果域 v∈[4,5] 族冻结口径（triadica/sophora/
 *  fraxinus 三重一致）**：果梗端 v=4.0 / 果尖 v=4.97（材质按 (v−4.0)/0.97 归一长轴
 *  分流核果配方——肾形轮廓的长轴坐标，面尖 4.97 < 5）；u = 逐果色档随机（位置散列，
 *  果内 12 顶点同值——材质果色档通道：绿-白绿未熟→紫黑蓝黑被白粉色序 [10]，
 *  冻结接口）。**2026-09-22 主代理终审裁定：派遣简报 u 轴系误写，对齐
 *  triadica/sophora/fraxinus 族冻结口径 v 轴**（族内不存无设计依据的轴向变体） */
const FRUIT_UV_V_BASE = 4.0;
const FRUIT_UV_V_TIP = 4.97;

// ── LOD 三档发射计划（家族方法复制——档位只改「发射」，不改骨架决策/rng 消费序；
//    核果：Mid 全量保留（身份信号）/ Low 省略（远距亚像素——先例 Low 果处理）──
 ...
 * 锁定见 ../assets/asset_tree_ligustrum.asset 模块头）：
 * - High 皮 24236（拓扑恒等不动：主干 464（14 段 ×16 + 底盖 16）+ L1 1008（7 枝 ×9 段
 ...
 * - Mid 叶 ≈ High 存活卡 × 掩码率（单叶卡簇 10 选 3 = 30%）+ 果簇全量；8 槽实测落
 *   6–10K 带。
...
function lodPlanFor(profile: BroadleafShapeProfile, level: ProceduralLevel): LodEmissionPlan {
  if (level === 'high') {
    ...
    // 24236 / rng 消费快照（ligustrumLod 测试锁）延续
    return {
  ...
  if (level === 'mid') {
    // 径向 16/8/7/6/5/4 → 6/5/4/3/3/3（骨架枝 5 边保中景圆度）+ 隔 1 抽 1 + L5 末梢
    // 不发射（亚厘米径亚像素）+ 叶卡掩码 j%3===1（单叶卡簇 10 选 3——8 槽实测 High 卡 →
    // Mid 总面落 6–10K 预算带的档率）+ 果簇全量保留（身份信号 [10]）
    return {
      radial: [6, 5, 4, 3, 3, 3],
...
/** Low 壳卡半幅余量（米）：High 散簇单叶大卡自簇心的最大伸出 ≈ 簇半径 + 卡长
 *  （≤0.34m 斜向——单叶卡 0.06–0.16 宽 × 长宽比 1.7–2.8 变比例 ⇒ 卡长 0.12–0.34m），
 *  典型伸出（r̂≤1 壳位 + 对生平摊）中位 ≈ 半卡长 0.10–0.17m——取 0.22（大卡档）使
 *  壳卡吞并簇半径 + 常态伸出域，且极值叶尖仍由 High 决定冠包络（Low 不涨出——
 *  8 槽实测 Low 水平跨恒窄于 High，档间 bbox 一致性测试容差依据见 ligustrumLod） */
const LOW_SHELL_MARGIN = 0.22;
/** Low 每保留簇壳卡数（交叉双竖卡：任意水平方位至少一卡正面可读——确定性几何，非 billboard） */
const LOW_SHELL_CARDS_PER_CLUSTER = 2;
/** Low 壳卡宽轴比（宽/高 = 0.44——**单叶卡中位长宽比 2.25（域 1.7–2.8 中位）的倒数
 *  延续**：变比例域的档间剪影取中位比例（vs 白蜡恒比例 0.36 冻结直用——女贞单叶
 *  变比例域的 Low 口径记档） */
...
/**
 * Low 壳卡发射：簇心交叉竖卡——宽轴 w（水平单位向量）× 宽半幅 = **0.44 × 半幅**
 * （**单叶卡中位长宽比在 Low 壳卡延续**——变比例域的档间剪影语义取中位；先例恒比例
 * 卡直用冻结比例的变体口径）、高轴 UP × 半幅；六顶点卡 / uv 0–1 四边形域 /
 * aLeafRand / aBend 契约与 High 叶卡同构（根边 v=0（叶基）、尖边 v=1（叶尖）；
 * aBend 根 0.12·hw / 尖 0.52+0.44·hw 与 High 同公式同常数——档间风相位/摆幅语义
...
function emitShellCard(
  ...
  const halfW = half * LOW_SHELL_WIDTH_RATIO; // 单叶中位长宽比 0.44 延续（宽轴）
...
/**
 * 单枚肾形核果卡发射：**扁平 quad 面片正反双 quad = 4 tri/果卡（12 顶点）**——fraxinus
 * 翅果选型沿用（双面由几何侧承担：材质组 0 FrontSide——冠下仰观（fruit-b/c 主视角）
 * 背面可见，身份信号关键视角）。正面绕序 (p0,p1,p2)/(p0,p2,p3) 法线 n = w×d、反面
 * 顶点序反转法线 −n（同 uv 同账）；uv 三角 (u,4.0)(u,4.0)(u,4.97)——**果域
 * v∈[4,5]**（几何身份标记：与叶卡 0–1 四边形域（组 1）及皮管弧长域（v = 累计弧长
 * × 0.5，8m 级最长枝弧实测 max 1.609——≤≈2.9 族纪律内无需压缩）双重隔离；
 * **无花资产（任务书裁决 2）→ v∈[5,7) 域空**）。**u = 逐果色档随机 ∈ [0,1)**
 * （位置散列、果内 12 顶点同值——材质果色档通道：绿-白绿未熟→紫黑蓝黑被白粉色序
 * [10]，冻结接口）；**账目入皮组（组 0）**——ligustrumMaterials（park-shader-agent
 * 并行交付、签名冻结）按 v 域分流核果配方；aLeafRand/aBend 随皮组恒 0（果簇刚性
 * 悬垂——摆动语义归材质层/缺口候选）。
 */
function emitDrupe(sink: BarkSink, card: DrupeCard): void {
  // 果宽轴：水平向 ⊥ 果长轴（肾形扁平面宽边；果长轴斜下外时取水平垂直）
  let w = card.axis.clone().cross(UP);
  if (w.lengthSq() < 1e-6) w = new THREE.Vector3(1, 0, 0); // 近铅垂果轴的确定性回退
  w.normalize();
  const halfW = card.width / 2;
  const d = card.axis;
  // quad 四角：果梗端边（v=4.0）p0/p1 → 果尖端边（v=4.97）p2/p3
  const p0 = card.base.clone().addScaledVector(w, -halfW);
  const p1 = card.base.clone().addScaledVector(w, halfW);
  const p2 = p1.clone().addScaledVector(d, card.length);
  const p3 = p0.clone().addScaledVector(d, card.length);
  const n = w.clone().cross(d).normalize();
  // 正面（绕序 (p0,p1,p2)/(p0,p2,p3)——法线 n 外向）+ 反面（顶点序反转——法线 −n）；
  // uv：果梗端 v=4.0 / 果尖 v=4.97（沿果长轴——材质肾形轮廓的长轴坐标）
  const push = (a: THREE.Vector3, b: THREE.Vector3, c: THREE.Vector3, sign: number): void => {
    ...
    for (const [v, vv] of verts) {
      ...
      sink.uv.push(card.colorRoll, vv);
    }
  };
  push(p0, p1, p2, 1);
  push(p0, p2, p3, 1);
...
//    全在 shapeProfile.barkRelief 三字段（女贞幅度/谐波/游走率见
//    ligustrumShapeProfile）──
...
/** 脊深呼吸（权重沿轴调制）频率（rad/m）/幅度——起伏深浅沿干交错（女贞细浅脊沟的
 *  深浅缓变读向 [10]；幅度工程设定） */
const BARK_RELIEF_BREATHE_FREQ = 2.4;
const BARK_RELIEF_BREATHE_AMP = 0.3;
/** 亚视觉幅度地板（米，峰值，**3.0mm——本资产档**）：管起径 × amplitudeRatio < 此值
 *  → 该管整体跳过起伏。抬档依据（细窄纵脊浅沟显于主干、皮孔在枝不在干）：FRPS
 *  「枝……疏生圆形或长圆形皮孔」（皮孔在枝 [1]）+ bark-a 主干细纹浅色质感 [10]；
 *  0.016 幅度下起径 < 0.1875m 的管（L1 骨架起径 0.07–0.09 亦在内）平滑发射 →
 *  **仅主干有效起伏**、全部分枝管光滑（vs fraxinus 3.5mm 同款抬档地板先例——
 *  女贞 8m 级根径 0.22–0.28 下 0.016 幅度取 3.0mm 保主干有效起伏域；灰褐色调/
 *  细纹浅色质感/细鳞微翘归材质层） */
const LIGUSTRUM_BARK_RELIEF_MIN_EFFECTIVE = 0.003;
...
interface ReliefHarmonic {
  ...
  /** 相位原点 */
  psi: number;
  /** 轴向游走率（±drift 域内确定性取值——女贞 drift 3.4：**顺直档**（轴向去相关
   *  ≈2π/3.4 ≈ 1.85m——「纵行大体平行」纵脊长程连续为主）；顺直脊沟族（樟 ±3.2/
   *  白蜡 ±3.6 邻档）/ 国槐 ±5.5 局部交叉 / 重阳 ±6.5 扭转交错。速率值 =
 ...
 * （女贞 driftₘ ±3.4 rad/m——顺直长程）、wₘ(s) = baseWₘ·(1 + breathe 正弦项)
...
function emitTube(
  ...
  // 亚视觉地板（3.0mm）：起径 × 幅度比 < 0.003 的管（L1 骨架起径 0.07–0.09 亦在
  // 内）整体按 0 起伏发射——仅主干有效起伏、全部分枝管光滑（细窄脊浅沟显于主干、
  // 皮孔在枝不在干 [1] 的几何侧读向，灰褐色调/细纹质感归材质层；抬档依据见常量注释）
  const effectiveAmplitude =
    relief.amplitudeRatio * radii[0]! >= LIGUSTRUM_BARK_RELIEF_MIN_EFFECTIVE ? relief.amplitudeRatio : 0;
 ...
 * L5×2 末梢）；枝角/长度/粗度/曲率 rng 驱动（骨架枝**单段角域直读**（36–58° 中角
 * 中带——中角骨架 30–60° 三源双问 [10]，无两段角带）+ upturn 链中正广卵上收
 * （[0.24,0.20,0.14,0.08,0.04]——樟广卵链端稍紧）+ 领导枝中庸-弱 0.50 续顶（圆钝顶
 * 由上段骨架链共构）——卵圆-广卵满密冠的结构成因，Spec §3 per 终审记档修正口径；
 * 无 zigzag 交替偏置（Spec 无女贞证据——对照轴记档））。单叶卡挂点：末两级枝梢
 * decussate 对生平展散布单叶卡簇（大簇 + 10 候选 = 5 对生对，对内共享节位 r̂）+
 * 保留 L5 簇位核果簇确定性抽选（满冠下垂密簇带判据 q ≥ 0.30，见模块头）。主次分级
 * 与冠内通透规则见模块头。
 * profile 缺省 = slot-0 标准组合（锚点回落——单测直调便捷路径，资产路径显式传槽
 * profile）。level 缺省 = 'high'（三档同流派生——档位只改发射密度，不改骨架决策/
 * rng 消费序，见 lodPlanFor）。
 */
export function buildLigustrumGeometry(
  rng: () => number,
  profile: BroadleafShapeProfile = LIGUSTRUM_SLOT0_PROFILE,
  level: ProceduralLevel = 'high',
): LigustrumGeometryResult {
  ...
  const totalHeight = 7.3 + rng() * 0.8; // 形态参数域 7.3–8.1m（≈8m 锚——终审裁决 2 维持：form-d 单整树样木 ≈8m 双系统一致 + NC 栽培域 + 25m 志书上限的中龄带；**Step 3 探针回调记档**：初值 7.5–8.3 实测 slot-0 涌现 8.82 偏锚 +0.8（广卵上收链 + 中庸领导的纵向外泄），回调 −0.2 后终测带见 asset 模块头）
  const crownRadius = (totalHeight * profile.crownWidthRatio * 0.5) * (0.94 + rng() * 0.12); // 冠幅半径 = 冠幅比驱动（含 ±6% 抖动）——工程域 0.75–0.85 涌现（form-d 主锚 + 开展端 1.0 读向，探针定档见 profile）
  const trunkBaseR = 0.22 + rng() * 0.06; // 根径 0.22–0.28m（DBH Unknown（照片不可标定 [10]）——工程设定，8m 中龄常绿乔木家族带内偏细档（vs 樟 0.26–0.32 / 白蜡 0.24–0.30））
  const trunkH = totalHeight * (0.36 + rng() * 0.06); // 干高 36–42%（trunk_height_ratio ≈0.30 form-d + bark-b 双证 Inferred [10] 的带内映射——挂高段消费后涌现视觉冠底 ≈0.30）
  const lean = randUnit(rng).multiplyScalar(jitter(rng, 0.05)); // 主干倾轴（≤2.9°——单干典型（form-d 双问 [10]）；丛生多干相记档不建模）
  ...
  // ── L1 骨架枝 ×6（**单段角域直读**：scaffoldAngleMin/Max 36–58° 中角中带直接消费
  //    ——中角骨架 30–60° 三源双问 [10]（无国槐两段角带/无白蜡修正史）；**无干向长度
  //    分级**（rank 乘子承担环内势差）；方位 60° 均分 + asymmetry 0.42 散布抖动（骨架
  //    排列 Unknown [10]——散布口径）；挂高 0.56–0.74 干高段 span 0.18（**中低分枝
  //    0.30**——冠下压读向）──
  ...
  // ── L1 领导枝 ×1（apical_dominance「中庸-弱」Inferred [10]——卵圆冠顶部圆钝读向：
  //    圆钝顶由上段骨架链共构、领导不翻转树顶；leaderLengthRatio 0.50 中庸带（白蜡
  //    0.50 同档），直立系数 0.92 为家族方法常量）──
  ...
  // ── 核果簇挂点确定性抽选（零 rng——消费顺序契约不含果簇；簇位表与档位无关 ⇒
  //    决策逐位同源；011.6/011.7/011.9/011.10 方法复制）：**资格 = 保留 L5 簇位
  //    （末级枝梢——圆锥果序顶生于当年生枝端 [1][2][3]）∧ q ≥ DRUPE_ZONE_Q_MIN
  //    （满冠下垂密簇带滤——fruit-b/c「满冠下垂密簇」[10]：q ≥ 0.30 覆盖冠外-中部带
  //    （fraxinus 帘幕型同判据直接沿用——满冠显著 vs 国槐 0.42 冠缘散点））**；
  //    位置散列排序 + 固定步长抽选（每 DRUPE_STRIDE 取 1）→ 下垂果簇（8–12 果卡，
  //    果参数逐簇/逐卡散列派生——见常量注释）──
  const l5Kept = ctx.clusters.filter((c) => c.level === 4);
  const drupeCands: { h: number; c: ClusterRecord }[] = [];
  for (const c of l5Kept) {
    if (qOf(c.center) >= DRUPE_ZONE_Q_MIN) {
      drupeCands.push({ h: posHash(c.center.x, c.center.y, c.center.z, 1.7), c });
    }
  }
  drupeCands.sort((a, b) => a.h - b.h);
  for (let i = 0; i < drupeCands.length; i += DRUPE_STRIDE) {
    const { c } = drupeCands[i]!;
    ...
    outward.normalize();
    // 果卡数：n = 8 + ⌊散列×5⌋ ∈ {8,…,12}（真实「每簇 20–80 果/序」[10] 的视觉密度档
    //  映射——绝对数不承重（终审记档 5））
    const countRoll = posHash(c.center.x, c.center.y, c.center.z, 9.1);
    const count = DRUPE_COUNT_BASE + Math.floor(countRoll * DRUPE_COUNT_SPAN);
    // 果序轴：自簇心下垂（-Y 主导 + 水平微斜散列——「熟果自枝端下垂悬挂」[10]），
    //  轴长 0.12–0.26m（真圆锥果序 8–20cm 的下垂主轴工程档）
    const axisLen = DRUPE_AXIS_MIN + DRUPE_AXIS_SPAN * posHash(c.center.x, c.center.y, c.center.z, 11.3);
    const axisDir = outward
      .clone()
      .multiplyScalar(0.3 * posHash(c.center.x, c.center.y, c.center.z, 13.7))
      .add(UP.clone().multiplyScalar(-1))
      .normalize();
    const cards: DrupeCard[] = [];
    for (let k = 0; k < count; k++) {
      // 果挂点：沿轴 t_k + 水平散布（簇内散布半径 0.03–0.08——果沿果序轴散生）
      ...
      const spreadR = DRUPE_SPREAD_R_MIN + DRUPE_SPREAD_R_SPAN * spreadRoll;
      const base = c.center
        .clone()
      ...
      // 果长轴：斜下外（自轴外伸 + 下垂主导——果尖朝下读向 [10]；**熟果下垂读向由
      //  簇卡朝向弱表达**）；近轴散列微变
      const droop = outward
        .clone()
      ...
      cards.push({
        base,
        axis: droop,
        length: DRUPE_LEN_MIN + DRUPE_LEN_SPAN * posHash(base.x, base.y, base.z, 31.9),
        width: DRUPE_WID_MIN + DRUPE_WID_SPAN * posHash(base.x, base.y, base.z, 37.3),
        colorRoll: posHash(base.x, base.y, base.z, 41.3),
      });
    }
    ctx.drupes.push({ cards });
  }

  // ── 烘焙名单：High = 全存活卡；Mid = 簇内候选序掩码子集（j % every === phase——
  //    单叶卡簇 10 候选 3 张；通透过滤对全候选照常执行 → Mid 存活卡 ⊂ High 存活卡
  //    逐位同位）；Low 壳卡模式不烘焙簇内叶卡与果簇（候选生成/通透过滤已照常走完
  //    ——果簇决策零 rng、决策表档间同源）──
  ...
  const bakedLeafCount = lod.shellCards
    ...
    : bakedCards.length;
  /** 烘焙核果簇数（High/Mid 全量保留——身份信号；Low 省略记档） */
  const bakedDrupeClusters = lod.shellCards ? 0 : ctx.drupes.length;
  /** 烘焙核果果卡数（High/Mid 全量保留；Low 省略） */
  const bakedDrupeCards = lod.shellCards ? 0 : ctx.drupes.reduce((s, f) => s + f.cards.length, 0);
  ...
  if (lod.shellCards) {
    ...
    //    半幅吞并簇半径（LOW_SHELL_MARGIN 校准依据见常量注释）；宽轴 = 0.44 × 半幅
    //    （单叶卡中位长宽比在 Low 延续——变比例域的档间剪影取中位比例）；果簇不发射
    //    （省略记档）──
    for (const cluster of ctx.clusters) {
  ...
  // ── 核果烘焙（High/Mid 全量：逐果卡双 quad（emitDrupe）——**入皮组（组 0）**：uv
  //    v = 果长轴 ∈[4.0,4.97] / u = 逐果色档 + aLeafRand/aBend 随皮组恒 0（材质侧
  //    冻结接口——按 v 域分流核果配方，见 emitDrupe 注释）；「满冠下垂密簇」Spec
  //    判定做（任务书裁决 1）的几何侧表达；Low 省略记档 ──
  if (!lod.shellCards) {
    for (const site of ctx.drupes) {
  ...
  const geometry = mergeGeometries([barkGeo, leafGeo], true); // 层间成组 → 恰 2 组（皮 0 / 叶 1——核果入皮组）
  barkGeo.dispose(); // 合并拷贝数据，层中间体即弃
  leafGeo.dispose();
  if (!geometry) throw new Error('程序化资产 asset_tree_ligustrum 层合并不兼容（属性集应一致：position/normal/uv/aLeafRand/aBend）');

  const groups = geometry.groups;
  /** 组 0 = 皮拓扑 + 核果（核果入皮组——材质接口约定）；皮拓扑面数 = 组 0 三角 −
   *  核果（结构锁 24236 口径不变） */
  const group0Tris = (groups[0]?.count ?? 0) / 3;
  const drupeTris = bakedDrupeCards * 4;
  return {
    geometry,
    stats: {
      barkTriangles: group0Tris - drupeTris,
      leafTriangles: bakedLeafCount * 2,
      leafCards: bakedLeafCount,
      drupeClusters: bakedDrupeClusters,
      drupeCards: bakedDrupeCards,
      drupeTriangles: drupeTris,
...
/**
 * 分枝递归：level 0–4（L1–L5）；路径 = 起点方向 + 每步游走 + 上举偏置（女贞 upturn
 * [0.24,0.20,0.14,0.08,0.04] 单调递减中正链——**广卵上收链端稍紧于樟**：外层末段上收
 * 使冠缘向上收口、顶部圆钝（卵圆读向上收快于樟广卵），同时抑制中角横展的冠幅外泄
 * ——探针校准依据；**无 zigzag 交替偏置**（Spec 无女贞证据——细级形态由 wander 梯度
 * 承载，末级外斜-稍下垂（果序重压 [10]）由末级乱幅 0.38 承载，对照轴记档））；起径 =
 * 父径 × profile.radiusRatio[level]、末径 = 起径 × profile.endRatio[level]；子枝挂点
 * 内埋父径内（起点回退 2.5×子径，杜绝接缝黑洞——树皮微起伏仅主干有效（抬档地板），
 * ≪ 内埋余量，接缝安全不变）；末两级 = **单叶卡挂点**（decussate 对生散簇挂枝梢簇位）；
 * 通透过滤在收冠后统一执行，内层稀疏由密度场接管。
...
function growBranch(
  ...
  for (let i = 0; i <= spec.segs; i++) {
    ...
    if (i < spec.segs) {
      ...
      // 上举偏置随 t 增强（末段姿态——女贞广卵上收链 [0.24,…,0.04]：外层末段上收
      // 成冠缘收口圆钝顶；卵圆-广卵主剪影由骨架侧驱动链承载：单段角域 + 挂高段
      // 中低 + 中庸领导）
  ...
  // ── 单叶卡挂点（末两级 L4/L5）──
  // 散簇（家族「枝梢驱动叶簇」的女贞语义）：枝梢 → 簇空间（⊥ 枝切向平面）→
  // **decussate 对生对散排**单叶卡——L4 外段 1 簇 / L5 沿途外段 + 枝端共 2 簇
  // （「叶对生，单叶」属级 Verified [3][4][7]——簇 = 末级枝对生单叶组的卡布置球）；
  // 簇间间隙 = 「簇只挂枝梢离散位 + 间距抑制」（拓扑相关处的父子/兄弟共位由簇级距离
  // 抑制保险，见下）；L5 簇位表同时为核果簇承载位表（满冠带判据 + 散列抽选——主生成
  // 段，零 rng）
  ...
  if (clusterCount > 0) {
    ...
    for (let i = clusterCount - 1; i >= 0; i--) {
      ...
      // 散簇平面基（⊥ 簇方向 = 挂点枝切向——革质大叶平展层叠的簇平面 [10]）
      ...
      /** 对内共享簇内径向位（同节对生语义——一对单叶自同一节位向相反方向展开：对首卡
       *  （j 偶数）消费 rng、对次卡（j 奇数）复用；消费次数由 j 奇偶确定性决定——
       *  固定次数纪律不破，女贞对生挂点语言的消费结构差异点 vs 互生系逐卡独立 r̂） */
      let pairRhat = 1;
      for (let j = 0; j < leavesPerCluster; j++) {
        ...
        // 对生线两正交列交替，「叶对生」属级 Verified [3][4][7] 的几何落地（家族首例
        // 对生单叶挂点语言）；vs 黄金角互生螺旋 φ_j = j×137.5°——单叶系内排列的直接
        // 对照轴；fraxinus 机制直接继承（任务书裁决 5））
        const phi =
        ...
        // 卡尖方向：辐射外向 + 平展上举（「革质大叶平展层叠」[10]——PLANE_LIFT）
        ...
        // 法线不变），叶尖恒指簇外——「单叶自枝梢节位向外生长」读向（根边 uv v=0 =
        ...
        // 长宽比变域消费（leafAspectSpan 1.1——真叶 1.7–2.8 域如实映射，vs 白蜡恒比例）
  ...
  if (level < profile.levels.length - 1) {
    ...
    for (let i = 0; i < plan.ts.length; i++) {
      ...
      if (isTip) {
        ...
        // branch_angle 度数 Unknown（中角开展定性 Inferred [10]，读向由角域 + upturn
        // 链承载），家族方法常量承载）
