/**
 * runtime/procedural/tree/ligustrum/ligustrumStage —— window.__ligustrum DEV
 * 出图面（T011.11；fraxinusStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/
 * T011.1–T011.10 能力面一次交付；方法零新面，数值面换女贞实数）。
 *
 ...
 *      sophora/fraxinus Stage 同款 = docs/procedural-assets/shadow-visual-sop.md §3：
 ...
 *      相位摆动，D19.7；女贞叶/皮材质同款消费实例 aSeed——整树缓摆同公式同相位 +
 *      aBend 单叶卡快颤双层（革质小叶卡抖动感）；皮/果组 aBend 恒 0 只随整树
 ...
 *      网格（间距缺省 11m——SOP §3 P42 口径；8 槽冠幅实测带最大 ≈8.54m 与邻槽窄端
 *      互补防交叠）挂 slots 组直挂 scene 兄弟层；viewSlots 全景 / viewSlot(i) 单槽
 *      特写固定机位（球坐标同 view 语义——「批量出图 → 分轮裁定」的取证载体）；
 *      stats 扩逐槽账目（**组 0 槽间平滑带**——皮拓扑恒 24236，肾形核果满冠下垂密簇
 *      并入皮组随槽 1596–2360 tri 平滑带（triadica/sophora/fraxinus 果序并入同款；
 *      核果挂点 = 确定性账目零 rng——簇数随槽簇位平滑变化），单叶卡数随槽形态向量
 *      各异（8 槽实测带 2600–4931 卡——**本模块不感知槽内容**，morphSeed → slot →
 ...
 *      createLigustrumLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。deps.build 注入位 =
 *      测试 seam（缺省女贞 asset build，产品路径不变——档位透传断言不依赖真实
 *      level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__ligustrum，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor/zelkova/ginkgo/platanus/
 *      koelreuteria/triadica/bischofia/sophora/fraxinus Stage 同为纯工厂，window 装配归组合根）；
 *      资源所有权归本句柄——unmount/dispose 摘自己的 group 并 dispose source 资源
 ...
 * 树高参考（机位 target y 基准——与 fraxinus 的差异点）：女贞 slot-0 锚 ≈8.41m 高
 *      （**Stage 实测涌现 8.4064**〔经正式 build() 路径探针 2026-09-22，bbox minY
 *      精确 0；几何代理初报 8.41 同值口径〕= ≈8m 长江流域城市公园/行道中龄个体锚 =
 *      终审裁决 2（form-d 单整树样木主锚——8 槽带 7.09–8.50、slot-3 8.496 全槽最
 *      高端、slot-0 8.406 次高），w/h 0.8223 落 Spec 主档 0.75–0.85 中带），视觉冠底/
 *      实高 **0.3231**（叶卡最低 Y 质心口径 2.7165——fraxinusShapeSlots.leafMetrics
 *      同款家族口径〔6 顶点/卡质心〕；顶点最低 Y 2.5341 = 几何代理初报 2.53 口径、
 *      比值 0.3015——两口径均落 [0.24,0.38] 带，中低分枝 ≈0.30 目标带）→ 冠域
 *      2.72–8.41 → 视心取**冠心 ≈5.6**（(2.7165+8.4064)/2 = 5.5614；vs fraxinus
 *      6.3 / sophora 5.9 / koelreuteria 5.9 / triadica 5.8 / bischofia 5.8 /
 *      camphor 5.5 / ginkgo 5.4 / zelkova 5.3 / celtis 4.3——≈8.4m 常绿密冠与
 *      camphor（≈8.6m 常绿同类）5.5 同档高半档；Stage 实际渲染 M25 观感复核（卵圆-
 *      广卵满密常绿冠 + 中低分枝裸干段 + 满冠核果下垂簇，5.6 视心上下留边均衡）；
 *      **材质树高锚消费口径**：实测涌现 8.4064 / 材质 nominal 8.0
 *      （LIGUSTRUM_TREE_HEIGHT_NOMINAL，风动 GLSL 1/8.0 = 0.125 + 冠基 2.4 工程
 *      锚）——同步轮（1/8.4064 ≈ 0.1190）归 ligustrumMaterials 侧，本 Stage 只
 *      记档）；stats 锚点实数 = slot-0 High 组 0 皮+核果 26116（皮拓扑恒 24236 +
 *      肾形核果 470 果×4 = 1880 并入皮组——果域 uv u∈[4,5] 冻结接口，
 *      triadica/sophora/fraxinus 先例）/ 单叶卡 7654 tri / 3827 卡（资产预算锁定
 *      账目：triangleCount 33770 = 26116 + 7654；单叶卡 = 2 tri/卡承载**单枚全缘
 *      单叶**（卵形-长卵形-椭圆-宽椭圆变比例）——vs fraxinus 复叶卡同 2 tri 承载
 *      整枚一回奇数羽状复叶、卡语义不同 tri/卡同 2；核果扁平 quad 正反双面 4 tri/
 *      果卡非卡面不入 leafCards——本模块不感知组内果/皮分离，归资产侧测试）。
 * 011.3 缺口 D 评估结论（T011.11 DEV Step 记档，归 011.13）：女贞树皮 = **灰褐
 *      细窄纵脊浅沟低浮雕（第 12 树皮语言，任务书裁决 7——0.016/{5,7}/3.4，沟深
 *      低于樟、脊细于白蜡；主干 radial 16 承载细窄脊奈奎斯特域）+ 干面细纹浅色质感
 *      + 小枝两档（当年生黄褐-红铜 vs 老枝灰褐）+ 小枝皮孔场**（FRPS「树皮灰褐色」
 *      +「枝黄褐色」+ 照片多源）——材质侧幼干-大枝上部弱化门控 smoothstep(2.8,5.0)
 *      为**连续过渡**（实测冠底 2.72/顶 8.41——3.6–6.4m 细枝两档高位×小弧长 v 双
 *      门控同为高位连续过渡（冠缘、机位天然覆盖）；同步编辑归 park-shader-agent，
 *      判定只依赖连续 vs 限带的结构性事实、不依赖常数值）；小枝皮孔场（High 专属
 *      近景身份层，FRPS「疏生圆形或长圆形皮孔」+ NC "Conspicuous"）域 = 当年生枝带
 *      smoothstep(4.8,6.4) 高位门控——**限带门控但位于冠心视域**（视心 5.6 正落带
 *      内，M25 机位天然覆盖拍得到——vs fraxinus 皮孔带 2.6–3.8m 干上段靠画面下部
 *      覆盖，女贞皮孔带在冠缘细枝、取景语义更顺）；核果域色序/白粉霜感为 u∈[4,5]
 *      域门控（满冠末级枝梢下垂簇——机位天然覆盖）；几何浅浮雕 = 主干起伏抬档地板
 *      的**起径门控**（半径门控非高度门控，0.020 下**仅主干有效起伏、全部分枝管
 *      光滑**（L1 骨架起径在地板下），主干满段有效起伏 → 冠心取景机位（视心 5.6）
 *      可拍全灰褐细窄纵脊浅沟身份（下半干脊沟 + 干基暗化 + 上部近光滑过渡 + 高位
     *      当年生黄褐-红铜细枝带皮孔浅点），**Stage 低目标机位能力 Step 4 复拍补 view()
     *      targetY 覆盖**（密冠近距下冠心机位叶幕遮挡主干取不到干段——白蜡同机位中通透
     *      成立、女贞 5–15% 空隙密档失效；仅 view 单树加参，slots/levels 机位面不扩散；
     *      缺口 D 其余收口仍归族门 011.13）。
 * Stage 观察项（T011.11 Step 4 裁定输入——本 Stage 只记档不修数值，沿 fraxinus
 *      「M25 绿带 px / 果色档 px」先例）：
 *      ①M25 冠面绿带 px 与密冠读向（空隙 5–15% 密档 vs 国槐 10–20% 致密端 / 白蜡
 *      25–40% 中通透——常绿密冠队列读向，族门密度横向对照第三对样本）；
 *      ②果簇色档像素存在性（紫黑-蓝黑被白粉 + 绿-白绿未熟色序在 M25 中距是否可辨
 *      ——满冠下垂密簇身份的像素证据，沿 fraxinus 翅果帘幕 M25 剪影读向先例）；
 *      ③对生叶序中距可辨性（decussate 对生**单叶**簇 vs 白蜡 decussate 对生**复叶**
 *      的横向对照——「家族首例对生单叶挂点语言」的画面验证）；
 *      ④冠底裸干带读向（视觉冠底比 0.3231 中低分枝 vs 白蜡 0.207 中位分歧 vs
 *      国槐低位放射）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_ligustrum.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createLigustrumLeafDepthMaterial } from './ligustrumMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface LigustrumStageDeps {
  ...
  /** 构建函数注入位（缺省 = 女贞 asset build；测试注桩 fake build——透传断言不依赖真实
   *  level 路由落地时序；产品路径 bootstrap 不传，行为不变） */
  build?: ProceduralBuild;
}

/** mountSlots 模式逐槽账目（slot 序号 + 该槽组 0 皮+核果/叶三角与叶卡数——组 0 含核果簇） */
export interface LigustrumSlotStats {
  slot: number;
...
/** mountLevels 模式逐档账目（LOD 档位 + 该档组 0 皮+核果/叶三角与叶卡数——档间可比的取证面） */
export interface LigustrumLevelStats {
...
/** window.__ligustrum 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface LigustrumHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'ligustrum-dev-slots' 组
   *  直挂 scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；8 槽冠幅实测带最大
   *  ≈8.54m 与邻槽窄端互补防交叠）；每 Mesh castShadow + customDepthMaterial（与
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：8 槽冠幅 XZ 最大 ≈8.54m 防交叠）；每档
   ...
   *  自建 createLigustrumLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
   ...
   *  targetY 目标高缺省 = 冠心 5.6（不传零变化），可传覆盖取低目标机位——树皮干段取证用
   *  （如 targetY 1.6 取裸干段：密冠近距下冠心机位叶幕遮挡主干，T011.11 Step 4 复拍） */
  view(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number; targetY?: number }): void;
  /** 8 槽全景固定机位：目标 = 网格中心（组位 + (0, 5.6, spacing/2)）；缺省 distance 42
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+8.54 ≈
   *  41.5m、纵深 ≈20m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨（P42 为全族固定口径，
   *  沿 tree3a/celtis/camphor/zelkova/ginkgo/platanus/koelreuteria/triadica/
   *  bischofia/sophora/fraxinus——42m 全景机位与网格几何绑定，不随冠幅缩距；celtis
   *  横跨 ≈42.9 / 栾 42.89 / 乌桕 42.57 / 重阳木 42.64 / 国槐 44.01 / 白蜡 42.79
   *  先例同沿 42，女贞 41.54 冠幅同位续沿） */
  viewSlots(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单槽特写机位：球坐标绕该槽树位（复用 view 公式，目标 = 该槽 x/z、视心高 ≈5.6）；
  ...
  /** 三树全景固定机位：目标 = 排中心（组位，三树 −s/0/+s 几何中心即组位）视心高 ≈5.6；
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 8.54
   *  ≈ 30.5m（8 槽冠幅 XZ 实测带 max ≈8.5442m = slot-2 开展宽端 w/h 1.104，
   *  T011.11 Stage 探针），32m 距离 35° 斜视 16° 俯角下三棵全入画可辨且留边（对齐
   *  viewSlots「distance ≈ 实宽」换算口径上取留边——沿 zelkova 30.5→32 / celtis 32 /
   *  platanus 32 / koelreuteria 31.9→32 / triadica 31.6→32 / bischofia 31.64→32 /
   *  sophora 33.0→34 / fraxinus 31.8→32 同族定档法；女贞 30.54 与 zelkova 30.5 同
   *  位 → 32 同法定档；垂直半角需求 ≈10°（视心 5.6 至地面 5.6m 向下端）同口径余量内） */
  viewLevels(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单档特写机位（viewSlot 的档位版）：球坐标绕该档树位（复用 view 公式，目标 = 该档
   *  x/z、视心高 ≈5.6）；缺省 distance 25 / azimuth 35 / elevation 8（同 view/viewSlot）；
  ...
  /** 账目：挂载态 + 面数（组 0 皮+核果/叶三角 + 叶卡数——顶层恒「总量」语义，slots 模式
   *  = 8 棵合计、levels 模式 = 三档合计；组 0 皮含并入核果簇） */
  stats(): {
    mounted: boolean;
    barkTriangles: number;
    leafTriangles: number;
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；8 槽冠幅实测带最大 ≈8.5442m（T011.11
 *  Stage 探针实测 slot-2 开展宽端 w/h 1.104；meta widthRange max 8.7 为声明带），11m
 *  网格下宽端与邻槽窄端互补（slot-2 邻槽 1/3 窄端半幅 ≈2.9/3.2m，半幅和 ≈7.5 < 11
 *  不交叠——P42 全族固定口径沿 tree3a/celtis/camphor/zelkova/ginkgo/platanus/
 *  koelreuteria/triadica/bischofia/sophora/fraxinus；42m 全景机位与网格几何绑定，
 *  不随冠幅缩距） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（8 槽冠幅 XZ 最大 ≈8.54m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 8.5442 ≈ 30.5m，「distance ≈ 实宽」
 *  口径上取留边（沿 zelkova 30.5→32 / celtis 32 / platanus 32 / koelreuteria 31.9→32 /
 *  triadica 31.6→32 / bischofia 31.64→32 / sophora 33.0→34 / fraxinus 31.8→32 同族
 *  定档法，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 2.7165–8.4064 → 5.5614 取 5.6；vs fraxinus
 *  6.3 / sophora 5.9 / koelreuteria 5.9 / triadica 5.8 / bischofia 5.8 / camphor
 *  5.5 / ginkgo 5.4 / zelkova 5.3 / celtis 4.3——≈8.4m 常绿密冠与 camphor（≈8.6m
 *  常绿同类）5.5 同档高半档，见模块头「树高参考」） */
const VIEW_TARGET_Y = 5.6;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createLigustrumHandle(deps: LigustrumStageDeps): LigustrumHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省女贞 asset build 行为不变） */
   ...
   *  fraxinus Stage 度→弧度换算同款）：绕 (tx, targetY, tz) 以方位/仰角定距放相机 */
  const placeCamera = (
    tx: number,
    tz: number,
    targetY: number,
  ...
  return {
    mount(opts = {}) {
      ...
      group.name = 'ligustrum-dev-stage';
      ...
      if (sourceDepth) {
        ...
        leafDepth = createLigustrumLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createLigustrumLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'ligustrum-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'ligustrum-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createLigustrumLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[ligustrumStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'ligustrum-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createLigustrumLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高缺省 ≈ 冠心（8.4064m 锚点树冠域 2.7165–8.4064 → ~5.6m 视心，见模块头「树高参考」）；
      // targetY 覆盖 = 低目标机位（树皮干段取证——密冠近距下叶幕遮挡主干，T011.11 Step 4 复拍）；
      // 树未挂时回退原点
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[ligustrumStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[ligustrumStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: LigustrumLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: LigustrumSlotStats[] = slotsSources.map((tree, slot) => {
