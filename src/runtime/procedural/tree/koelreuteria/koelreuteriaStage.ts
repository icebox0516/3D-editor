/**
 * runtime/procedural/tree/koelreuteria/koelreuteriaStage —— window.__koelreuteria DEV
 * 出图面（T011.6；platanusStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/T011.1/
 * T011.2/T011.3/T011.4/T011.5 能力面一次交付；方法零新面，数值面换栾树实数）。
 *
 ...
 *      celtis/camphor/zelkova/ginkgo/platanus Stage 同款 = docs/procedural-assets/
 *      shadow-visual-sop.md §3：主方位 az35 / 逆光 az215–275 / 侧方位 az110–125，
 *      缺省 25m/35°/8° = M25 机位）。mountWindDemo(count) 风动验收载体——build() 一次
 *      锚点源 → InstancedMesh ×N 间距 8m 一排，桶几何挂 aSeed InstancedBufferAttribute
 *      （值各异 → 同槽树不同相位摆动，D19.7；栾树叶/皮材质同款消费实例 aSeed——整树
 *      缓摆同公式同相位 + aBend 复叶快颤双层）；mesh 挂 customDepthMaterial（叶影 SDF
 *      裁切——build 产物自带 source.customDepthMaterial，挂载点同源消费 = 正式场景同一
 ...
 *      4×2 行主序网格（间距缺省 11m——SOP §3 P42 口径；展开槽冠幅最大 ≈9.89m 防交叠）
 *      挂 slots 组直挂 scene 兄弟层；viewSlots 全景 / viewSlot(i) 单槽特写固定机位
 *      （球坐标同 view 语义——「批量出图 → 分轮裁定」的取证载体）；stats 扩逐槽账目
 *      （**组 0 槽间平滑带**——皮拓扑恒 20782，花果并入皮组随槽 1900–2184 面平滑变化
 *      （花 44–47 序 ×4 + 灯笼 216–251 ×8 的确定性账目——零 rng 挂点法，vs platanus
 *      果序 rng roll 的组 0 带状浮动本例规避记档），组 0 带 22682–22966；复叶卡数随槽
 *      形态向量各异——**本模块不感知槽内容**，morphSeed → slot → shapeProfile 路由全在
 *      build 内）；转台旋转目标 = 当前挂载的主组（单树 / slots / levels；风动排不转
 *      ——平排观察语义不变）。mountLevels 档位强制出图面——指定槽三档实例沿 X 一字排开
 *      （high 左 / mid 中 / low 右），build({seed: morphSeedOf(id, slot), level}) 逐档
 *      独立 Mesh（castShadow）+ 逐档深度材质同源消费 source.customDepthMaterial
 *      （build({level}) 返回的深度材质已随 level 档位匹配；fake build 不带字段回退自建
 *      createKoelreuteriaLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。deps.build 注入位 =
 *      测试 seam（缺省栾树 asset build，产品路径不变——档位透传断言不依赖真实 level
 *      路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__koelreuteria，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor/zelkova/ginkgo/platanus Stage 同为
 *      纯工厂，window 装配归组合根）；资源所有权归本句柄——unmount/dispose 摘自己的
 *      group 并 dispose source 资源（geometry/material/customDepthMaterial——深度材质随
 *      source 释放）与自建回退深度材质/InstancedMesh 实例缓冲（build 契约每次 new 全部
 *      资源，绝无缓存共享误拆；深度材质两路对账：源带的归 disposeSource、fake build
 *      不带字段时回退自建的进自持数组释放——slots/levels 模式 = 8/3 份 source 逐一
 *      释放）；rAF 成对取消、unmount/dispose 幂等（StrictMode 双挂载下先卸载者只拆自己
 *      的）；time deps 未注入时 freeze/unfreeze 为 no-op（测试注桩/独立使用安全）。
 * 树高参考（机位 target y 基准——与 platanus 的差异点）：栾树 slot-0 锚 ≈9.85m 高
 *      （涌现 bbox 实测 9.8540，T011.6 探针——≈10m 中龄公园个体锚 = 主代理裁定，8–12m
 *      弱 Inferred 收口 9–11 默认 10 per 终审裁决 7；速生上探 12 变体入 slot-1 终测
 *      11.44 为 8 槽最高），视觉冠底/实高 0.193（叶卡最低质心 Y 1.906m，T009.3 口径
 *      ——koelreuteriaShapeSlots 同款测量）→ 冠域 1.91–9.85 → 视心取**冠心 ≈5.9**
 *      （(1.906+9.854)/2 ≈ 5.88），vs platanus 7.5（12m 级上层大乔）/ camphor 5.5 /
 *      ginkgo 5.4 / zelkova 5.3 / celtis 4.3——栾树 ≈10m 级中量级（五先例 8–8.6m 与
 *      悬铃木 12m 之间），冠心居族内第二高；stats 锚点实数 = slot-0 High 组 0 皮 22682
 *      （皮拓扑恒 20782 + 花序 43 序×4 = 172 + 灯笼 216×8 = 1728 并入皮组——花果入
 *      皮组冻结接口，koelreuteriaGeometry/koelreuteriaMaterials）/ 复叶卡 1586 / 793 卡
 *      （资产预算锁定账目：triangleCount 24268 = 22682 + 1586；复叶卡 = 2 tri/卡承载
 *      整枚二回羽叶，花卡/灯笼非卡面不入 leafCards——本模块不感知组内花果/皮分离，归
 *      资产侧测试）。
 * 011.3 缺口 D 评估结论（T011.6 Step 3 记档，归 011.13）：栾树树皮 = **满干型**——
 *      浅色光滑基底 + 皮孔麻点 52×78 格满干均匀（全干有效、无高度门控）+ 局部浅细纵裂
 *      为 tone>0.58 噪声域局部片门控（非高度限带）+ 干上部/细枝红褐收敛 smoothstep(3.2,
 *      5.5) 为**连续过渡**（非榉树「满斑区 <2.2m」式限带；3.2/5.5 为材质侧待同步的过时
 *      冠基锚——实测 9.854/1.906，同步编辑归 park-shader-agent，本判定只依赖连续 vs
 *      限带的结构性事实、不依赖常数值）+ 几何浅浮雕起径门控（<0.115m 管平滑——半径
 *      门控非高度门控，仅主干起伏且满段有效）→ 冠心取景机位可拍全干皮孔麻点身份，
 *      **Stage 低目标机位能力本轮不补**（缺口 D 维持榉树单例，归族门 011.13 收口）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_koelreuteria.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createKoelreuteriaLeafDepthMaterial } from './koelreuteriaMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface KoelreuteriaStageDeps {
  ...
  /** 构建函数注入位（缺省 = 栾树 asset build；测试注桩 fake build——透传断言不依赖真实
   *  level 路由落地时序；产品路径 bootstrap 不传，行为不变） */
  build?: ProceduralBuild;
}

/** mountSlots 模式逐槽账目（slot 序号 + 该槽组 0 皮/叶三角与叶卡数——组 0 含花果） */
export interface KoelreuteriaSlotStats {
  slot: number;
...
export interface KoelreuteriaLevelStats {
...
/** window.__koelreuteria 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface KoelreuteriaHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'koelreuteria-dev-slots' 组
   *  直挂 scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；展开槽冠幅最大
   *  ≈9.89m 防交叠）；每 Mesh castShadow + customDepthMaterial（与 mount 单树同待遇）；
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：同槽冠幅 XZ 最大 ≈9.89m 防交叠）；每档
   ...
   *  自建 createKoelreuteriaLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
   ...
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+9.89 ≈
   *  42.9m、纵深 ≈20m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨且留边（垂直半角需求 ≈8°
   *  < 典型 fov 25° 余量；P42 为全族固定口径，沿 tree3a/celtis/camphor/zelkova/ginkgo/
   *  platanus——42m 全景机位与网格几何绑定，不随冠幅缩距；celtis 横跨 ≈42.9m 先例同沿
   *  42，栾树 42.89 同位先例续沿） */
   ...
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 9.89
   *  ≈ 31.9m（同槽冠幅 XZ 最大 ≈9.89m = widthRange max，T011.6 探针实测带），32m 距离
   *  35° 斜视 16° 俯角下三棵全入画可辨且留边（对齐 viewSlots「distance ≈ 实宽」换算
   *  口径上取留边——沿 zelkova 30.5→32 / celtis 32 / platanus 32 同族定档法；栾树树高带
   *  9.8–11.5 不超 platanus 需求，32 同位定档；垂直半角需求 ≈11° 同口径余量内） */
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；展开槽冠幅最大 ≈9.89m（widthRange max
 *  9.89 = slot-2 老龄开张伞形端，T011.6 探针实测带），11m 留 ≈1.1m 防交叠（P42 全族
 *  固定口径沿 tree3a/celtis/camphor/zelkova/ginkgo/platanus——celtis 9.9m 冠幅同距
 *  ≈1m 先例；42m 全景机位与网格几何绑定，不随冠幅缩距） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（同槽冠幅 XZ 最大 ≈9.89m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 9.89 ≈ 31.9m，「distance ≈ 实宽」
 *  口径上取留边（沿 zelkova 30.5→32 / celtis 32 / platanus 32 同族定档法，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 1.91–9.85 → ≈5.88 取 5.9；vs platanus 7.5 /
 *  camphor 5.5 / ginkgo 5.4 / zelkova 5.3 / celtis 4.3，族内第二高——≈10m 级中量级，
 *  见模块头「树高参考」） */
const VIEW_TARGET_Y = 5.9;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createKoelreuteriaHandle(deps: KoelreuteriaStageDeps): KoelreuteriaHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省栾树 asset build 行为不变） */
   ...
   *  celtis/camphor/zelkova/ginkgo/platanus Stage 度→弧度换算同款）：绕 (tx, targetY, tz)
   *  以方位/仰角定距放相机 */
  const placeCamera = (
    tx: number,
    tz: number,
  ...
  return {
    mount(opts = {}) {
      ...
      group.name = 'koelreuteria-dev-stage';
      ...
      if (sourceDepth) {
        ...
        leafDepth = createKoelreuteriaLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createKoelreuteriaLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'koelreuteria-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'koelreuteria-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createKoelreuteriaLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[koelreuteriaStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'koelreuteria-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createKoelreuteriaLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（9.85m 锚点树冠域 1.91–9.85 → ~5.9m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[koelreuteriaStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[koelreuteriaStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: KoelreuteriaLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: KoelreuteriaSlotStats[] = slotsSources.map((tree, slot) => {
