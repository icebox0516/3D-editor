 ...
 *     每簇 10 候选（细叶中数量档）；簇间列距抑制 0.38（帘幕列间隙 0.1–0.25 的下界
 *     保险——2026-09-23 单发密度回调 0.44→0.38：垂帘列加密，保留簇带见 asset 模块头）。
 ...
 *     canopyDensity 0.88 + 壳带 0.42 + 芯层地板 0.10 + 空腔 3 个 0.45–0.78 + 中簇
 *     （0.26–0.40）+ shellBias 0.44（帘幕薄壳——细长卡放射）。（2026-09-23 单发密度
 *     回调：0.80→0.88 / 0.06→0.10 / 空腔 0.55–0.95→0.45–0.78——「一缕一缕」帘幕加密
 *     为连续绿帘读向、通透身份保留；改候选数路线经探针否决（rng 位移致 w/h 越域），
 *     记档见 salixShapeProfile / asset 模块头。）
 ...
 *        簇位沿索拉长 → 保留簇 640–822 级（vs 先例 ~474；2026-09-23 密度回调后带），双竖卡口径越 Low 预算
 ...
 *  0.30/0.35 域外段化）→ 保留簇 640–822 级（vs 先例 ~474；2026-09-23 密度回调后带）——双竖卡口径下 Low 总面
