/**
 * runtime/procedural/tree/zelkova/zelkovaStage —— window.__zelkova DEV 出图面（T011.3；
 * 香樟 camphorStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/T011.1/T011.2 能力面
 * 一次交付；方法零新面，数值面换榉树实数）。
 *
 ...
 *      celtis/camphor Stage 同款 = docs/procedural-assets/shadow-visual-sop.md §3：主方位
 ...
 *      相位摆动，D19.7；榉树叶/皮材质同款消费实例 aSeed——整树缓摆同公式同相位）；
 ...
 *      SOP §3 P42 口径；展开槽冠幅最大 ≈8.52m 防交叠）挂 slots 组直挂 scene 兄弟层；
 ...
 *      createZelkovaLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。
 *      deps.build 注入位 = 测试 seam（缺省榉树 asset build，产品路径不变——档位透传
 *      断言不依赖真实 level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__zelkova，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor Stage 同为纯工厂，window 装配归
 ...
 * 树高参考（机位 target y 基准——与 camphor 的差异点）：榉树 slot-0 锚 ≈7.96m 高
 *      （bbox 实测 7.958，T011.3 探针——与 ≈8m 中龄公园个体 Spec 锚一致），冠域
 *      2.74–7.96（干高中庸：干高参数域 0.33–0.38 vs 香樟 0.38–0.43）→ 视心取
 *      **冠心 ≈5.3**（(2.74+7.96)/2 ≈ 5.35），vs camphor 5.5（香樟冠域 2.47–8.58
 *      冠心 5.52）、celtis 4.3（朴树树半高口径）——榉树冠域与香樟同深但整树矮
 *      ≈0.6m，冠心随之下移；stats 锚点实数 = slot-0 High 皮 20782 / 叶 16038 /
 *      8019 卡（资产预算锁定账目：triangleCount 36820 = 20782 + 8019×2 + 探针
 *      复测一致）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_zelkova.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createZelkovaLeafDepthMaterial } from './zelkovaMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface ZelkovaStageDeps {
...
export interface ZelkovaLevelStats {
...
/** window.__zelkova 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface ZelkovaHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'zelkova-dev-slots' 组直挂
   *  scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；展开槽冠幅最大
   *  ≈8.52m 防交叠）；每 Mesh castShadow + customDepthMaterial（与 mount 单树同待遇）；
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：同槽冠幅 XZ 最大 ≈8.52m 防交叠）；每档
   ...
   *  自建 createZelkovaLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
  ...
  /** 8 槽全景固定机位：目标 = 网格中心（组位 + (0, 5.3, spacing/2)）；缺省 distance 42
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+8.52 ≈ 41.5m、
   *  纵深 ≈19m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨且留边（垂直半角需求 ≈10° <
   *  典型 fov 25° 余量；P42 为全族固定口径，沿 tree3a/celtis/camphor） */
  viewSlots(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单槽特写机位：球坐标绕该槽树位（复用 view 公式，目标 = 该槽 x/z、视心高 ≈5.3）；
  ...
  /** 三树全景固定机位：目标 = 排中心（组位，三树 −s/0/+s 几何中心即组位）视心高 ≈5.3；
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 冠幅 8.52
   *  ≈ 30.5m（同槽冠幅 XZ 最大 ≈8.52m，T011.3 探针实测带 widthRange max），32m 距离
   ...
   *  x/z、视心高 ≈5.3）；缺省 distance 25 / azimuth 35 / elevation 8（同 view/viewSlot）；
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；展开槽冠幅最大 ≈8.52m（widthRange max
 *  8.518，T011.3 探针实测带），11m 留 ≈2.5m 防交叠（夏栎 8.9/朴树 9.9 同距先例） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（同槽冠幅 XZ 最大 ≈8.52m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 8.52 ≈ 30.5m，「distance ≈ 实宽」口径
 *  上取留边（沿 P42 网格口径同族量级，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 2.74–7.96 → ≈5.35 取 5.3；vs camphor 5.5 /
 *  celtis 4.3，见模块头「树高参考」） */
const VIEW_TARGET_Y = 5.3;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createZelkovaHandle(deps: ZelkovaStageDeps): ZelkovaHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省榉树 asset build 行为不变） */
   ...
   *  camphor Stage 度→弧度换算同款）：绕 (tx, targetY, tz) 以方位/仰角定距放相机 */
  const placeCamera = (
    tx: number,
    tz: number,
    targetY: number,
  ...
  return {
    mount(opts = {}) {
      ...
      if (sourceDepth) {
        ...
        leafDepth = createZelkovaLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createZelkovaLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'zelkova-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'zelkova-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createZelkovaLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[zelkovaStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'zelkova-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createZelkovaLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（7.96m 锚点树冠域 2.74–7.96 → ~5.3m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[zelkovaStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[zelkovaStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: ZelkovaLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: ZelkovaSlotStats[] = slotsSources.map((tree, slot) => {
