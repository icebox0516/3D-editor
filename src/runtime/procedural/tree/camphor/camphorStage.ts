/**
 * runtime/procedural/tree/camphor/camphorStage —— window.__camphor DEV 出图面（T011.2；
 * 朴树 celtisStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/T011.1 能力面一次交付）。
 *
 ...
 *      （写主相机 + controls.target——固定机位取证的驱动面；球坐标口径与 tree3a/celtis
 *      Stage 同款 = docs/procedural-assets/shadow-visual-sop.md §3：主方位 az35 / 逆光
 ...
 *      相位摆动，D19.7；香樟叶/皮材质同款消费实例 aSeed——整树缓摆同公式同相位）；
 *      mesh 挂 customDepthMaterial（叶影 SDF 裁切——T011.2 起 build 产物自带
 ...
 *      SOP §3 P42 口径；展开槽冠幅最大 ≈7.96m 防交叠）挂 slots 组直挂 scene 兄弟层；
 ...
 *      createCamphorLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。
 *      deps.build 注入位 = 测试 seam（缺省香樟 asset build，产品路径不变——档位透传
 *      断言不依赖真实 level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__camphor，
 *      生产零痕迹——本模块与 tree3a/celtis Stage 同为纯工厂，window 装配归组合根）；
 ...
 * 树高参考（机位 target y 基准——与 celtis 的差异点）：香樟 slot-0 锚 ≈8.58m 高
 *      （bbox 实测 8.582，T011.2 探针），冠域 2.47–8.58（干矮冠深——trunk_height_ratio
 *      0.30–0.35 vs 朴树 0.44–0.50）→ 视心取**冠心 ≈5.5**（(2.47+8.58)/2 ≈ 5.52），
 *      vs celtis 4.3（朴树树半高口径）——香樟树半高 4.29 偏低于冠体质量中心，冠心
 *      取景冠层更居中；stats 锚点实数 = slot-0 High 皮 20782 / 叶 12920 / 6460 卡
 *      （资产预算锁定账目 + 探针复测一致）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_camphor.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createCamphorLeafDepthMaterial } from './camphorMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface CamphorStageDeps {
...
export interface CamphorLevelStats {
...
/** window.__camphor 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface CamphorHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'camphor-dev-slots' 组直挂
   *  scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；展开槽冠幅最大
   *  ≈7.96m 防交叠）；每 Mesh castShadow + customDepthMaterial（与 mount 单树同待遇）；
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：同槽冠幅 XZ 最大 ≈7.96m 防交叠）；每档
   ...
   *  自建 createCamphorLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
  ...
  /** 8 槽全景固定机位：目标 = 网格中心（组位 + (0, 5.5, spacing/2)）；缺省 distance 42
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+7.96 ≈ 41m、
   *  纵深 ≈19m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨且留边（垂直半角需求 ≈10° <
   *  典型 fov 25° 余量） */
  viewSlots(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单槽特写机位：球坐标绕该槽树位（复用 view 公式，目标 = 该槽 x/z、视心高 ≈5.5）；
  ...
  /** 三树全景固定机位：目标 = 排中心（组位，三树 −s/0/+s 几何中心即组位）视心高 ≈5.5；
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 冠幅 7.96
   *  ≈ 30m（同槽冠幅 XZ 最大 ≈7.96m，T011.2 探针实测带），32m 距离 35° 斜视 16° 俯角
   *  下三棵全入画可辨且留边（对齐 viewSlots「distance ≈ 实宽」换算口径上取留边；垂直
   *  半角需求 ≈6° 同口径余量内） */
  viewLevels(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单档特写机位（viewSlot 的档位版）：球坐标绕该档树位（复用 view 公式，目标 = 该档
   *  x/z、视心高 ≈5.5）；缺省 distance 25 / azimuth 35 / elevation 8（同 view/viewSlot）；
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；展开槽冠幅最大 ≈7.96m（widthRange max
 *  7.964，T011.2 探针实测带），11m 留 ≈3m 防交叠 */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（同槽冠幅 XZ 最大 ≈7.96m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 7.96 ≈ 30m，「distance ≈ 实宽」口径
 *  上取留边（沿 P42 网格口径同族量级，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 2.47–8.58 → ≈5.52 取 5.5；vs celtis 树半高 4.3，
 *  见模块头「树高参考」） */
const VIEW_TARGET_Y = 5.5;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createCamphorHandle(deps: CamphorStageDeps): CamphorHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省香樟 asset build 行为不变） */
  ...
  return {
    mount(opts = {}) {
      ...
      if (sourceDepth) {
        ...
        leafDepth = createCamphorLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createCamphorLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'camphor-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'camphor-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createCamphorLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[camphorStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'camphor-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createCamphorLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（8.58m 锚点树冠域 2.47–8.58 → ~5.5m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[camphorStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[camphorStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: CamphorLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: CamphorSlotStats[] = slotsSources.map((tree, slot) => {
