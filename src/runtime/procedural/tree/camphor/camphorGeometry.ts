/**
 * runtime/procedural/tree/camphor/camphorGeometry —— 香樟（Cinnamomum camphora）CPU
 * 几何生成器（T011.2，阔叶家族第三实例——方法复制自夏栎第一实例经朴树（T011.1）二次
 * 验证的通路 ../celtis/celtisGeometry，契约字段语义不变：五级递归分枝拓扑 / 锥度管状
 * 枝干 / 枝梢驱动叶簇 / 冠内通透三规则 / 树皮近景微起伏 / LOD 三档同流派生全部沿用；
 * 香樟数值与算法细节差异点见下）。
 *
 ...
 *      叶 1）。形态参数类型 = 阔叶家族契约 ../broadleaf/broadleafShapeProfile（香樟为
 *      第三实例），数值与槽组合见 ./camphorShapeProfile（全部数值依据
 *      docs/research/camphor-reference.md Spec 1.0——含主代理终审记档：比例类数值经
 *      第二视觉系统交叉确认）；build 只做 slot → profile 路由，不进 ProceduralBuild
 *      公共签名。
 * 香樟算法细节差异点（vs 朴树模板，逐条 Spec 引用见 camphorShapeProfile 内联注释）：
 *   - 尺度锚：中龄公园个体 ≈8m 量级（Spec §2 公园中龄弱 Inferred [5][6]，任务书锚 ≈8m
 *     在域内）——形态参数域树高 7.9–8.8m；**干高参数域 0.38–0.43**（vs 朴树 0.44–0.50
 *     ——香樟干更矮、冠更下压：trunk_height_ratio ≈0.30–0.35，Spec 域扩展节 A 主代理
 *     终审双系统一致 [6]）；根径 0.26–0.32m（DBH 典型值 Unknown，工程设定——樟速生
 *     [5]、干基轻膨大定性 [6]）。
 *   - 树形：**广卵形冠**（中下部宽、向上渐收、顶部圆钝——Spec §3 树形语言 Verified
 *     [1][2][5][6] + 主代理终审照片双样木一致）——几何侧由低挂高段 + 开张横展角 +
 *     中庸偏强领导枝（leaderLengthRatio 0.52 基准：apical_dominance 中庸偏强 Inferred
 *     [1][6]，顶钝渐收读向）实现；密度场参考系（crownCenter 0.62 / crownTopBias 0.10）
 *     同读向配合（T009.3 消费语义：不动几何只调分布）。
 *   - 骨架：**5 骨架枝 + 1 领导枝**（≈5–7 域下段 Inferred [6]——vs 朴树 6+1；拓扑分化：
 *     L1 枝数 6 vs 朴树 7、簇位 810 vs 朴树 945，皮面 20782 vs 朴树 24178）；横展
 *     46–68° 对铅垂（Spec scaffold_angle ≈45–70° 开张横展，主代理终审交叉一致 [6]——
 *     上限比朴树 46–64° 更开张）。
 *   - 常绿满密：冠密度基调高于朴树（Spec §3 冠层满密浓绿四样木四季 Verified [6]；
 *     crown_transparency 空隙 ≈10–15% Inferred）——满密壳带更厚（crownShellStart 0.48）+
 *     空腔更小（voidRadius 0.42–0.72）+ 芯层不空透（coreDensityFloor 0.08——内膛空枝
 *     存在但幅度小 Inferred [6]）+ 每簇叶量 18（末级枝叶量中-高密 Inferred [6] + 预算
 *     校准）。
 *   - 树皮近景微起伏：**纵裂深沟语言**（bark_archetype 不规则纵裂、脊宽沟深、局部横纹
 *     连接成块状感 Verified [1][2][6] + 主代理终审；中龄即充分纵裂 Inferred [6]）——
 *     amplitudeRatio 0.036（中-强浮雕 bark_relief Verified [6]，高于朴树 0.016、量级
 *     强于夏栎 0.033）+ 谐波 {3,4,6}（k3/4 宽主脊 + k6 次级细纹）+ drift 3.2 rad/m
 *     （轴向去相关 ≈2.0m——纵脊轴向连续性强于朴树短斑（drift 16）、弱于夏栎连续纵脊
 *     （0.85），呼吸调制使局部横断成**块状感**——三分化中间偏脊端）。机制不变：纯确定
 *     性函数零 rng（拓扑/uv/绕序不变、wrap 位浮点无缝、法线解析修正）；起伏幅度 ∝ 局部
 *     半径（0.036 下起径 < 0.042m 的管（L3 以下细枝典型域）平滑发射——主干+L1+L2 深沟
 *     浮雕、细枝平滑）；主干 radial 14（奈奎斯特域 7 容纳 k=6 留 1 档边际——家族同款）。
 *   - 叶卡：卡宽 0.06–0.095m（真叶典型 7–10 × 3–4.5cm Verified [1][2][5][6] × ≈2 工程
 *     映射——夏栎/朴树先例口径）+ 长宽比 1.8–2.4（leaf_aspect_ratio Inferred [1][2][6]
 *     ——比朴树 1.3–2.0 明显狭长的卵状椭圆语言，香樟叶身份）。
 * 不建模（Spec 有事实、几何不表达——记档见 camphorShapeProfile 模块头）：樟脑气味 /
 *    果实 6–8mm / 春季换叶季相 / 叶柄 / 离基三出脉与脉腋腺窝（归材质 SDF）/ 苔藓附生
 *    （归树皮材质）。
 ...
 *      域（SDF 叶形 alpha 消费归 camphorMaterials——并行交付，签名冻结）；叶法线取卡面
 *      单侧（cross(side, dir)）。
 * LOD 三档（T011.2，家族方法逐位复制：level 为 Runtime 可选参数——不参与
 ...
 *      消费 rng（三档 rng 消费总数恒等，快照数见 camphorLod 测试），枝路径/簇位/冠形
 *      包络/通透过滤决策逐位同源：
 ...
 *      ../assets/asset_tree_camphor.asset 模块头（预算制 D19.8，家族行沿用）。
 */
import * as THREE from 'three';
import { mergeGeometries } from 'three/examples/jsm/utils/BufferGeometryUtils.js';
import { CAMPHOR_SLOT0_PROFILE } from './camphorShapeProfile';
...
export interface CamphorGeometryResult {
 ...
 * 锁定见 ../assets/asset_tree_camphor.asset 模块头）：
...
function lodPlanFor(profile: BroadleafShapeProfile, level: ProceduralLevel): LodEmissionPlan {
  if (level === 'high') {
    ...
    // 20782 / rng 消费快照（camphorLod 测试锁）延续
    return {
...
/** Low 壳卡半幅余量（米）：High 簇内叶卡自簇心的最大伸出 ≈ 簇半径 + 卡长（≤0.23m 斜向），
 *  典型伸出（r̂≤1 壳位 + 近水平摊开卡）中位 ≈ 半卡长 0.08–0.12m——取 0.12 使壳卡吞并簇
...
//    全在 shapeProfile.barkRelief 三字段（香樟幅度/谐波/游走率见 camphorShapeProfile）──
...
/** 脊深呼吸（权重沿轴调制）频率（rad/m）/幅度——起伏深浅沿干交错（香樟「局部横纹连接
 *  成块状感」Spec bark_archetype Verified [1][2][6] 定性方向的工程映射——纵脊连续（低
 *  drift）+ 呼吸调制断块，幅度工程设定） */
const BARK_RELIEF_BREATHE_FREQ = 2.4;
const BARK_RELIEF_BREATHE_AMP = 0.3;
/** 亚视觉幅度地板（米，峰值）：管起径 × amplitudeRatio < 此值 → 该管整体跳过起伏
 *  （近景不可辨的亚毫米层，兼免低径向段细枝上高次谐波混叠成无意义计算——香樟
 *  amplitudeRatio 0.036 下起径 < 0.042m 的管（L3 以下细枝典型域）平滑发射，主干+L1+L2
 *  纵裂深沟、细枝平滑的读向自然涌现） */
...
interface ReliefHarmonic {
  ...
  /** 相位原点 */
  psi: number;
  /** 轴向游走率（±drift 域内确定性取值——香樟 drift 3.2：纵脊轴向连续 + 局部游走断块；
   *  夏栎 0.85：纵脊缓游走；朴树 16：短斑快断裂。速率值 = profile.barkRelief.drift） */
  driftRate: number;
  /** 游走正弦相位 */
  chi: number;
  /** 呼吸正弦相位 */
 ...
 * 半径；d_θ/d_s 为解析导数（法线修正源）。φₘ(s) = ψₘ + driftₘ·s + wander 正弦项（香樟
 * driftₘ ±3.2 rad/m——纵脊轴向连续 + 局部游走断块；夏栎 ±0.85 纵脊缓游走、朴树 ±16
 * 短斑快断裂）、wₘ(s) = baseWₘ·(1 + breathe 正弦项)（脊深呼吸）——均弧长 s 的缓变函数。
...
function emitTube(
  ...
  // 亚视觉地板：起径 × 幅度比 < 1.5mm 的管（香樟 0.036 幅度下 L3 以下细枝典型域）
 ...
 * L5×2 末梢）；枝角/长度/粗度/曲率 rng 驱动（骨架枝横展开张——香樟干矮冠下压语言；
 * 领导枝中庸偏强续顶——广卵形顶部圆钝渐收）。主次分级与冠内通透规则见模块头。
 * profile 缺省 = slot-0 标准组合（锚点回落——单测直调便捷路径，资产路径显式传槽
 * profile）。level 缺省 = 'high'（三档同流派生——档位只改发射密度，不改骨架决策/
 * rng 消费序，见 lodPlanFor）。
 */
export function buildCamphorGeometry(
  rng: () => number,
  profile: BroadleafShapeProfile = CAMPHOR_SLOT0_PROFILE,
  level: ProceduralLevel = 'high',
): CamphorGeometryResult {
  ...
  const totalHeight = 7.9 + rng() * 0.9; // 形态参数域 7.9–8.8m（Spec §2 公园中龄 ≈8m 弱 Inferred [5][6]；与夏栎/朴树同量级可混植）
  const crownRadius = (totalHeight * profile.crownWidthRatio * 0.5) * (0.94 + rng() * 0.12); // 冠幅半径 = 冠幅比驱动（含 ±6% 抖动）
  const trunkBaseR = 0.26 + rng() * 0.06; // 根径 0.26–0.32m（DBH 典型值 Unknown——工程设定；干基轻膨大定性 [6] 由 flare 承载）
  const trunkH = totalHeight * (0.38 + rng() * 0.05); // 干高 38–43%（vs 朴树 44–50%——香樟干更矮冠更下压，Spec 域扩展节 A 主代理终审双系统一致 [6]）
  const lean = randUnit(rng).multiplyScalar(jitter(rng, 0.07)); // 主干倾轴（≤4°——trunk_lean_angle 近直立 Inferred [6]「笔直」）
  ...
  for (let i = 0; i <= profile.trunk.segs; i++) {
    ...
    const flare = 1 + 0.32 * Math.exp(-t * 7); // 根部张拉（basal_flare Unknown——干基轻膨大定性 [6] 的方向性沿用）
  ...
  // ── L1 骨架枝 ×5（横展 46–68° 对铅垂——Spec scaffold_angle ≈45–70° 开张横展主代理终审
  //    交叉一致 [6]；方位 72° 均分 + asymmetry 缩放抖动，挂高 0.48–0.68 干高段——干高
  //    占比 0.30–0.35 的挂高段实现；rank 乘子拉开水势差——首枝主导 ×1.14，主次分级）──
  ...
  for (let i = 0; i < scaffoldCount; i++) {
    ...
    // 横展角（对铅垂 46–68°）→ 方向分量：水平 sin / 铅垂 cos
  ...
  // ── L1 领导枝 ×1（中庸偏强续顶——Spec apical_dominance 中庸偏强 Inferred [1][6]，
  //    广卵形冠顶部圆钝渐收的顶链闭合；强度差异全部由 profile.leaderLengthRatio 表达，
  //    直立系数 0.92 为家族方法常量）──
  ...
  leafGeo.dispose();
  if (!geometry) throw new Error('程序化资产 asset_tree_camphor 层合并不兼容（属性集应一致：position/normal/uv/aLeafRand/aBend）');
...
/**
 * 分枝递归：level 0–4（L1–L5）；路径 = 起点方向 + 每步游走 + 上扬偏置（香樟 upturn 比
 * 朴树整体收小——46–68° 开张角的冠体横展、末级 0.03 近水平摊出，冠缘圆钝收口）；起径 =
 * 父径 × profile.radiusRatio[level]、末径 = 起径 × profile.endRatio[level]；子枝挂点内埋
 * 父径内（起点回退 2.5×子径，杜绝接缝黑洞——树皮微起伏幅度 ≈ 3.6% 局部半径，≪ 内埋
 * 余量，接缝安全不变）；末两级挂枝梢驱动叶簇（簇挂点外段/枝端、簇方向 = 局部切向、簇内
 * 壳偏置发叶片候选；通透过滤在收冠后统一执行，内层稀疏由密度场接管）。
...
function growBranch(
  ...
  for (let i = 0; i <= spec.segs; i++) {
    ...
    if (i < spec.segs) {
      ...
      // 上扬偏置随 t 增强（横枝末段姿态——香樟末级 0.03 近水平摊出 + 游走 = 冠缘圆钝
      // 收口；广卵形向上渐收由领导链与 upturn 分级承载）
  ...
  // 枝端共 2 簇（叶互生一年生枝 Verified [1][2][5]、叶集中末级枝外段受光区 Inferred [6]）；
  ...
  if (level < profile.levels.length - 1) {
    ...
    for (let i = 0; i < plan.ts.length; i++) {
      ...
      if (isTip) {
        ...
        // 侧枝：方位按父相位角均分展开（冠向四周填充 + 抖动；二级枝对一级枝 ≈40–60°——
        // Spec branch_angle Inferred [6]，家族方法常量承载）
