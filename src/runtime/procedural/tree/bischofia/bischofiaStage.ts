/**
 * runtime/procedural/tree/bischofia/bischofiaStage —— window.__bischofia DEV
 * 出图面（T011.8；triadicaStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/T011.1/
 * T011.2/T011.3/T011.4/T011.5/T011.6/T011.7 能力面一次交付；方法零新面，数值面换重阳木
 * 实数）。
 *
 ...
 *      celtis/camphor/zelkova/ginkgo/platanus/koelreuteria/triadica Stage 同款 =
 ...
 *      InstancedBufferAttribute（值各异 → 同槽树不同相位摆动，D19.7；重阳木叶/皮材质
 *      同款消费实例 aSeed——整树缓摆同公式同相位 + aBend 复叶卡快颤双层（长总柄三出
 *      复叶飘逸感）；皮组 aBend 恒 0 只随整树缓摆）；mesh 挂
 ...
 *      冠幅最大 ≈9.64m 防交叠）挂 slots 组直挂 scene 兄弟层；viewSlots 全景 /
 *      viewSlot(i) 单槽特写固定机位（球坐标同 view 语义——「批量出图 → 分轮裁定」的
 *      取证载体）；stats 扩逐槽账目（**组 0 槽间恒等**——**无花果资产组 0 = 纯皮拓扑**
 *      恒 24178（vs triadica 果序并入皮组随槽 480–672 面平滑带/platanus 果序 rng roll
 *      浮动——本例无附加元素账目，组 0 带退化为恒等记档），复叶卡数随槽形态向量各异
 *      （8 槽实测带 2052–3097 卡——**本模块不感知槽内容**，morphSeed → slot →
 *      shapeProfile 路由全在 build 内）；转台旋转目标 = 当前挂载的主组（单树 / slots /
 *      levels；风动排不转——平排观察语义不变）。mountLevels 档位强制出图面——指定槽
 *      三档实例沿 X 一字排开（high 左 / mid 中 / low 右），build({seed: morphSeedOf(id,
 *      slot), level}) 逐档独立 Mesh（castShadow）+ 逐档深度材质同源消费
 *      source.customDepthMaterial（build({level}) 返回的深度材质已随 level 档位匹配；
 *      fake build 不带字段回退自建 createBischofiaLeafDepthMaterial(level)）；
 *      viewLevels 三树全景 / viewLevel(level) 单档特写（球坐标同 view 语义）；stats 扩
 *      levels 逐档账目。deps.build 注入位 = 测试 seam（缺省重阳木 asset build，产品路
 *      径不变——档位透传断言不依赖真实 level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__bischofia，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor/zelkova/ginkgo/platanus/
 *      koelreuteria/triadica Stage 同为纯工厂，window 装配归组合根）；资源所有权归本
 *      句柄——unmount/dispose 摘自己的 group 并 dispose source 资源（geometry/material/
 ...
 * 树高参考（机位 target y 基准——与 triadica 的差异点）：重阳木 slot-0 锚 ≈9.90m 高
 *      （涌现 bbox 实测 9.8960，T011.8 探针——≈10m 长江流域公园夏绿中龄个体锚 = 主代
 *      理裁定 per 终审裁决 1；8 槽带 8.78–10.73、slot-1 速生窄高端最高 10.73——速生
 *      上探 12 变体入 slot-1），视觉冠底/实高 0.1775（叶卡最低质心 Y 1.7569m，T009.3
 *      口径——bischofiaShapeSlots 同款测量）→ 冠域 1.76–9.90 → 视心取**冠心 ≈5.8**
 *      （(1.7569+9.8960)/2 ≈ 5.826；叶量加权质心实测 P50 ≈3.87 仅参考——伞形-开展
 *      圆头冠叶幕偏下，全树取景沿家族冠域中点口径），vs platanus 7.5（12m 级上层大
 *      乔）/ koelreuteria 5.9 / triadica 5.8 / camphor 5.5 / ginkgo 5.4 / zelkova
 *      5.3 / celtis 4.3——重阳木 ≈9.9m 级中量级（与栾树 ≈9.85/乌桕 ≈9.5 同带），冠心
 *      与乌桕同档 5.8；stats 锚点实数 = slot-0 High 组 0 皮 24178（皮拓扑恒 24178
 *      ——主干 14 段×14 + 底盖 14 + 五级 7/21/63/189/378 枝 + 领导拓扑；**无花果资产
 *      组 0 = 纯皮拓扑**，vs triadica 组 0 = 皮 24178 + 果序 672 并入 = 24850——花果
 *      不做终审 ③-2，无附加块材质接口）/ 复叶卡 5466 tri / 2733 卡（资产预算锁定账
 *      目：triangleCount 29644 = 24178 + 5466；复叶卡 = 2 tri/卡承载**整枚三出复叶**
 *      ——vs 乌桕中卡承载单枚菱形叶，卡语义不同、tri/卡同 2；本模块不感知组内细节，
 *      归资产侧测试）。
 * 011.3 缺口 D 评估结论（T011.8 DEV Step 记档，归 011.13）：重阳木树皮 = **褐-深灰褐
 *      纵裂深沟宽脊 + 裂纹扭转**（非榉/悬铃木满斑门控型）——材质侧裂线游走低频场 +
 *      8 宽脊/周深沟剖面 + 沟内暖暗 AO 为皮域**全程有效**（uv 驱动全 v 轴、无高度门
 *      控）；干上部弱化 smoothstep(3.0, 5.4) 为**连续过渡**（沟深连续弱化至浅沟——
 *      3.0/5.4 为材质侧待同步的冠基 2.9m 占位锚，实测冠底 1.757/顶 9.896，同步编辑归
 *      park-shader-agent，判定只依赖连续 vs 限带的结构性事实、不依赖常数值）；老干
 *      交叉网状为 High 专属近景**次级微暗层**（×0.87 微暗调制、非身份主体；三重门控
 *      = tone 噪声域局部片门 × 低位高度门 (1−smoothstep(1.6, 3.0)) × 低弧长 v 门——
 *      高度限带项但该低位带本身在冠心机位（视心 5.8、M25 8° 俯角 25m）画面下部覆盖
 *      内（画面含干基 0–3m 带），拍得到）；干基暗化 0.5–2.4m 为低调 tone 项（家族惯
 *      例）；细枝绿-绿褐过渡 4.4–7.8m 为高位**连续过渡**（冠缘、机位天然覆盖）+ 皮孔
 *      两档为细枝域高位近景身份点（细枝域门控非低位限带）；几何浅浮雕 = 亚视觉地板
 *      抬档 3.5mm 的**起径门控**（起径×幅度比 < 0.0035 的管平滑发射——半径门控非高
 *      度门控，**仅主干有效起伏、全部分枝管光滑**（「细枝红褐-灰褐较光滑带皮孔」—
 *      —皮孔/色序归材质层），主干 + 底盖满段有效起伏）→ 冠心取景机位（视心 5.8）可
 *      拍全干褐纵裂深沟宽脊扭转身份（下半干深沟最强 + 低位老干网状 + 干基暗化，上半
 *      干连续过渡细枝绿收敛），**Stage 低目标机位能力本轮不补**（缺口 D 维持榉树单
 *      例，归族门 011.13 收口）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_bischofia.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createBischofiaLeafDepthMaterial } from './bischofiaMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface BischofiaStageDeps {
  ...
  /** 构建函数注入位（缺省 = 重阳木 asset build；测试注桩 fake build——透传断言不依赖真实
   *  level 路由落地时序；产品路径 bootstrap 不传，行为不变） */
  build?: ProceduralBuild;
}

/** mountSlots 模式逐槽账目（slot 序号 + 该槽组 0 皮/叶三角与叶卡数——无花果资产组 0 = 纯皮拓扑） */
export interface BischofiaSlotStats {
  slot: number;
...
export interface BischofiaLevelStats {
...
/** window.__bischofia 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface BischofiaHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'bischofia-dev-slots' 组
   *  直挂 scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；展开槽冠幅最大
   *  ≈9.64m 防交叠）；每 Mesh castShadow + customDepthMaterial（与 mount 单树同待遇）；
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：同槽冠幅 XZ 最大 ≈9.64m 防交叠）；每档
   ...
   *  自建 createBischofiaLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
   ...
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+9.64 ≈
   *  42.6m、纵深 ≈20m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨且留边（垂直半角需求 ≈8°
   *  < 典型 fov 25° 余量；P42 为全族固定口径，沿 tree3a/celtis/camphor/zelkova/ginkgo/
   *  platanus/koelreuteria/triadica——42m 全景机位与网格几何绑定，不随冠幅缩距；celtis
   *  横跨 ≈42.9 / 栾树 42.89 / 乌桕 42.57 先例同沿 42，重阳木 42.64 同位续沿） */
   ...
   *  缺省 distance 32 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 9.64
   *  ≈ 31.6m（同槽冠幅 XZ 最大 ≈9.64m = 8 槽实测带 max，T011.8 探针；与乌桕 31.6 同值
   *  ——档间不放大排距需求），32m 距离 35° 斜视 16° 俯角下三棵全入画可辨且留边（对齐
   *  viewSlots「distance ≈ 实宽」换算口径上取留边——沿 zelkova 30.5→32 / celtis 32 /
   *  platanus 32 / koelreuteria 32 / triadica 32 同族定档法；重阳木树高带 8.78–10.73
   *  不超 platanus 需求，32 同位定档；垂直半角需求 ≈11° 同口径余量内） */
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；展开槽冠幅最大 ≈9.64m（8 槽实测带 max
 *  = slot-2 幼龄开张端 w/h 0.992，T011.8 探针实测；meta widthRange max 9.7 为声明带），
 *  11m 留 ≈1.4m 防交叠（P42 全族固定口径沿 tree3a/celtis/camphor/zelkova/ginkgo/
 *  platanus/koelreuteria/triadica——celtis 9.9m / 栾树 9.89m / 乌桕 9.57m 冠幅同距先例；
 *  42m 全景机位与网格几何绑定，不随冠幅缩距） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（同槽冠幅 XZ 最大 ≈9.64m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 9.64 ≈ 31.6m，「distance ≈ 实宽」
 *  口径上取留边（沿 zelkova 30.5→32 / celtis 32 / platanus 32 / koelreuteria 31.9→32 /
 *  triadica 31.6→32 同族定档法，见接口注释） */
const LEVELS_VIEW_DISTANCE = 32;
/** 取景视心高（米）——slot-0 冠心（冠域 1.76–9.90 → ≈5.826 取 5.8；vs platanus 7.5 /
 *  koelreuteria 5.9 / triadica 5.8 / camphor 5.5 / ginkgo 5.4 / zelkova 5.3 / celtis
 *  4.3，与乌桕同档——≈9.9m 级中量级，见模块头「树高参考」） */
const VIEW_TARGET_Y = 5.8;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createBischofiaHandle(deps: BischofiaStageDeps): BischofiaHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省重阳木 asset build 行为不变） */
   ...
   *  celtis/camphor/zelkova/ginkgo/platanus/koelreuteria/triadica Stage 度→弧度换算同款）：
  ...
  return {
    mount(opts = {}) {
      ...
      group.name = 'bischofia-dev-stage';
      ...
      if (sourceDepth) {
        ...
        leafDepth = createBischofiaLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createBischofiaLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'bischofia-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'bischofia-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createBischofiaLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[bischofiaStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'bischofia-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createBischofiaLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（9.90m 锚点树冠域 1.76–9.90 → ~5.8m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[bischofiaStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[bischofiaStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: BischofiaLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: BischofiaSlotStats[] = slotsSources.map((tree, slot) => {
