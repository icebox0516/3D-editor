/**
 * runtime/procedural/tree/ginkgo/ginkgoStage —— window.__ginkgo DEV 出图面（T011.4；
 * 榉树 zelkovaStage 同构复制——T008.2/T008.3/T009.3/T009.5/T009.6/T011.1/T011.2/T011.3
 * 能力面一次交付；方法零新面，数值面换银杏实数）。
 *
 ...
 *      celtis/camphor/zelkova Stage 同款 = docs/procedural-assets/shadow-visual-sop.md §3：
 ...
 *      相位摆动，D19.7；银杏叶/皮材质同款消费实例 aSeed——整树缓摆同公式同相位）；
 *      mesh 挂 customDepthMaterial（叶影 SDF 裁切——T011.3 起 build 产物自带
 ...
 *      SOP §3 P42 口径；展开槽冠幅最大 ≈5.89m 防交叠）挂 slots 组直挂 scene 兄弟层；
 *      viewSlots 全景 / viewSlot(i) 单槽特写固定机位（球坐标同 view 语义——「批量
 *      出图 → 分轮裁定」的取证载体）；stats 扩逐槽账目（皮面数槽间恒等 20782 = 拓扑
 ...
 *      createGinkgoLeafDepthMaterial(level)）；viewLevels 三树全景 / viewLevel(level)
 *      单档特写（球坐标同 view 语义）；stats 扩 levels 逐档账目。
 *      deps.build 注入位 = 测试 seam（缺省银杏 asset build，产品路径不变——档位透传
 *      断言不依赖真实 level 路由落地时序）。
 * 边界：DEV 专用（组合根 bootstrap import.meta.env.DEV 守卫挂 window.__ginkgo，
 *      生产零痕迹——本模块与 tree3a/celtis/camphor/zelkova Stage 同为纯工厂，window 装配归
 ...
 * 树高参考（机位 target y 基准——与 zelkova 的差异点）：银杏 slot-0 锚 ≈8.17m 高
 *      （bbox 实测 8.17，T011.4 探针——≈8m 中龄公园个体工程锚弱推断 per 终审 C-1），
 *      视觉冠底/实高 0.328（ginkgoShapeProfile 挂高段探针回推——干高工程默认域无实证
 *      per 终审 C-3）→ 冠域 2.68–8.17 → 视心取**冠心 ≈5.4**（(2.68+8.17)/2 ≈ 5.42），
 *      vs zelkova 5.3（榉树冠域 2.74–7.96 冠心 5.35）/ camphor 5.5（香樟冠域 2.47–8.58
 *      冠心 5.52）/ celtis 4.3（朴树树半高口径）——银杏整树高于榉树 ≈0.2m 而冠底近似
 *      （2.68 vs 2.74），冠心随之上移 ≈0.1m；stats 锚点实数 = slot-0 High 皮 20782 /
 *      叶 9676 / 4838 卡（资产预算锁定账目：triangleCount 30458 = 20782 + 4838×2，
 *      莲座+散生长短枝双挂点合计卡数——本模块不感知长短枝账目分离，归资产侧测试）。
 * 011.3 缺口 D 评估结论（T011.4 Step 3 记档，归 011.13）：银杏树皮 = **全干型**——
 *      纵裂脊沟材质配方整干有效（ginkgoMaterials GINKGO_BARK_RIDGE 三档共用、无高度
 *      带门控；干上部 2.2–4.6m smoothstep 仅为裂深渐弱 + 淡褐黄偏色的**连续过渡**，
 *      非榉树「满斑区 <2.2m」式限带特征）+ 几何微起伏主干整段有效（亚视觉地板为
 *      起径门控非高度门控）→ 冠心取景机位可拍全干纵裂身份，**Stage 低目标机位能力
 *      本轮不补**（缺口 D 维持榉树单例，归族门 011.13 收口）。
 */
import * as THREE from 'three';
import { build, meta } from '../../assets/asset_tree_ginkgo.asset';
import { morphSeedOf } from '../../../../domain/assets';
import type { ProceduralLevel } from '../../../../domain/assets';
import type { ProceduralBuild } from '../../types';
import { createGinkgoLeafDepthMaterial } from './ginkgoMaterials';
import type { InstanceSource } from '../../../instancing/InstancedAssetPool';

/** 取景依赖：主相机 + 轨道控制目标（结构类型——bootstrap 注入 renderer 实件，测试可注桩） */
export interface GinkgoStageDeps {
...
export interface GinkgoLevelStats {
...
/** window.__ginkgo 句柄（类型在 runtime，bootstrap 经 import type 声明 window 槽） */
export interface GinkgoHandle {
   ...
   *  独立 Mesh（跨槽几何各异——InstancedMesh 不适用）挂一个 'ginkgo-dev-slots' 组直挂
   *  scene 兄弟层；4×2 行主序网格（slot 0–3 前排 z=0、slot 4–7 后排 z=+spacing；
   *  x=(i%4−1.5)×spacing）；spacing 缺省 11m（SOP §3 P42 口径；展开槽冠幅最大
   *  ≈5.89m 防交叠）；每 Mesh castShadow + customDepthMaterial（与 mount 单树同待遇）；
   ...
   *  （沿用 SLOTS_DEFAULT_SPACING 依据：同槽冠幅 XZ 最大 ≈5.89m 防交叠）；每档
   ...
   *  自建 createGinkgoLeafDepthMaterial(level)）；与 mount/mountWindDemo/mountSlots
   *  互斥（先 unmount 再建） */
  ...
  /** 8 槽全景固定机位：目标 = 网格中心（组位 + (0, 5.4, spacing/2)）；缺省 distance 42
   *  （SOP P42）/ azimuth 35 / elevation 16——4×2×11m 网格含冠幅实宽 ≈3×11+5.89 ≈ 38.9m、
   *  纵深 ≈19m，42m 距离 35° 斜视 16° 俯角下 8 棵可辨且留边（垂直半角需求 ≈10° <
   *  典型 fov 25° 余量；P42 为全族固定口径，沿 tree3a/celtis/camphor/zelkova） */
  viewSlots(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单槽特写机位：球坐标绕该槽树位（复用 view 公式，目标 = 该槽 x/z、视心高 ≈5.4）；
  ...
  /** 三树全景固定机位：目标 = 排中心（组位，三树 −s/0/+s 几何中心即组位）视心高 ≈5.4；
   *  缺省 distance 29 / azimuth 35 / elevation 16——三树一字排横向总跨 = 2×11 + 冠幅 5.89
   *  ≈ 27.9m（同槽冠幅 XZ 最大 ≈5.89m，T011.4 探针实测带 widthRange max），29m 距离
   *  35° 斜视 16° 俯角下三棵全入画可辨且留边（对齐 viewSlots「distance ≈ 实宽」换算
   *  口径上取留边；垂直半角需求 ≈6° 同口径余量内） */
  viewLevels(opts?: { distance?: number; azimuthDeg?: number; elevationDeg?: number }): void;
  /** 单档特写机位（viewSlot 的档位版）：球坐标绕该档树位（复用 view 公式，目标 = 该档
   *  x/z、视心高 ≈5.4）；缺省 distance 25 / azimuth 35 / elevation 8（同 view/viewSlot）；
...
/** slots 缺省间距（米）——SOP §3 P42 网格口径；展开槽冠幅最大 ≈5.89m（widthRange max
 *  5.89，T011.4 探针实测带），11m 留 ≈5.1m 防交叠（P42 全族固定口径沿 tree3a/celtis/
 *  camphor/zelkova——42m 全景机位与网格几何绑定，不随冠幅缩距） */
const SLOTS_DEFAULT_SPACING = 11;
/** LOD 三档排布与账目序（mountLevels：high 左 / mid 中 / low 右） */
const LEVEL_ORDER: ProceduralLevel[] = ['high', 'mid', 'low'];
/** levels 缺省间距（米）——沿用 SLOTS_DEFAULT_SPACING 防交叠依据（同槽冠幅 XZ 最大 ≈5.89m） */
const LEVELS_DEFAULT_SPACING = SLOTS_DEFAULT_SPACING;
/** viewLevels 缺省距离（米）——三树横向总跨 = 2×11 + 5.89 ≈ 27.9m，「distance ≈ 实宽」口径
 *  上取留边（沿 P42 网格口径同族量级，见接口注释） */
const LEVELS_VIEW_DISTANCE = 29;
/** 取景视心高（米）——slot-0 冠心（冠域 2.68–8.17 → ≈5.42 取 5.4；vs zelkova 5.3 /
 *  camphor 5.5 / celtis 4.3，见模块头「树高参考」） */
const VIEW_TARGET_Y = 5.4;

/** 句柄工厂：资源全封闭于闭包，句柄间零共享 */
export function createGinkgoHandle(deps: GinkgoStageDeps): GinkgoHandle {
  ...
  /** 构建函数（deps.build 注入位——测试 seam；缺省银杏 asset build 行为不变） */
   ...
   *  camphor/zelkova Stage 度→弧度换算同款）：绕 (tx, targetY, tz) 以方位/仰角定距放相机 */
  const placeCamera = (
    tx: number,
    tz: number,
    targetY: number,
  ...
  return {
    mount(opts = {}) {
      ...
      if (sourceDepth) {
        ...
        leafDepth = createGinkgoLeafDepthMaterial();
    ...
    mountWindDemo(count = 3) {
      ...
      if (windSourceDepth) {
        ...
        windLeafDepth = createGinkgoLeafDepthMaterial();
        mesh.customDepthMaterial = windLeafDepth;
      }
      windGroup = new THREE.Group();
      windGroup.name = 'ginkgo-dev-wind';
      windGroup.add(mesh);
    ...
    mountSlots(opts = {}) {
      ...
      slotsGroup.name = 'ginkgo-dev-slots';
      for (let slot = 0; slot < SLOT_COUNT; slot++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createGinkgoLeafDepthMaterial();
    ...
    mountLevels(opts = {}) {
      ...
      if (slot < 0 || slot >= SLOT_COUNT) {
        ...
        console.warn(`[ginkgoStage] mountLevels: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      this.unmount();
      levelsSpacing = opts.spacing ?? LEVELS_DEFAULT_SPACING;
      levelsGroup = new THREE.Group();
      levelsGroup.name = 'ginkgo-dev-levels';
      for (let index = 0; index < LEVEL_ORDER.length; index++) {
        ...
        if (sourceDepth) {
          ...
          const depth = createGinkgoLeafDepthMaterial(level);
    ...
    view(opts = {}) {
      // 目标高 ≈ 冠心（8.17m 锚点树冠域 2.68–8.17 → ~5.4m 视心，见模块头「树高参考」；树未挂时回退原点）
    ...
    viewSlot(slot, opts = {}) {
      if (slot < 0 || slot >= SLOT_COUNT) {
        console.warn(`[ginkgoStage] viewSlot: slot ${slot} 越界（0..${SLOT_COUNT - 1}）——no-op`);
        return;
      }
      const gx = slotsGroup?.position.x ?? 0;
      const gz = slotsGroup?.position.z ?? 0;
    ...
    viewLevel(level, opts = {}) {
      ...
      if (index < 0) {
        console.warn(`[ginkgoStage] viewLevel: 未知档位 ${String(level)}（high|mid|low）——no-op`);
        return;
      }
      const gx = levelsGroup?.position.x ?? 0;
      const gz = levelsGroup?.position.z ?? 0;
    ...
    stats() {
      if (levelsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const levels: GinkgoLevelStats[] = levelsSources.map((tree, index) => {
      ...
      if (slotsSources.length > 0) {
        let bark = 0;
        let leaf = 0;
        const slots: GinkgoSlotStats[] = slotsSources.map((tree, slot) => {
