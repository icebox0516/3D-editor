 ...
 *      中龄公园水边个体锚 ≈10m（**8 槽带 8.43–11.22m 高 / 10.15–12.80m 宽**，
 *      proceduralProfile 全槽实测带，2026-09-23 续作会话终测）；**Step 3 探针回调
 *      记档（六轮，2026-09-23）**：①初值树高域 9.4–10.0 实测涌现 8.97–12.81 跨域
 ...
 *      簇 × 2）下沿的补偿回调）。原点 = 底部中心 minY 精确 0（叶卡极值斜伸的
 *      ≤8cm 穿地允许带 rawMinY ≥ −0.08——终测 8 槽 −0.01～−0.06）；**VIEW_TARGET_Y
 *      参考（3c Stage 消费）：slot-0 High 档整体包围盒中心 4.92（≈0.50h——垂枝
 *      冠心偏低读向）/ 叶组中心 5.83**。回调记档详见 profile 各槽注释。
 ...
 *      （2026-09-23 续作会话终测，规范种子 = morphSeedOf(id, slot)；探针六轮回调
 *      后定档）：High 总面 **33968–36532**（slot-6 斜弯最低 / slot-1 深垂帘最高；
 *      单叶卡 3894–5176 × 2、保留簇 587–752 / 簇位 945）；Mid 总面 **8206–9050**
 *      （单叶卡 1279–1701 ≈ High 存活卡 × 0.31——L5 簇 9 选 3 + L4 簇 10 选 3 掩码
 *      率 j%3===1；皮 5648）；Low 总面 **1544–1874**（**壳卡 = 保留簇 × 1 单竖卡
 *      （垂柳分化：簇位沿索拉长 → 保留簇 587–752 级（vs 先例 ~474），双竖卡口径越
 *      Low 预算上沿——单竖卡落带记档；宽轴 = 0.077 × 半幅 = 卡长宽比域 8–18 中位
 *      13 的倒数延续——垂帘窄竖剪影）× 2 tri + 皮 370**）；皮恒 **26180 / 5648 /
 *      370**（High/Mid/Low——主干 14 段 ×14 + 底盖 14 + 五级 7/21/63/189/378 枝，
 *      拓扑 6 骨架 + 1 领导；Mid L4 = 3402 账目勘误记档：初记 2268 为段数笔误）；
 *      rng 消费 slot-0 三档恒等 **85485**（快照锁）、跨槽随保留簇数浮动（通透 roll
 *      计数机制，先例同款）；minY 三档恒 0；8 槽 w/h 涌现带 **1.046–1.269**
 *      （slot-0 锚点 1.140 落 1.0–1.3 主档中带 / slot-1 深垂帘窄端 1.046 + slot-6
 *      斜弯宽端 1.269 = Spec 域两端身份）；树高带 **8.43–11.22**（slot-0 9.83 ≈10
 *      锚）。
 */
...
export const meta: ProceduralAssetMeta = {
  ...
  triangleCount: 35250, // 实数 = slot-0 锚点 High 档结构实数（皮 26180 恒定 + 单叶卡 4535×2；多档起声明面取细模档；见模块头预算锁定账目）
  ...
  proceduralProfile: {
    // 跨 8 槽细模包围盒实测带（T011.12 终测 2026-09-23：h 8.43–11.22 / w 10.15–12.80，
    // 声明带外沿放宽——实测带全含）。
